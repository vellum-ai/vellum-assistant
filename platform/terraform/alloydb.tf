...
locals {
  ...
  # Construct the DATABASE_URL in PostgreSQL format
  database_url = "postgresql://${var.db_user}:${local.db_password}@${google_alloydb_instance.primary.ip_address}:5432/${var.db_name}?sslmode=require"
}
...
# Grant the GitHub Actions service account access to read the secret
...
resource "google_secret_manager_secret_iam_member" "gke_workload_secret_accessor" {
  secret_id = google_secret_manager_secret.database_url.id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${var.project_id}.svc.id.goog[${var.cluster_name}/default]"
}
