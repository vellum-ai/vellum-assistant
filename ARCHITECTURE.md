...
- **One process per conversation turn.** Agent turns run in the daemon and in the schedule worker, so `conversations.processing_started_at` with `processing_pid` is an atomic claim (`persistence/processing-claim.ts`): a turn start claims the row or is refused, a holder that is dead, has no pid, or is older than the lease ceiling is taken over, and a release clears only its own claim. A refused claim answers busy: the fenced acquire returns null, a wake skips with `busy` and the scheduler retries it, and the send route queues the message. A worker's release notifies the daemon over IPC (`notify_conversation_released_externally`), which marks its resident history stale and drains the queue.
