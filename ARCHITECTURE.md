...
The assistant Dockerfile installs the X server, window manager, dock, compositor, clipboard bridge, wallpaper setter (`feh`), fonts, Chrome libraries and pinned WezTerm terminal directly. Architecture-specific SHA-256 checksums verify the WezTerm download. The dependency layer precedes Bun and application source so application and Bun updates reuse it. `desktop-dependencies.ts` checks binaries and system assets before preparing Chrome; missing image components fail setup rather than installing system packages at runtime. Google Chrome is an exact-version, SHA-256-verified download for Linux x64 or ARM64, extracted under `external/desktop` on the persistent workspace. Extraction does not run Chrome package scripts, register its repository or change the system's default browser. Successful setup is recorded only after Chrome runs. Container replacement reuses the persisted Chrome installation and profile alongside the baked system packages. Configured `NODE_EXTRA_CA_CERTS` are combined with the system CA bundle for runtime downloads without changing system trust. Chrome installation errors remain retryable. Existing browser-tool and PDF installations retain their own Playwright behavior.
