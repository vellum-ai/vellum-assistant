...
Eligible web guardian turns preactivate the existing `computer-use` skill. Untargeted `computer_use_*` calls route through `surfaceProxyResolver` to a screenshot-only X11 executor on the streamed display. Explicit personal-computer targets and native app turns keep the host proxy path. Shared `HostCuProxy` action history, step budget, observation formatting and transcript image handling apply to both backends. The executor uses `xdotool` and `scrot`, installed by the desktop setup job, and feeds JPEG observations into the existing formatter. Unsupported native-only operations fail before input is sent.

Computer use and browser automation share `DesktopAutomationLease`, including setup, guardian ownership, serialization, cancellation, human-help reservations, idle cleanup and activity notifications. Terminal CU tools release the lease. Sequence input is validated before any action runs, and a cancelled drag releases its mouse button.
