...
The browser scope packages the existing Chrome extension dispatcher as a distinct managed client. On desktop startup, the assistant signs a versioned CRX3 with an installation key under the protected directory outside the workspace and writes Linux Chrome policy and a native-messaging host manifest. Chrome keeps its existing desktop profile and authenticates the package by signature. The extension has no cloud sign-in, personal-browser registration, or remote debugging port. Its native host keeps a process-scoped capability outside the workspace and exchanges commands through the gateway's `/v1/desktop/browser/bridge`. The gateway stamps the bound guardian; the runtime checks capability, connection generation, and the lease owner's actor before dispatch. This channel never registers `host_browser` with the general client roster.
