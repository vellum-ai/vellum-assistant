...
## Experimental combined sandbox image

[`docker/combined/README.md`](docker/combined/README.md) describes an opt-in single-image runtime for the assistant, gateway, and CES. A supervisor starts the direct processes in readiness order under Tini, shares the existing IPC sockets, and persists state under one mounted volume. An optional adapter adds Blaxel's sandbox-api. Existing separate-image deployments are unchanged. This mode combines the services' filesystem trust boundary and aggregate resource budget; it does not preserve the production six-volume access isolation or enforce per-service CPU/memory limits.
