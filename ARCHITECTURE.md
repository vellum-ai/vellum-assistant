...
The browser scope packages the existing Chrome extension dispatcher as a distinct managed client. On desktop startup, the assistant sends the built ZIP over gateway IPC. The gateway signs a versioned CRX3 with an installation key in its durable security volume and returns a scoped native capability. The assistant writes Linux Chrome policy and a native-messaging host manifest. Chrome keeps its existing desktop profile and authenticates the package by signature. The extension has no cloud sign-in, personal-browser registration, or remote debugging port. Its native host keeps a process-scoped capability outside the workspace and exchanges commands through the gateway's `/v1/desktop/browser/bridge`. The gateway checks the capability against its persisted hash before stamping the bound guardian; the runtime checks capability, connection generation, and the lease owner's actor before dispatch. This channel never registers `host_browser` with the general client roster.

Browser observations return bounded AX controls and reading text. References bind to connection generation, tab, frame, and document loader, and expire on navigation, reconnect, takeover, or X11 input. Clicks validate enabled, visible, stable, unobstructed elements before sending input. Frames support reading; embedded-frame input, native UI, and canvas use screenshot/X11 fallback. Cancellation discards unsent work, cancels in-flight result delivery, and releases held browser input. Commands Chrome has received cannot be recalled and are never automatically retried. See [managed desktop extension](assistant/docs/desktop-browser-extension.md) for provisioning and validation.
