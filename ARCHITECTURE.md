...
| macOS Apple Containers runtime | [`clients/macos/README.md`](clients/macos/README.md#apple-containers-runtime-optional) |
...
- **Apple Containers local runtime (macOS 15+, feature-flagged):** The macOS app can orchestrate the Vellum three-service stack (assistant, gateway, credential-executor) directly in Swift using Apple's `Containerization` framework (`LinuxPod` API) instead of shelling out to Docker or the `container` CLI. Orchestration is split across two modules: the main app target (macOS 14+) handles availability gating, launcher logic, onboarding, and the lockfile write; the optional `AppleContainersRuntime.framework` (macOS 15+, dlopen-loaded at runtime) owns all `Containerization` API calls and the Kata kernel/vminitd image store. All three OCI workloads (assistant, gateway, CES) run inside one `Virtualization.framework` VM; the macOS app never calls `container`, `docker`, or any external tool. The feature is gated by `apple_containers_enabled` (`scope: macos`, default off) and by a three-step availability check: feature flag → OS version ≥ 15.0 → runtime module load. Env override: `VELLUM_FLAG_APPLE_CONTAINERS_ENABLED=1`. Lockfile entries written by this path carry `runtimeBackend: "apple-containers"`. See [`clients/macos/README.md`](clients/macos/README.md#apple-containers-runtime-optional) for build requirements and rollout controls.
