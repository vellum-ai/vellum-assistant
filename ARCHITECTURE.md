...
Eligible guardian turns preactivate the existing `computer-use` skill. Calls with `target: "assistant-desktop"` route through `surfaceProxyResolver` to a screenshot-only X11 executor on the streamed display. Omitted targets and `target: "connected-computer"` keep the host proxy path. One target selector drives dispatch and invocation-specific permission routing for the bundled skill, with no fallback between computers. Shared `HostCuProxy` action history, step budget, observation formatting and transcript image handling apply to both backends. The executor uses `xdotool` and `scrot`, installed by the desktop setup job, and feeds JPEG observations into the existing formatter. Unsupported native-only operations fail before input is sent.
