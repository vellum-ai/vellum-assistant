...
A containerized assistant can serve an interactive desktop on demand: one `Xtigervnc` X server (display `:99`, built-in VNC server on `localhost:5999`, `-localhost -SecurityTypes None`), `openbox`, the `xcompmgr` compositor, a `tint2` dock, the `tigervncconfig -nowin` clipboard bridge (shipped by `tigervnc-common` on Debian trixie), and Playwright's Chromium with its own profile under `$VELLUM_WORKSPACE_DIR/data/desktop-profile`, launched at the X server's geometry (`--window-position` / `--window-size`) so its window does not depend on openbox being up to maximize it. The image bakes Chromium into `/opt/ms-playwright` (`PLAYWRIGHT_BROWSERS_PATH`) in a layer before the source copies, and installs `xfonts-base` for the `fixed` font Xvnc needs; the on-demand `ensureChromium` install in `tools/browser/runtime-check.ts` is a single-flight fallback shared with the browser tools. The dock is generated, not baked: `desktop-panel-config.ts` writes a `tint2rc` plus two `.desktop` launchers and their icons under `$VELLUM_WORKSPACE_DIR/data/desktop-panel` at start and the panel runs as `tint2 -c <that file>`, because one launcher points at Playwright's Chromium and that path is only known at runtime. It reads as a macOS-style dock (`panel_position = bottom center horizontal`, `panel_shrink = 1` so it floats at its content width instead of spanning, a rounded translucent background, `panel_layer = top`, `strut_policy = follow_size`, icon-only launchers and window buttons, no clock or tray) and pins Chromium, launched on the same profile as the desktop's own browser so it opens a window in that instance, and `xterm`. The launcher icons are PNGs written beside the config, since the image carries no icon theme for an `Icon=` name to resolve against, and every key in the generated file is a documented tint2 option: tint2 ignores unknown keys in silence, which would quietly restore the stock panel look. `xcompmgr` is what makes those rounded corners and that translucency real: tint2 only gets an ARGB visual when a compositor is already running, and without one the same config renders as an opaque square strip, so the compositor is spawned right after `openbox` and before the dock. The dock itself comes up with Chromium rather than with the rest of the tree, since it needs that resolved path and both the window manager and the compositor are long up by then. The children get a minimal allowlisted env (`PATH`, `HOME`, locale, `XDG_RUNTIME_DIR`, `TMPDIR`, `PLAYWRIGHT_BROWSERS_PATH`, `DISPLAY`) rather than `buildSanitizedEnv()`, whose kata chroot `PATH` and `LD_LIBRARY_PATH` overlay would give this Chromium different libraries than the browser tool's. `DesktopSessionManager` (`assistant/src/desktop/desktop-session-manager.ts`) owns the tree: it resolves all six binaries (`Xtigervnc`, `openbox`, `xcompmgr`, `tint2`, the clipboard bridge, `xterm`) with `Bun.which` before spawning anything, the first viewer starts it, one viewer holds the slot at a time, the tree lingers five minutes after the last viewer leaves so a reconnect is instant, and it is torn down on that expiry, when the X server, window manager or clipboard bridge exits, when Chromium cannot be installed, when a start fails partway (everything already spawned is killed so a retry begins clean), or by `RuntimeHttpServer.stop()`, which latches the shared manager closed even on a daemon that never served a desktop, so a socket arriving mid-shutdown cannot start a tree nothing would kill. A kill is SIGTERM, a two second grace, then SIGKILL of whatever is left; the viewer hears about a loss before the kill starts, and a start awaits any kill still in its grace before spawning, so a reconnect during that window never puts a second X server on a display and port the dying one still holds. The dock and the compositor are cosmetic children: a config that cannot be written, a spawn that fails and an exit are all logged and left alone, since a squarer or missing dock is not worth tearing a working desktop down for, and teardown still kills them. A browser exit with nobody watching only costs the next viewer a fresh window; under a viewer Chromium is relaunched, and more than three exits within a minute fail the session. Gated by the `assistant-desktop` flag and `IS_CONTAINERIZED` (`desktop-feature.ts`), so a daemon without the X binaries never advertises it.
