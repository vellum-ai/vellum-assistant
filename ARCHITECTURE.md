...
On Linux containers with writable cgroup v2 CPU delegation, the desktop process tree shares one threaded CPU group capped at 80% of the detected container CPU capacity, further bounded by CPU affinity and visible ancestor quotas. Each child joins before exec, so Chrome renderers and dock-launched applications inherit the same aggregate budget. The assistant stays outside that group. CPU preparation uses asynchronous filesystem operations and runs before desktop startup; incompatible or read-only hierarchies log a warning and retain uncapped desktop operation. The empty group is reused across desktop sessions, with its quota recalculated at startup. Memory limits and container-wide allocations are unchanged.
