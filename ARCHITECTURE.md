...
| A2A (assistant-to-assistant) | [`assistant/docs/architecture/a2a.md`](assistant/docs/architecture/a2a.md) |
...
- **A2A (assistant-to-assistant)** communication uses the `"vellum"` channel with `RuntimeInboundPayload`, not a new channel type. All A2A ingress goes through the gateway webhook (`/webhook/a2a`); outbound delivery uses `/deliver/a2a` with query-param routing context. Pairing envelopes are intercepted before the conversation pipeline by the A2A interceptor. `conversationExternalId` is server-derived by the gateway. The feature is gated behind `feature_flags.assistant-a2a.enabled` (default: off). See [`assistant/docs/architecture/a2a.md`](assistant/docs/architecture/a2a.md).
...
subgraph "Text Q&A Session"
    ...
    subgraph "Gateway (Bun + TypeScript)"
        ...
        GW_A2A_WEBHOOK["A2A Webhook<br/>/webhook/a2a<br/>(per-envelope auth)"]
        GW_A2A_DELIVER["A2A Deliver<br/>/deliver/a2a<br/>(internal, from runtime)"]
    ...
    %% Gateway flow — A2A channel (assistant-to-assistant)
    GW_A2A_WEBHOOK -->|"parseA2AEnvelope<br/>+ per-envelope auth<br/>+ forwardToRuntime"| GW_FORWARD
    HTTP_RT -->|"POST /deliver/a2a<br/>(via gatewayInternalBaseUrl)"| GW_A2A_DELIVER
    GW_A2A_DELIVER -->|"POST /webhook/a2a<br/>(target gateway)"| GW_A2A_WEBHOOK
