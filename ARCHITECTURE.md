...
Human help requests use `ask_question({ desktopHelp })` and the existing question interaction lifecycle. The assistant releases browser control, and the web question slot hosts the shared virtual desktop preview with Step In, Done and Skip. The optional question presentation survives pending-interaction recovery; older clients render ordinary question options. Done requests a fresh browser observation, while Skip leaves the obstacle unresolved.
