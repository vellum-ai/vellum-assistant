...
An enabled platform-hosted assistant serves an interactive desktop on demand. Current images bake Chrome and desktop system components into cached layers. Assistants on older images install missing components through one shared background setup job. Opening the picture-in-picture viewer starts setup and displays progress; browser commands and direct streams join the same job. Cancellation or feature revocation prevents a delayed action without cancelling setup for other viewers. `GET /v1/desktop/setup` reads readiness without installing; POST starts or joins setup. Both flat and assistant-scoped gateway paths require guardian authentication. Setup publishes `assistant:self:desktop` sync invalidations, and reconnects refetch status. Ready images start directly without installation. Readiness also reports optional `automationActive`; lease acquisition and release publish `desktop_activity_changed` so the desktop icon reflects active automation.

The Dockerfile and compatibility installer share package names, versions and architecture-specific SHA-256 checksums in `assistant/src/desktop/desktop-packages.json`. Chrome resolves to `/opt/google/chrome/chrome` when present, otherwise to the verified workspace installation under `external/desktop/chrome-<version>`. Workspace Chrome requires its installation completion marker. The installer combines the system CA bundle with the configured extra CA for downloads, preserves existing packages, and extracts Chrome without registering its repository. The browser profile remains at `data/desktop-profile` on persistent storage, so both install paths preserve sessions. Existing browser-tool and PDF installations retain their own Playwright behavior.
