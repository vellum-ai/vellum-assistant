...
An enabled platform-hosted assistant can serve an interactive desktop on demand. Opening the virtual desktop picture-in-picture view automatically prepares Google Chrome on the persistent workspace; desktop system packages are included in the assistant image. The first eligible browser command waits for the same installation job and continues its original action once ready. Setup progress uses the same sync invalidations as the viewer. Cancellation or feature revocation prevents a delayed action. Browser CLI requests opt into IPC cancellation on disconnect so timed-out commands cannot act after setup finishes. Authenticated direct stream requests also start or join the same background setup for clients without the setup UI. A client that times out during installation can reconnect after setup finishes; a disconnected viewer does not start a desktop process tree. `GET /v1/desktop/setup` checks readiness without installing anything; `POST` starts one shared background installation. Both flat and assistant-scoped gateway paths require guardian authentication and proxy to a gateway-service-only runtime route. `assistant:self:desktop` sync invalidations refresh setup status as installation progresses, and a reopened modal or reconnected event stream refetches it. Older assistants returning 404 retain the direct streaming flow. Setup status also reports optional `automationActive` from the browser lease. Lease acquisition and abort publish domain-specific `desktop_activity_changed` events so the header icon pulses in the assistant's avatar accent color while automation owns the desktop, sharing the progress indicator's accent and neutral fallback, with a solid accent for reduced motion.

The assistant image includes the X server, window manager, dock, compositor, clipboard bridge, wallpaper setter (`feh`), fonts, Chrome libraries and pinned WezTerm terminal. Image builds and the runtime fallback share `desktop-system-packages.json`, including architecture-specific WezTerm SHA-256 checksums. The dependency layer precedes application source so code-only updates reuse it. `desktop-dependencies.ts` checks binaries and system assets before installing packages; older images or missing components use image-root `/usr/bin/apt-get` with `--no-upgrade` and `--no-remove`. The fallback bypasses Kata persistent-apt wrappers so binaries, X assets and shared libraries live in the same filesystem. Google Chrome is an exact-version, SHA-256-verified download for Linux x64 or ARM64, extracted under `external/desktop` on the persistent workspace. Extraction does not run Chrome package scripts, register its repository or change the system's default browser. Successful setup is recorded only after Chrome runs. Container replacement reuses the persisted Chrome installation and profile alongside the baked system packages. Configured `NODE_EXTRA_CA_CERTS` are combined with the system CA bundle for runtime downloads without changing system trust. Installation errors remain retryable. Existing browser-tool and PDF installations retain their own Playwright behavior.
