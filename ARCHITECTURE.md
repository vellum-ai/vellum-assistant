...
Voice escalation normally starts with `[1]` before a holding phrase. The shared voice verdict parser also recovers a standalone `[1]` at the end of a completed front-door reply. This starts the tool-capable leg using the already streamed speech as its acknowledgement, without repeating it or delaying normal answer streaming. Interior or incomplete markers do not trigger this recovery, and cancelled turns cannot hand off.
