...
`DesktopSessionManager` owns a private CDP connection to its Chrome child, launched on an allocated nonzero loopback debug port. Startup, dock launchers and relaunch use shared Chrome arguments and the existing `desktop-profile`. Discovery verifies the Linux listener inode against the managed Chrome PID and rejects redirects or non-loopback WebSocket endpoints. No gateway route exposes CDP. The bundled tool offers bounded semantic browser actions and observations with text, controls and fresh IDs, using the shared CDP transport, AX transformer and DOM helpers. References bind to the selected target, frame, document and connection epoch; navigation, scope switches, restart and takeover invalidate them. Browser actions do not request screenshots. Actionability checks reject detached, disabled, covered, moving or noneditable controls. Native UI, canvas and cross-frame clicks use X11. Bounded command failures after dispatch report an unknown outcome and never retry the action. Takeover aborts pending commands and releases input, but cannot undo commands already delivered to Chrome.
