...
The bundled `computer-use` skill exposes the same `computer_use_*` tools for connected computers and the assistant desktop. An omitted target or `target: "connected-computer"` uses `HostCuProxy` and the connected client's native helper. Explicit `target: "assistant-desktop"` dispatches to the container's X11 backend against the display and Chrome profile the viewer sees; it cannot include `target_client_id` or fall back to a connected computer. The shared skill manifest is the action schema source of truth. The bundled executor resolves each invocation's host/sandbox policy boundary from the same target selector. Local control requires the default-off desktop flag, completed automatic installation, and an identified guardian conversation; no connected host client is needed. Browser usage guidance lives in `assistant browser --help`; native workflows use `computer-use`.

`DesktopAutomationLease` serializes automation and binds ownership to one conversation and actor. Browser operations use the lease directly. The separate `DesktopControl` executor consumes an observation ID for each native action, invalidating it when another operation uses the lease. The X11 executor uses `xwd` (MIT) with in-process PNG encoding for screenshots and `xdotool` (BSD-3-Clause) for input; both are installed by desktop setup. Commands run with a fixed display, system binary paths, a restricted environment, bounded output and timeouts. Text travels over stdin. `assistant browser --virtual-desktop` borrows a scoped direct CDP client inside the same lease and dispatches through the existing browser operation handlers. Snapshot references use a separate desktop namespace and are invalidated on navigation, tab changes, release and native desktop input. A page overlay animates the CDP pointer in the stream; X11 input remains available for browser chrome and native apps. See [desktop browser CLI](assistant/docs/desktop-browser-cli.md).

An automation slot keeps the desktop alive independently of the viewer. The picture-in-picture preview is view-only; opening the desktop allows direct mouse, keyboard and clipboard input without a handoff step. Viewer interaction does not pause automation. Cancellation, desktop loss, inactivity, `detach` and `done` release the automation slot and clean up held browser and native input. No control state or screenshots are persisted outside normal tool history.
