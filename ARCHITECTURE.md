...
The modal's **Add apps** catalog offers Calculator (`xcalc`) and Text Editor (`xedit`). `GET /v1/desktop/apps` reads actual binary and launcher availability; `POST` accepts only a catalog app ID and `add` or `open`. Both path forms use the same guardian gateway middleware as setup, and the runtime remains gateway-service-only and feature-gated. Add jobs are deduplicated per app and share a serialized package-manager queue with desktop setup. The client refetches on desktop sync invalidations, event-stream reconnect, and while an install is active. A 404 yields an upgrade message with no write actions.

Installation downloads the distribution's authenticated `x11-apps` package, extracts it into temporary storage, and copies only the requested MIT/BSD app, its X resources, and license notices into the image root. The bundled GPL utilities and maintainer scripts are not installed or run. The desktop's existing xterm dependencies provide the shared X libraries. The binary is published last, so interrupted installs can retry. Existing binaries skip installation and only receive an atomically seeded launcher and PNG icon under `data/desktop-panel`; user launchers and pins are never overwritten. Open spawns a tracked child on the live streamed display with an allowlisted environment. Closing an app does not stop the desktop; desktop teardown stops tracked apps. Plank matches each app's `StartupWMClass` and users can select **Keep in Dock** to pin it.

Apps remain installed across X11 session restarts but image-root replacement, including a Kata save, requires explicit reinstallation. Workspace-backed launchers and pins survive; availability is checked against the actual binary rather than a persisted installed flag. No existing persisted format changes, so no migration is needed. The initial catalog intentionally excludes arbitrary package names, URLs, uninstall, and automatic restore. See [desktop app management](assistant/docs/desktop-apps.md) for the supported lifecycle and validation.
