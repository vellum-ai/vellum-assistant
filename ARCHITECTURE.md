...
- The pipeline carries only what a user can act on. Placement is a derived `bucket` (`needs_you` / `worth_knowing` / `activity`) computed before the model runs, so the model owns channels and wording but not importance. Three other surfaces carry the rest: **runs** (`assistant/src/runs/`) for async work with a lifecycle, one feed row rewritten in place, entering the pipeline only on `needs_input` / `failed` / a notable success; **System health** (`assistant/src/home/system-health.ts`) for a subsystem failing repeatedly, one durable counter that never pushes; and an **activity digest** folding routine finished work into one row. All four render as row kinds in the home feed, so they share its persistence, its `home_feed_updated` broadcast, and client read state. In-app attention rides the `feed_toast` event, which clients draw only while their window is focused so a toast and an OS banner never both fire.
