...
- The macOS and Windows Electron shells share platform-neutral window security, IPC validation, origin checks, and preload capability registration through `@vellumai/electron-desktop`. Each client keeps platform lifecycle and native features in its own adapter modules under `clients/<platform>/src/`.
