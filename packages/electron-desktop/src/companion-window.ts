...
import { getName as getAssistantName, onNameChange } from "./identity";
...
/** The app's own window, as far as the surface reads it. */
...
/**
 * The desktop-specific half of the surface: the IPC registrars the handlers
 * land in, the app's window they dispatch at, and the system settings that
 * decide how a move is drawn.
 */
export interface CompanionPlatform {
  handle: IpcHandle;
  on: IpcOn;
  /** The app's window, or null when there is none. */
  currentMainWindow: () => MainWindowHandle | null;
  dispatchToMain: (command: VellumCommand) => void;
  ensureMainWindowVisible: () => Promise<unknown>;
  onMainWindowVisibilityChange: (listener: () => void) => void;
  /** The system's "Reduce motion", read per move. */
...
const handle: IpcHandle = (channel, schema, fn) =>
  configuration.get().handle(channel, schema, fn);
const on: IpcOn = (channel, schema, fn) =>
  configuration.get().on(channel, schema, fn);
const currentMainWindow = (): MainWindowHandle | null =>
  configuration.get().currentMainWindow();
const dispatchToMain = (command: VellumCommand): void => {
  configuration.get().dispatchToMain(command);
};
const ensureMainWindowVisible = (): Promise<unknown> =>
  configuration.get().ensureMainWindowVisible();
const onMainWindowVisibilityChange = (listener: () => void): void => {
  configuration.get().onMainWindowVisibilityChange(listener);
};
...
const prefersReducedMotion = (): boolean =>
  configuration.get().prefersReducedMotion();
