...
- **Local source deps can render stale in dev.** `@vellumai/design-library`, `@vellumai/assistant-api`, and other local source packages are consumed with `preserveSymlinks`, so Vite *prebundles* them into `node_modules/.vite/deps`. Vite's optimizer cache key ignores the *contents* of those local packages, so branch switches can leave a stale prebundle behind (and the macOS Electron dev window, which loads from the same `:5173` server, shows it too). This is handled automatically: the `dev` script runs `scripts/ensure-fresh-vite.ts`, which fingerprints the local source packages and clears stale Vite cache before startup; the `watch-design-library` plugin in `vite.config.ts` drops the dep cache and restarts on in-session design-library edits. If you ever still see a stale local package change, `rm -rf apps/web/node_modules/.vite` and restart. **Don't** "fix" this by switching to `optimizeDeps.exclude` or source-aliasing design-library — it ships its own React copy, which white-screens (the `#34771`→`#34773` revert); keep prebundling and keep the cache fresh.
