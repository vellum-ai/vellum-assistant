...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      // `/account/*` and `/logout` at the root (see comment in routes.tsx),
      // so a hard-reload on those URLs would otherwise 404 in dev. Rewriting
      // the request to `/assistant/` makes Vite serve index.html; the browser
      // URL is unchanged, so React Router still matches the original path.
      // Production relies on the deployed server's catch-all fallback.
      {
