...
import { useAppRootContainer } from "@/components/app/core/AppRootContext.js";
...
function isAllowedFile(file: File): boolean {
  ...
  // Fallback for files without a MIME type (some drag sources omit it).
  if (!file.type) {
...
// Build a single-file USTAR tar entry: 512-byte header + data padded to 512.
...
function buildTarEntry(filename: string, data: Uint8Array): Uint8Array {
  ...
  // checksum field (148-155) initialized to spaces for checksum calculation
  for (let i = 148; i < 156; i++) buffer[i] = 0x20;
  buffer[156] = 0x30; // typeflag '0' = regular file
...
async function fetchPlatformLogs(
  ...
  try {
    ...
    // Direct fetch: the log export endpoint is a wildcard proxy route returning
    // a binary tar.gz blob — it has no OpenAPI schema or generated HeyAPI client.
...
async function buildClientLogsFile(
  ...
  // USTAR archives end with two zero-filled 512-byte blocks.
...
export function ShareFeedbackModal({
  ...
  // Auto-toggle logs inclusion when reason changes, unless user manually toggled
    ...
    try {
      const logsFile =
        includeLogs && selectedReason !== "feature_request"
          ...
          : null;
      // bodySerializer is a valid HeyAPI runtime option for multipart uploads
      // but is not reflected in the generated mutation types.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (mutation.mutateAsync as (opts: any) => Promise<unknown>)({
        ...
        body: {
          ...
          client_version: typeof process !== "undefined" && process.env.NEXT_PUBLIC_APP_VERSION
  ...
  return createPortal(
    ...
    >
      {/* TODO: migrate to Modal — this is a bespoke modal shell, not a card */}
      <div
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 id={titleId} className="!m-0 text-title-small text-(--content-default)">
            Share Feedback
          </h2>
          <Button
            variant="ghost"
        ...
        <div className={`flex flex-col gap-4 overflow-y-auto ${isSubmitting ? "pointer-events-none opacity-60" : ""}`}>
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-body-small-default" style={{ color: "var(--content-secondary)" }}>
              Category
            </span>
            <div className="flex flex-col gap-1.5">
          ...
          {selectedReason !== "feature_request" && (
            <div className="flex flex-wrap items-center gap-3">
              ...
              />
              <span className="text-body-medium-lighter" style={{ color: "var(--content-default)" }}>
                Include diagnostics
              </span>
              {includeLogs && (
                <select
          ...
          {submitError && (
            <p className="text-body-medium-lighter" style={{ color: "var(--system-negative-strong)" }}>
              {submitError}
            </p>
          )}
        </div>
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          {isSubmitting ? (
            <span className="inline-flex items-center gap-2 text-body-medium-lighter" style={{ color: "var(--content-secondary)" }}>
...
function ReasonCard({
  ...
  return (
    ...
    >
      ...
      />
      <span className="flex-1 text-body-medium-lighter" style={{ color: "var(--content-default)" }}>
        {option.label}
      </span>
      <span
        aria-hidden
  ...
  return (
    ...
    >
      {isImage && previewUrl ? (
        ...
        <Paperclip className="h-5 w-5 text-(--content-secondary)" />
      )}
      <Button
        variant="ghost"
        size="compact"
