...
import { computeTransforms, resolveDefinitions } from "@/lib/avatar/svg-compositor.js";
...
interface AnimatedAvatarProps {
  ...
  size: number;
  /** When true, plays the streaming morph animation on the body. */
  isStreaming?: boolean;
}
...
function wobblePath(d: string, seed: number, amount: number): string {
  ...
  return d.replace(/-?\d+\.?\d*(?:e[+-]?\d+)?/gi, (match, offset) => {
    ...
    const pairedVal = pairedIdx >= 0 && pairedIdx < otherNums.length
      ? otherNums[pairedIdx]!
      : refVal;
    ...
    const wobble = Math.sin(angle * 2.0 + phase) * 0.7 +
...
function precomputeWobbledPaths(basePath: string, count: number, amount: number): string[] {
 ...
 *   - Morph: body path cycles through 16 wobbled variants (±6% radial)
 *   - Scale: ±2% asymmetric squash/stretch (2.4s)
 *   - Rotation: ±0.86° (3s)
 *   - Blink + twitch paused (matching macOS behaviour)
 *
 * All animations respect `prefers-reduced-motion`.
 */
export function AnimatedAvatar({ components, traits, size, isStreaming = false }: AnimatedAvatarProps) {
  const reduce = useReducedMotion();

  // --- Resolve definitions & transforms ---
  ...
  const eyeCenterOutputX = bodyScaleFactor * (remapTx + eyeStyle.eyeCenter.x * remapScale) + bodyTx;
  const eyeCenterOutputY = bodyScaleFactor * (remapTy + eyeStyle.eyeCenter.y * remapScale) + bodyTy;

  // --- Pre-compute 16 wobbled body path variants for streaming morph ---
    ...
    >
      <g
        style={{
          ...
          transition: effectiveTwitchAngle !== 0 ? "transform 0.2s ease-in-out" : "transform 0.3s ease-out",
        }}
      >
        <path
          d={currentBodyPath}
          ...
          style={{ transition: isStreaming ? "d 0.3s ease-in-out" : "none" }}
        />
      </g>
