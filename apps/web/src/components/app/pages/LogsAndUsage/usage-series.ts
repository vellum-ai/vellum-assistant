import { resolveUsageGroupLabel } from "@/lib/usage/group-labels.js";
import type { UsageGroupLabelMetadata } from "@/lib/usage/group-labels.js";
import type {
  UsageDayBucket,
  UsageGroupBreakdown,
  UsageSeriesBucket,
...
export function sortUsageSeriesBuckets(
  ...
  return [...buckets].sort((lhs, rhs) => {
    ...
    if (lhsOffset !== rhsOffset) {
      // DST fall-back can emit duplicate local-hour labels; the higher UTC
      // offset starts earlier in absolute time, matching the macOS chart.
