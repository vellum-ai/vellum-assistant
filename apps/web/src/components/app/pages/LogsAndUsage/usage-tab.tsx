...
import { decorateUsageBreakdownGroups } from "@/lib/usage/group-labels.js";
import { fetchUsageProfileMetadata } from "@/lib/usage/profile-metadata.js";
import type {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h3
          className="!m-0 text-title-small"
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        >
          <Button leftIcon={<Sparkles />} onClick={onAnalyze}>
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        <h3
          // off-scale: 14/600 (usage section heading, not on the 12-style scale)
          className="!m-0 text-body-medium-default"
          style={{ color: "var(--content-default)" }}
        >
          {title}
        ...
        {subtitle ? (
          <p className="!m-0 text-body-small-default" style={{ color: "var(--content-tertiary)" }}>
            {subtitle}
          </p>
        ) : null}
      </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-baseline justify-between gap-4">
        <div className="flex flex-col gap-1">
          {/* typography: off-scale — hero cost metric */}
          { }
          <span
            style={{
              fontSize: "30px",
...
function ColumnToggle({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="text-label-medium-default rounded-full border px-2.5 py-1"
      style={{
        borderColor: active
      ...
      <table className="w-full table-fixed">
        <thead>
          <tr style={{ borderBottom: "1px solid var(--border-base)" }}>
            <th
              className="text-label-medium-default px-3 py-2.5 text-left"
              style={{ color: "var(--content-tertiary)" }}
            >
              Group
            </th>
            ...
            {showPct ? (
              <th
                className="text-label-medium-default px-3 py-2.5 text-right"
        ...
        <tbody>
          {groups.map((group, index) => {
            ...
            return (
              <tr
                key={group.groupKey ?? group.groupId ?? `${group.group}-${index}`}
                style={{
                  borderTop:
              ...
              >
                ...
                >
                  <span
                    className="text-body-medium-lighter block truncate"
                    title={group.group}
                  >
                    {group.group}
                  </span>
                ...
                {showTokens ? (
                  ...
                  >
                    <span
                      className="text-body-small-default block truncate"
                      title={tokenDetail}
                    >
                      {tokenShort}
                    </span>
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-8 text-center">
      ...
      <span className="text-body-small-default" style={{ color: "var(--content-tertiary)" }}>
        {subtitle}
      </span>
    </div>
  );
    ...
    <div className="flex flex-col gap-2 py-2">
      ...
      {onRetry ? (
        <button
          type="button"
          onClick={onRetry}
          className="text-body-small-default self-start rounded-md border px-3 py-1"
          style={{
            background: "var(--surface-lift)",
            borderColor: "var(--border-base)",
            color: "var(--content-default)",
