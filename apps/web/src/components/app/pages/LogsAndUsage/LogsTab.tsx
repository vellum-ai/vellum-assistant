...
import { listConversations, type Conversation } from "@/domains/chat/lib/api.js";
import {
  loadLastViewedConversationKey,
  saveLastViewedConversationKey,
} from "@/domains/chat/lib/lastViewedConversationStorage.js";
import { fetchTraceEvents } from "@/lib/trace-events/api.js";
import type { TraceEventRow } from "@/lib/trace-events/types.js";
...
function SessionMetricsCard({ metrics }: { metrics: ConversationMetrics }) {
  ...
  return (
    ...
    >
      <h3
        // off-scale: 14/600 (session metrics card heading, not on the 12-style scale)
...
function MetricStatCard({
  ...
  return (
    <div
      ...
      style={{ background: "color-mix(in srgb, var(--border-base) 15%, transparent)" }}
    >
      <span
        // off-scale: 14/600 (metric stat value, not on the 12-style scale)
...
function TraceTimeline({
  ...
  return (
    <section className="flex flex-col gap-4">
      <h3
        // off-scale: 14/600 (timeline section heading, not on the 12-style scale)
        className="!m-0 text-body-medium-default"
        style={{ color: "var(--content-default)" }}
      >
        Timeline
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-0.5">
          ...
          <span
            // off-scale: mono 12/400 (log row timestamp — monospaced by design)
        ...
        >
          <dl className="!m-0 flex flex-col gap-1">
            {attributeEntries.map(([key, value]) => (
              ...
              >
                <dt
                  // off-scale: mono 12/400 (log attribute key — monospaced by design)
                  className="!m-0 font-mono text-body-small-default shrink-0"
                  style={{ color: "var(--content-tertiary)" }}
                >
                  {key}
                </dt>
                <dd
                  // off-scale: mono 12/400 (log attribute value — monospaced by design)
                  className="!m-0 font-mono text-body-small-default break-all"
                  style={{ color: "var(--content-secondary)" }}
                >
...
function EmptyPlaceholder({
  ...
  return (
    ...
    >
      ...
      <p className="text-body-small-default !m-0 mt-1">{subtitle}</p>
    </div>
  );
}
