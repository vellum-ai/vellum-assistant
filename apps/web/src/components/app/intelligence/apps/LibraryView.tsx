...
import { AppPreviewThumbnail } from "@/components/app/core/AppCard/AppCard.js";
...
// ---------------------------------------------------------------------------
// LibraryAppCard — macOS-style card for the library grid
// ---------------------------------------------------------------------------
...
function LibraryAppCard({
  ...
  // Keep the trigger visible while the menu is open — without this, moving
  // the cursor into the floating menu leaves the card's hover group and the
  // trigger fades out.
  ...
  return (
    ...
    >
      ...
      {/* More menu — top-right of thumbnail. Mobile keeps it always visible
          (no hover to reveal); desktop hides it by default and reveals on
          hover, keyboard focus, or while the menu is open. */}
      <div
        className={cn(
...
export function LibraryView({
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      // Drop the id from pinned storage if it was pinned. `togglePin` toggles,
      // so it's safe to call only when the app is currently pinned.
  ...
  const handleImportBundle = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Refresh list first so the imported app appears even if opening fails
      const updatedApps = await listApps(assistantId);
      setApps(updatedApps);
      setLastImportedAppId(result.appId);
      // Then try to open the imported app
      try {
  ...
  // -----------------------------------------------------------------------
  // Render — app grid with Pinned / Recents sections
  // -----------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// LibraryAppCardActionsMenu — Pin/Unpin and Delete actions for a single app
// card. Renders as a Radix dropdown on desktop and as a `BottomSheet` on
// mobile. Extracted so the mobile vs desktop branch can be unit-tested in
// isolation (the parent card pulls in app html caching, pin context, etc.).
// ---------------------------------------------------------------------------
      ...
      <BottomSheet.Root open={open} onOpenChange={onOpenChange}>
        ...
        <BottomSheet.Content>
          {/* Radix Dialog requires a Title for screen readers — visually-hidden
              since the rows are self-describing. */}
