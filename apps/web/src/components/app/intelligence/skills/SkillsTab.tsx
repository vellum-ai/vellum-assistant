...
} from "@/components/app/intelligence/MobileSidebarDrawer.js";
import { CategorySidebar } from "@/components/app/intelligence/skills/CategorySidebar.js";
import { SkillDetail } from "@/components/app/intelligence/skills/SkillDetail.js";
import { SkillRow } from "@/components/app/intelligence/skills/SkillRow.js";
...
interface SkillsTabProps {
   ...
   * the `?skill=<id>` deep-link consumed by AssistantPageClient. Only seeds
   * the initial state — internal navigation thereafter is local state.
   */
...
export function SkillsTab({ assistantId, initialSkillId }: SkillsTabProps) {
  ...
  // Sidebar counts must reflect the dataset across all categories, so when a
  // category is selected we run a parallel query without the category param.
  // When no category is selected, skillsQuery already has the unfiltered data.
