...
import { AssistantChannelsDetail } from "@/components/app/intelligence/contacts/AssistantChannelsDetail.js";
import { ContactDetailView } from "@/components/app/intelligence/contacts/contact-detail-view.js";
import { ConnectAssistantDialog } from "@/components/app/intelligence/contacts/connect-assistant-dialog.js";
import { ContactMergeDialog } from "@/components/app/intelligence/contacts/contact-merge-dialog.js";
import { ContactsList } from "@/components/app/intelligence/contacts/contacts-list.js";
import { GuardianDetailView } from "@/components/app/intelligence/contacts/guardian-detail-view.js";
import type {
...
interface ContactsTabProps {
  assistantId: string;
  // Opens a fresh conversation and seeds it with the given message
  // (mirrors macOS ConversationManager.openConversation(forceNew: true)).
  onStartSetupConversation?: (prompt: string) => void;
  /** Contact ID to pre-select on mount. Pass null to auto-select the guardian. */
  initialContactId?: string | null;
  /** Called when the selected contact changes. null means the assistant channels view. */
  onContactSelected?: (contactId: string | null) => void;
}

// Prompts for the principal setting up the assistant's own identity on a
// channel (e.g. "let's configure your Slack workspace"). Distinct from the
// per-channel `setupMessages` returned by `/v1/channels/available`, which
// cover identity verification once the channel is configured. Keep until the
// backend surfaces these too.
...
export function ContactsTab({
  ...
  const { a2aChannel } = useAppFeatureFlags();
  ...
  const [selection, setSelection] = useState<ContactSelection>(() =>
    initialContactId ? { kind: "contact", contactId: initialContactId } : { kind: "assistant" },
  );
  ...
  const handleSelect = useCallback(
    (sel: ContactSelection) => {
      setSelection(sel);
      setDrawerOpen(false);
      // Close any open merge dialog when the user navigates — it was scoped
      // to the previous survivor.
      setMergeDialogOpen(false);
      setMergeError(null);
      onContactSelected?.(sel.kind === "contact" ? sel.contactId : null);
    },
    [onContactSelected],
  );
  ...
  // `handleCloseMerge` is declared further down, after `mergeMutation`, so it
  // can guard against close-while-pending without a forward reference.
  ...
  // Per-channel display metadata + setup prompts. The frontend doesn't know
  // which channels the assistant supports or how to name/describe/icon them —
  // the gateway is the source of truth. Empty array on miss so rows fall back
  // to the contact's already-configured channels (revoke-only).
  ...
  // Memo so the empty-array fallback doesn't recreate on every render and
  // bust useCallback dep arrays downstream.
  ...
  // Candidates the user can merge into the currently-selected contact.
  // We always exclude the guardian (its role is structurally unique — deleting
  // it would break the single-guardian invariant) and we exclude the survivor
  // itself (the merge route rejects self-merges anyway). Computed lazily so
  // we don't recreate the list while the dialog is closed.
  ...
  // When navigating to /assistant/contacts (no specific ID), auto-select the guardian
  // once the contacts list loads so they always land on a meaningful detail view.
  const guardianAutoSelectedRef = useRef(false);
  useEffect(() => {
    if (initialContactId !== undefined && initialContactId !== null) return;
    ...
    onContactSelected?.(guardian.id);
  }, [guardian, initialContactId, onContactSelected]);

  const channels = useMemo(
    () => deriveChannelStates(readinessData),
    // deriveChannelStates is pure over readinessData
    [readinessQuery.data],
  );
  ...
  const handleConnectSuccess = useCallback(
    ...
    [invalidateContacts, onContactSelected],
  );
