...
import { hatchAssistant } from "@/lib/assistants/api.js";
...
export function AssistantLifecyclePanel() {
  ...
  } = useQuery(assistantsActiveRetrieveOptions());

  const {
    data: assistantsList,
    isLoading: listLoading,
  } = useQuery(assistantsListOptions({ query: { hosting: "all" } }));
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between gap-4">
          ...
          <Button
            variant="outlined"
            leftIcon={hatching ? <Loader2 className="animate-spin" /> : undefined}
...
function AssistantInfoCard({ assistant, loading }: AssistantInfoCardProps) {
  ...
  return (
    <SettingsCard
      ...
      subtitle="Current assistant details."
    >
      <div className="grid grid-cols-[140px_minmax(0,1fr)] gap-y-3">
        ...
        <InfoValue>{new Date(assistant.created).toLocaleDateString()}</InfoValue>

        <InfoLabel>Last Modified</InfoLabel>
        <InfoValue>{new Date(assistant.modified).toLocaleDateString()}</InfoValue>
