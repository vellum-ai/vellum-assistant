...
/**
 * Developer-tab panel for triggering Sentry test events.
 * Mirrors the macOS SettingsDeveloperTab Sentry section.
 */
  ...
  return (
    ...
    >
      <div className="space-y-3">
        <SentryTestRow
          icon={<Flame className="h-4 w-4 text-[var(--system-negative-strong)]" />}
          label="Fatal Event"
          description="Send a fatal-level event to Sentry."
          onClick={handleCaptureFatal}
        />
        <SentryTestRow
          icon={<XCircle className="h-4 w-4 text-[var(--system-negative-default)]" />}
          label="Error Event"
          description="Capture a test Error exception."
          onClick={handleCaptureError}
        />
        <SentryTestRow
          icon={<AlertTriangle className="h-4 w-4 text-[var(--system-warning-default)]" />}
          label="Warning Event"
        ...
        />
        <SentryTestRow
          icon={<Timer className="h-4 w-4 text-[var(--system-positive-default)]" />}
...
function SentryTestRow({ icon, label, description, onClick }: SentryTestRowProps) {
  return (
