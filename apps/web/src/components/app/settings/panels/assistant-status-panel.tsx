...
import { CapacityBar } from "@/components/app/settings/CapacityBar.js";
import { type Assistant, type AssistantHealthz, getAssistant, getAssistantHealthz } from "@/lib/assistants/api.js";
...
export function useAssistantWithHealthz(): AssistantWithHealthz {
  // Mirror the chat page's resolver (getAssistant): platform-first, fall
  // back to local. The settings page configures whichever assistant the
  // user sees in chat.
  const {
    data: assistant = null,
  ...
  const fetchHealthz = useCallback(async () => {
    ...
    try {
      ...
      if (requestId !== healthzRequestIdRef.current) return;
      setHealthz(null);
      // Network-level failures (daemon sleeping, user offline, DNS timeout)
      // are expected transient conditions — skip Sentry to reduce noise.
...
/**
 * Renders the assistant metadata grid and capacity bars (disk / CPU / memory).
 * Intended to be placed inside a SettingsCard wrapper; the card owns the
  ...
  return (
    <div className="grid grid-cols-[140px_minmax(0,1fr)] gap-y-3">
      ...
      {healthzLoading && !assistant.current_release_version ? (
        ...
        <span className="text-body-medium-lighter text-[var(--content-tertiary)]">—</span>
      )}
    </div>
  );
}
...
/**
 * Renders the capacity bars (disk / CPU / memory) for an assistant. Intended
 * to be placed inside its own SettingsCard so the section reads as
 * "System Resources" independently of the General info card.
 */
    ...
    <div className="grid grid-cols-[140px_minmax(0,1fr)] gap-y-3">
      ...
      {healthzLoading ? (
        ...
        <CapacityBar
          ...
          caption={`${formatResourceMb(healthz.disk.usedMb)} used of ${formatResourceMb(
            healthz.disk.totalMb,
          )}`}
        />
      ) : (
        <span className="text-[var(--content-tertiary)]">Disk status unavailable</span>
      )}
      ...
      {healthzLoading ? (
        ...
        <CapacityBar
          ...
          caption={`${formatResourceMb(healthz.memory.currentMb)} used of ${formatResourceMb(
            healthz.memory.maxMb,
          )}`}
        />
      ) : (
        <span className="text-body-medium-lighter text-[var(--content-tertiary)]">—</span>
...
export function hasAssistantResourceMetrics(
  healthz: AssistantHealthz | null,
): boolean {
  return Boolean(healthz?.disk || healthz?.memory || healthz?.cpu);
}
