...
function buildMetadata(identifier: string): TimezoneEntry | null {
  ...
  try {
    ...
    offsetLabel = tz.startsWith("GMT") || tz.startsWith("UTC") ? tz : `GMT ${tz}`;
...
// TODO(core-lib-adopt): Consider migrating to the `Popover` primitive once
// we have a pattern for autocomplete-style combobox popovers. The current
// pattern anchors a listbox to a text input and needs the input to retain
// focus for typing — Radix's focus trap on `Popover.Content` would interfere
// with that. The bespoke outside-click effect is acceptable here because the
// typing UX is the primary concern.
export function TimezonePicker({ value, onChange }: TimezonePickerProps) {
  ...
  return (
    <div ref={containerRef} className="space-y-3">
      {/* Label sits above the input on mobile so the search field
          can use the full card width — at the narrow breakpoint the
          label's two-word "Closest city" wraps and squeezes the input
          when laid out in a row. Switch to the original side-by-side
          layout at `md+`. */}
              ...
              {filtered.slice(0, 200).map((entry) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "flex w-full cursor-pointer items-center justify-between gap-3 px-3 py-2 text-left text-body-medium-lighter transition-colors",
                      isSelected
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-body-medium-default">{entry.city}</div>
                      {entry.region && (
      ...
      <div className="flex items-center justify-between gap-4">
        <span className="text-body-medium-lighter text-[var(--content-tertiary)]">Time zone</span>
