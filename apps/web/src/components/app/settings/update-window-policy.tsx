...
import {
  assistantsUpgradePolicyRetrieveOptions,
  assistantsUpgradePolicyRetrieveQueryKey,
} from "@/generated/api/@tanstack/react-query.gen.js";
import { assistantsUpgradePolicyPartialUpdate } from "@/generated/api/sdk.gen.js";
import {
...
import type { FrequencyEnum, UpgradePolicy } from "@/generated/api/types.gen.js";

// Import for side effects: ensures HeyAPI client is configured.
import "@/lib/vellum-api/client.js";

// Match backend weekday() convention: Mon=0, Tue=1, ..., Sun=6
...
/**
 * Configures the automatic update window policy — frequency, days, and time
 * window during which automatic updates are applied.
 */
export function UpdateWindowPolicy({ assistantId, admin }: UpdateWindowPolicyProps) {
  ...
  const {
    ...
    ...assistantsUpgradePolicyDetailReadOptions({ path: { id: assistantId } }),
    enabled: !admin,
  });
  const policy = (admin ? adminPolicy : userPolicy) as UpgradePolicy | undefined;
  ...
  const policyUpdate = useMutation({
    mutationFn: async (body: { enabled?: boolean; timezone?: string; schedule?: { frequency: FrequencyEnum; days_of_week: number[]; days_of_month: number[]; window: { start: string; end: string } } }) => {
      if (admin) {
  ...
  const handleSavePolicy = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: admin
          ? assistantsUpgradePolicyRetrieveQueryKey()
          : assistantsUpgradePolicyDetailReadQueryKey({ path: { id: assistantId } }),
      });
    } catch {
