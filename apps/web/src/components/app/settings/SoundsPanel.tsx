...
import {
  ...
  saveSoundsConfig,
  type AvailableSound,
} from "@/lib/sounds/api.js";
import { getSoundManager } from "@/lib/sounds/sound-manager.js";
import {
  defaultSoundsConfig,
...
export function SoundsPanel() {
  ...
  // Keep the sound manager in sync with the config so playback reflects
  // the latest settings without every play-site re-reading the workspace.
  // Reset `featureEnabled` on unmount so the process-wide singleton doesn't
  // outlive the settings panel — future play-sites should re-enable it
  // themselves based on the server-side feature flag.
  useEffect(() => {
  ...
  const updateConfig = useCallback(
    (producer: (prev: SoundsConfig) => SoundsConfig) => {
      const prev = queryClient.getQueryData<SoundsConfig>(configQueryKey) ?? defaultSoundsConfig();
  ...
  // Track the slider's in-flight value so dragging feels responsive without
  // persisting to the workspace on every pixel of movement. We commit on
  // pointer/key release via `commitVolume`.
  ...
  const setEventEnabled = (event: SoundEventId, enabled: boolean) => {
    updateConfig((prev) => ({
      ...
      events: {
        ...prev.events,
        [event]: { ...(prev.events[event] ?? { enabled: false, sounds: [] }), enabled },
      },
    }));
  };
    ...
    <div className="max-w-[940px] space-y-6">
      <Card asChild>
        ...
        <div className="flex items-center gap-3 py-3">
          <span className="text-body-medium-lighter text-[var(--content-default)]">Volume</span>
          <input
            type="range"
            min={0}
            max={1}
        ...
        <div className="flex items-center justify-between py-3">
          ...
          <button
            ...
            disabled={!config.globalEnabled}
            className="inline-flex items-center gap-1.5 rounded-md border border-[var(--border-base)] bg-white px-3 py-1.5 text-body-medium-lighter text-[var(--content-default)] hover:bg-[var(--surface-base)] disabled:cursor-not-allowed disabled:opacity-50 dark:border-[var(--border-base)] dark:bg-[var(--surface-lift)] dark:text-[var(--content-default)] dark:hover:bg-[var(--ghost-hover)]"
          >
      ...
      <Card asChild>
        ...
        <div className="pb-2">
          ...
          <p className="text-body-small-default text-[var(--content-tertiary)]">
            Add one or more sounds per event. When multiple are configured,
            one plays at random.
          </p>
        </div>
        <div className="divide-y divide-[var(--border-base)]">
          {SOUND_EVENT_IDS.map((event) => (
            <SoundEventRow
              key={event}
              event={event}
              eventConfig={config.events[event] ?? { enabled: false, sounds: [] }}
              ...
              onRemoveSound={(filename) => removeSoundFromEvent(event, filename)}
...
function ToggleRow({ label, description, checked, disabled, onChange }: ToggleRowProps) {
  return (
    <div className="flex items-center justify-between gap-3 py-1">
      <div>
        <div className="text-body-medium-lighter text-[var(--content-default)]">{label}</div>
        {description ? (
    ...
    <div className="py-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <Toggle
          ...
          onChange={onToggle}
          aria-label={`Enable ${SOUND_EVENT_DISPLAY_NAMES[event]}`}
        />
      </div>
        ...
        <div className="mt-2 space-y-1 pl-2">
          ...
          {availableSounds.length === 0 ? (
            <p className="text-body-small-default italic text-[var(--content-disabled)]">
              No sound files yet. Drop audio files into data/sounds/ in your workspace.
            </p>
          ) : allAdded ? (
            <button
              type="button"
              disabled
              className="inline-flex items-center gap-1 rounded-md border border-[var(--border-base)] bg-white px-2 py-1 text-body-small-default text-[var(--content-disabled)] disabled:cursor-not-allowed dark:border-[var(--border-base)] dark:bg-[var(--surface-lift)] dark:text-[var(--content-tertiary)]"
            >
              All sounds added
            </button>
          ) : (
            <div className="relative inline-block">
              <button
                ...
                className="inline-flex items-center gap-1 rounded-md border border-[var(--border-base)] bg-white px-2 py-1 text-body-small-default text-[var(--content-default)] hover:bg-[var(--surface-base)] dark:border-[var(--border-base)] dark:bg-[var(--surface-lift)] dark:text-[var(--content-default)] dark:hover:bg-[var(--ghost-hover)]"
              >
                Add sound
              ...
              {pickerOpen ? (
                <div
                  className="absolute left-0 z-10 mt-1 max-h-64 w-56 overflow-auto rounded-md border border-[var(--border-base)] bg-white p-1 shadow-lg dark:border-[var(--border-base)] dark:bg-[var(--surface-lift)]"
                ...
                >
                  {remaining.map((s) => (
                    <button
                      ...
                      className="block w-full truncate rounded px-2 py-1 text-left text-body-small-default text-[var(--content-default)] hover:bg-[var(--surface-base)] dark:text-[var(--content-default)] dark:hover:bg-[var(--ghost-hover)]"
                      title={s.filename}
                    >
                      {s.label}
                    </button>
