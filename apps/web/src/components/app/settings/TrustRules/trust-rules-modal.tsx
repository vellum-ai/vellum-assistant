...
import { deleteTrustRule, fetchTrustRules } from "@/lib/trust-rules/api.js";
import type { TrustRuleItem, TrustRuleRisk } from "@/lib/trust-rules/types.js";

import { TrustRuleFormModal } from "@/components/app/settings/TrustRules/trust-rule-form-modal.js";
...
/**
 * Merge user-relevant rules (user-defined + modified defaults) with every
 * default rule, preferring the user-modified version when IDs collide. Sort
 * by tool then description to match the macOS TrustRulesView ordering.
 */
...
export function TrustRulesModal({ assistantId, onClose }: TrustRulesModalProps) {
  ...
  // Pure helper that fetches the rule set for the current toggle state.
  // Keeping it free of state writes lets both the effect below and the
  // imperative `loadRules` path reuse the same fetch + merge logic
  // without duplicating it.
  const fetchRulesForState = useCallback(
    async (showAll: boolean): Promise<TrustRuleItem[]> => {
      if (showAll) {
        // Match macOS TrustRulesView: fetch every default rule plus the
        // user-relevant set (user-defined + modified defaults), then merge
        // so modified defaults retain their edited risk while unmodified
  ...
  // Imperative refetch used by add / edit / delete callbacks. These are
  // one-shot user actions, so they don't need the stale-request guard
  // used by the toggle-driven effect below.
  ...
  // Refetch when assistantId or showAllDefaults changes. A stale flag in
  // the cleanup guards against rapid toggle flips where an older
  // in-flight request would otherwise resolve after a newer one and
  // overwrite state with data for the wrong toggle position.
  useEffect(() => {
    let stale = false;
    (async () => {
      setIsLoading(true);
  ...
  return createPortal(
    <>
      ...
      >
        <div className="mx-4 flex max-h-[80vh] w-full max-w-2xl flex-col rounded-xl border border-[var(--border-base)] bg-[var(--surface-raised)] shadow-xl dark:border-[var(--border-base)]">
                      ...
                      <div className="flex shrink-0 items-center gap-1">
                        ...
                        {(!isDefault || rule.userModified) && (
                          <button
                            ...
                            className="rounded-lg p-1.5 text-[var(--system-negative-strong)] transition-colors hover:bg-[var(--system-negative-weak)] dark:text-[var(--system-negative-strong)] dark:hover:bg-[var(--system-negative-weak)]"
                          >
