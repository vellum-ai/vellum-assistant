...
import { updateTrustRule } from "@/lib/trust-rules/api.js";
import type { TrustRuleItem, TrustRuleRisk } from "@/lib/trust-rules/types.js";
...
export function TrustRuleFormModal({
  ...
  return createPortal(
    ...
    >
      <div className="mx-4 flex max-h-[calc(100vh-2rem)] w-full max-w-md flex-col rounded-xl border border-[var(--border-base)] bg-[var(--surface-raised)] shadow-xl dark:border-[var(--border-base)]">
        <div className="flex items-center justify-between gap-3 border-b border-[var(--border-base)] px-6 py-4 dark:border-[var(--border-base)]">
          <h2
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="rounded-lg p-1.5 text-[var(--content-tertiary)] transition-colors hover:bg-[var(--surface-base)] hover:text-[var(--content-default)] dark:text-[var(--content-disabled)] dark:hover:bg-[var(--ghost-hover)] dark:hover:text-[var(--content-default)]"
          >
            <X className="h-4 w-4" />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg border border-[var(--border-element)] bg-white px-4 py-2 text-body-medium-default text-[var(--content-default)] transition-colors hover:bg-[var(--surface-base)] dark:border-[var(--border-base)] dark:bg-[var(--surface-lift)] dark:text-[var(--content-default)] dark:hover:bg-[var(--ghost-hover)]"
            >
              Cancel
            </button>
            <button
