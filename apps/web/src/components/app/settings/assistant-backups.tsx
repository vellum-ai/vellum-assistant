...
interface AssistantBackupsProps {
  assistantId: string;
}
...
export function AssistantBackups({ assistantId }: AssistantBackupsProps) {
  ...
  // Derive loading: true whenever resolvedId hasn't caught up to assistantId.
  // This resets automatically when assistantId changes without synchronous setState.
  ...
  const createBackupButton = (
    <div className="flex items-center gap-3">
      ...
      <Button
        variant="outlined"
        size="compact"
        leftIcon={creatingBackup ? <Loader2 className="animate-spin" /> : <Save />}
      ...
      {backups.map((backup) => (
        <Card.Root key={backup.snapshot_name}>
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <p className="truncate font-mono text-body-small-default text-[var(--content-secondary)]" title={backup.snapshot_name}>
              {backup.snapshot_name}
            </p>
            <div className="flex flex-wrap items-center gap-2">
              <BackupTypeBadge type={backup.backup_type} />
              {!backup.ready_to_use && (
                <span className="inline-flex items-center rounded-full bg-[var(--system-mid-weak)] px-2 py-0.5 text-body-small-default text-[var(--system-mid-strong)] dark:bg-[var(--system-mid-weak)] dark:text-[var(--system-mid-strong)]">
                  Not ready
                </span>
              )}
            </div>
          ...
          <div className="flex shrink-0 items-center gap-3">
            <div className="flex items-center gap-1 text-body-small-default text-[var(--content-tertiary)]">
              ...
              {new Date(backup.created_at).toLocaleString(undefined, { year: "numeric", month: "numeric", day: "numeric", hour: "numeric", minute: "2-digit" })}
            </div>
            <Button
              variant="outlined"
              size="compact"
              ...
              title={
                !backup.ready_to_use ? "Backup is not ready to use" : undefined
              }
              className="shrink-0"
            >
              Restore
