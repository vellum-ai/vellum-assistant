...
import { useLlmContext } from "@/domains/chat/lib/inspector-api.js";
import {
  llmLogPayloadQueryOptions,
  type LlmLogPayload,
} from "@/domains/chat/lib/inspector-payload-api.js";
import type { LlmContextResponse, LLMRequestLogEntry } from "@/domains/chat/lib/inspector-types.js";
...
import { TabBar, type InspectorTab } from "@/components/app/assistant/message-inspector/tab-bar.js";
import { MemoryTab } from "@/components/app/assistant/message-inspector/tabs/memory-tab.js";
...
import { RawTab } from "@/components/app/assistant/message-inspector/tabs/raw-tab.js";
import { ResponseTab } from "@/components/app/assistant/message-inspector/tabs/response-tab.js";
...
/**
 * Web port of `MessageInspectorView` from
 * `clients/macos/vellum-assistant/Features/Chat/MessageInspectorView.swift`.
 *
 * Renders the inspector's three-region layout: a header with back nav
 * and call count, a left call rail listing every LLM call captured for
 * this message, and a tabbed detail pane for the selected call.
 *
 * The selected call is encoded as `?callId=...` in the URL so each row
 * in the rail is a real hyperlink — sharable, right-click-openable,
...
export function MessageInspectorView({
  ...
  // Derive the selected log at render time. The URL's `callId` wins when
  // it still exists in the latest log set; otherwise we fall back to the
  // most recent call so the page never renders empty.
  ...
  const isLoading = isLoadingAssistant || isLoadingContext;
...
function Header({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      >
        <AppLink
          href={`${routes.assistant}?conversationKey=${encodeURIComponent(conversationKey)}`}
