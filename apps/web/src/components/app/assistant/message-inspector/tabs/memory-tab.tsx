...
/**
 * Web port of `MessageInspectorMemoryTab.swift` +
 * `MessageInspectorMemoryV2Tab.swift`.
 *
 * Receives the full `LlmContextResponse` (not just the per-call
 * `LLMRequestLogEntry`) because memory recall runs once per turn and
 * is not scoped to an individual LLM call.
 *
 * When both `memoryRecall` (v1) and `memoryV2Activation` are present
 * a pill switcher lets the user toggle between the two views. When
 * only one is present we render that one directly with no switcher.
 */
export function MemoryTab({
  ...
  // Reset the selected view when context changes (e.g. navigating to a
  // different message) — without this, if the previous context had v2 and
  // the new one is recall-only, view stays "v2" and the tab renders blank.
  useEffect(() => {
