...
import type { LLMContextSection, LLMRequestLogEntry } from "@/domains/chat/lib/inspector-types.js";
import { FileMarkdown } from "@/lib/markdown/file-markdown.js";
...
/**
 * Prompt tab — web port of `MessageInspectorPromptTab.swift`.
 *
 * Renders each normalized request section as a card. Text sections
 * render as Markdown by default (system prompts, user messages, and
 * tool inputs are almost always markdown-flavoured); a single
 * Markdown / Raw segmented control flips the entire tab to plain
 * `<pre>`-style text when the user wants to see source. Structured
 * payloads (JSON, etc.) always render as `<pre>` regardless of mode.
 */
