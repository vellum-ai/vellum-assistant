...
export interface UseTerminalSessionArgs {
  ...
  /** When true, terminal API calls route through admin-scoped endpoints. */
  admin?: boolean;
  /**
...
export function useTerminalSession({
  ...
  const apiOptions: (TerminalApiOptions & { service?: string }) | undefined = useMemo(
    () => (admin || service ? { ...(admin ? { admin: true } : {}), ...(service ? { service } : {}) } : undefined),
    [admin, service],
  );
  ...
  const startInputFlushTimer = useCallback((sessionId: string) => {
    ...
    inputFlushTimerRef.current = setInterval(() => {
      ...
      sendTerminalInput(assistantId, sessionId, buffered, apiOptions).catch(() => {
        // Best-effort — lost keystrokes are acceptable over a crash
      });
    }, INPUT_FLUSH_INTERVAL_MS);
  }, [assistantId, apiOptions]);
  ...
  const openSession = useCallback(
    async (isReconnect: boolean) => {
      ...
      if (dims && assistantId) {
        resizeTerminal(assistantId, sessionId, dims.cols, dims.rows, apiOptions).catch(() => {
  ...
  useEffect(() => {
    ...
    autoReconnectTimerRef.current = setTimeout(() => {
      ...
      if (prevSessionId && assistantId) {
        ...
        destroyTerminalSession(assistantId, prevSessionId, apiOptions).catch(() => {});
      }
  ...
  }, [terminalState.status, terminalState.reconnectAttempts, assistantId, apiOptions, openSession]);
  ...
  const reconnect = useCallback(() => {
    ...
    if (sessionId && assistantId) {
      destroyTerminalSession(assistantId, sessionId, apiOptions).catch(() => {
        // Best-effort
      });
    }
    ...
    dispatch({ type: "RECONNECT_REQUESTED" });
    openSession(true);
  }, [assistantId, apiOptions, openSession, stopInputFlushTimer]);
  ...
  }, [assistantId, apiOptions, stopInputFlushTimer]);
  ...
  const sendResize = useCallback(
    (cols: number, rows: number) => {
      ...
      resizeTimerRef.current = setTimeout(() => {
        ...
        pendingResizeRef.current = null;
        resizeTerminal(assistantId, currentSessionId, pending.cols, pending.rows, apiOptions).catch(() => {
          // Best-effort — resize failures are non-critical
        });
      }, RESIZE_DEBOUNCE_MS);
    },
