...
import type {
  ...
  TwilioConfig,
  TwilioPhoneNumber,
} from "@/lib/contacts/types.js";

// The assistant host exposes a wildcard proxy at
// `/v1/assistants/{assistant_id}/<anything>/` that forwards to the
// per-assistant runtime daemon. The contacts / channel-readiness /
// Slack-config endpoints live on that daemon and are therefore not in the
// generated OpenAPI schema — we call them through the HeyAPI client so the
// CSRF + Vellum-Organization-Id interceptors still apply.
...
export async function updateContact(
  ...
  // (Mirrors macOS ContactClient.updateContact.)
 ...
 * `/v1/channels/available` yet. Unlike the desktop app, the web client
 * isn't versioned alongside the assistant runtime, so we need to render
 * something sensible against an older gateway. This mirrors what the
 * backend currently emits for the always-on channels.
 *
 * TODO: delete this once assistant `0.8.1` is EOL — then the gateway is
...
/**
 * Fetch per-channel display metadata from the assistant gateway. The list
 * is the source of truth for what channels the assistant can surface
 * (including ordering) — the frontend should never hardcode this set when
 * a current assistant is on the other end. When the endpoint is missing
 * (older assistants pre-`0.8.1`) we fall back to `DEFAULT_CHANNELS`.
 */
export async function fetchChannelAvailability(
  ...
  // Older assistants don't have this route — fall back to the hardcoded
  // list so the UI keeps rendering the historical channel set.
