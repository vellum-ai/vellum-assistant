...
import { fetchSoundFile } from "@/lib/sounds/api.js";
import {
  type SoundEventId,
  type SoundsConfig,
...
class SoundManager {
  ...
  /** Inject/refresh the active assistant id (sounds are per-workspace). */
  ...
  /** Inject/refresh the live config so playback reflects current settings. */
  setConfig(config: SoundsConfig | null): void {
    this.config = config;
  }

  /** Feature-flag gate — controls whether any sound plays at all. */
  setFeatureEnabled(enabled: boolean): void {
    this.featureEnabled = enabled;
  }

  /**
   * Play the configured sound for an event. Respects the feature flag,
   * global enable, per-event enable, and random selection from the pool.
   */
  ...
  /**
   * Play a specific sound file regardless of event configuration.
   * Used by the settings UI to preview a selected sound at the configured
   * volume.
   */
  ...
  /**
   * Preview the default fallback blip at the configured volume.
   * Mirrors `SoundManager.playDefaultBlip()` on macOS.
   */
  ...
  /** Drop cached blob URLs (called when the assistant changes). */
  clearCache(): void {
  ...
  private playFallbackBlip(volume: number): void {
    ...
    try {
      ...
      // failed blip is non-fatal — users will get the next one after
      // interacting.
    }
  }
}
