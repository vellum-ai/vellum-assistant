...
// /v1/assistants/{id}/usage/* and are not part of the Django OpenAPI schema, so
// we maintain types by hand here. UsageGroupBy values are picker-facing labels;
// api.ts translates `task` and `profile` to daemon wire values at the boundary.
...
export interface UsageGroupBreakdown {
  group: string;
  groupId: string | null;
  // Daemon emits `groupKey` for call_site/inference_profile as a stable raw ID; `group` carries the human-readable label.
