...
export const PRECHAT_TOOLS: PreChatToolItem[] = [
  { id: "gmail", label: "Gmail", logoSrc: "/images/integrations/gmail.svg" },
  {
    id: "outlook",
    label: "Outlook",
    // PNG because the modern Outlook logo has complex gradients that don't reduce to a reasonable SVG size
...
/**
 * Collect all icon URLs from the tool catalog for server-side preloading.
 *
 * Used by the prechat page server component to emit `<link rel="preload">`
 * tags so the browser starts fetching tool icons during the initial HTML
 * parse, before React hydration.
 */
