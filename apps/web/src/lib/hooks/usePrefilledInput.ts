...
export function usePrefilledInput(seed: string | null | undefined): {
  ...
  useEffect(() => {
    ...
    if (!trimmed) return;
    // `prev || trimmed` preserves any value already present so a later
    // seed update does not overwrite the original fill; the `touched`
    // ref independently covers the user-typed case.
