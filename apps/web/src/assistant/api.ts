...
export async function listAssistantBackups(
  ...
  if (isLocalMode()) {
    return listAssistantBackupsViaDaemon(assistantId);
  }
...
async function listAssistantBackupsViaDaemon(
  assistantId: string,
): Promise<ListBackupsResult> {
...
export async function createAssistantBackup(
  ...
  if (isLocalMode()) {
    return createAssistantBackupViaDaemon(assistantId);
  }
...
// The daemon's POST /v1/backups/create rejects ("moved to gateway") so we
// call the gateway's top-level endpoint directly instead of using the daemon
// SDK, which would route through the proxy and hit the daemon handler.
async function createAssistantBackupViaDaemon(
  _assistantId: string,
): Promise<CreateBackupResult> {
  const gatewayUrl = getSelfHostedIngressUrl();
  if (!gatewayUrl) {
    return {
      ok: false,
      status: 0,
      error: { detail: "No gateway connection available." },
    };
  }
  ...
  const response = await fetch(
    `${gatewayUrl.replace(/\/$/, "")}/v1/backups/create`,
  ...
  try {
    ...
    return {
      ok: false,
      status: response.status,
      error: { detail: "Failed to create assistant backup." },
    };
  }
}
...
export async function restoreAssistantBackup(
  ...
  if (isLocalMode() && backup.path) {
    return restoreAssistantBackupViaDaemon(assistantId, backup.path);
  }
...
async function restoreAssistantBackupViaDaemon(
  assistantId: string,
  snapshotPath: string,
): Promise<RestoreBackupResult> {
