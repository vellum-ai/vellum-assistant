...
describe("mergeAdjacentAssistantMessages · contentOrder remap", () => {
  ...
  test("shifts tool:N positional ids in the donor by survivor.toolCalls.length (history shape)", () => {
    const survivor = makeAssistant({
      id: "a-1",
      toolCalls: [
        { id: "toolu_S0", name: "bash", input: {}, status: "completed" },
      ],
    ...
    const donor = makeAssistant({
      id: "a-2",
      toolCalls: [
        { id: "toolu_D0", name: "edit", input: {}, status: "completed" },
        { id: "toolu_D1", name: "test", input: {}, status: "completed" },
      ],
      contentOrder: [
        { type: "tool", id: "0" },
        { type: "tool", id: "1" },
  ...
  test("leaves real (non-numeric) toolCall / surface ids untouched (streaming shape)", () => {
    ...
    const survivor = makeAssistant({
      id: "a-1",
      toolCalls: [
        { id: "toolu_real_X", name: "bash", input: {}, status: "completed" },
      ],
      surfaces: [
        {
    ...
    const donor = makeAssistant({
      id: "a-2",
      toolCalls: [
        { id: "toolu_real_Y", name: "edit", input: {}, status: "completed" },
      ],
      surfaces: [
        {
  ...
  test("interleaved text + tool positional entries remap each by their own offset", () => {
    const survivor = makeAssistant({
      ...
      toolCalls: [
        { id: "toolu_S", name: "bash", input: {}, status: "completed" },
      ],
      contentOrder: [
        { type: "text", id: "0" },
        { type: "tool", id: "0" },
    ...
    const donor = makeAssistant({
      ...
      toolCalls: [
        { id: "toolu_D", name: "edit", input: {}, status: "completed" },
      ],
      contentOrder: [
        { type: "text", id: "0" },
        { type: "tool", id: "0" },
...
describe("mergeAdjacentAssistantMessages · cross-page bug repro", () => {
  ...
  test("folds three pages of a single turn back into one bubble", () => {
    const pageOld = makeAssistant({
      ...
      contentOrder: [{ type: "text", id: "0" }],
      toolCalls: [
        { id: "tool-A-1", name: "bash", input: {}, status: "completed" },
      ],
    });
    const pageMiddle = makeAssistant({
      ...
      contentOrder: [{ type: "text", id: "0" }],
      toolCalls: [
        { id: "tool-B-1", name: "edit", input: {}, status: "completed" },
      ],
    });
    const pageLatest = makeAssistant({
      ...
      contentOrder: [{ type: "text", id: "0" }],
      toolCalls: [
        { id: "tool-C-1", name: "test", input: {}, status: "completed" },
      ],
    });
