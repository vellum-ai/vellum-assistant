...
function TranscriptMessageBodyLegacy({
  message,
  assistantDisplayName,
  onSurfaceAction,
  onForkConversation,
 ...
 *
 * Leaf components and visual chrome (activity cards, inline links, thought
 * process links, surface router, user bubbles, attachments, subagent cards,
 * Slack attribution, hover actions) are shared with the legacy tree.
 */
function TranscriptMessageBodyFromBlocks({
  message,
  assistantDisplayName,
  onSurfaceAction,
  onForkConversation,
  ...
  // combined `ActivityRunCard`, a lone inline link, or a bare thought-process
  // link — mirroring the legacy interleaved branch but reading referents
  // straight off the grouped blocks.
  const renderActivityGroup = (
    ...
    for (const item of items) {
      if (item.kind === "thinking") {
        if (item.text) {
          thinkingContents.push(item.text);
          cardItems.push({
            kind: "thinking",
            text: item.text,
    ...
    // No renderable tool call — render the combined thinking as a minimal
    // inline `ThoughtProcessLink`, plus any spawn cards. A trailing run reads
    // as still-streaming only while the row is live.
    const combinedThinking = thinkingContents.join("\n");
  ...
  const renderGroupNode = (
    ...
    if (group.type === "surface") {
      return renderSurfaceNode(group.surfaceId, `b-surface-${gi}`);
    }
  ...
  const renderedSurfaceIds = new Set(
    groups.flatMap((g) => (g.type === "surface" ? [g.surfaceId] : [])),
  );
