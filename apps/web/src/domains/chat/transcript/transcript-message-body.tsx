...
/**
 * Positional render tree. Walks the message's `contentOrder` and resolves each
 * entry against the parallel `textSegments`/`thinkingSegments`/`toolCalls`/
 * `surfaces` arrays — `groupMessageActivityRuns` groups the order,
 * `resolveToolCall` / `resolveThinkingContent` / `resolveThinkingTiming` fill
 * content per entry, and surfaces resolve by id-or-index from
 * `message.surfaces`. `TranscriptRow` selects this tree by default and switches
 * to the blocks-driven `TranscriptMessageContent` under the
 * `renderFromContentBlocks` flag; the two share leaf components and visual
 * chrome.
 */
