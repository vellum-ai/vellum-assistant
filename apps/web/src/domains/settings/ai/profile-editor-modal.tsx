...
import { type ProfileEntry, formatCompactTokens } from "@/domains/settings/ai/page.js";
import { type Profile } from "@/domains/settings/ai/manage-profiles-modal.js";
import { resolveProfileParamVisibility } from "@/domains/settings/ai/profileParamVisibility.js";
...
function ProfileEditorModalInner({
  ...
  const visibleProviders = useMemo(() => {
    ...
    for (const c of connections ?? []) {
      if (c.status === "active") activeProviderSet.add(c.provider);
    }
    if (provider) activeProviderSet.add(provider);
    return ALL_PROVIDERS.filter((p) => activeProviderSet.has(p));
  ...
  const providerOptionsSource =
    connections === undefined ? ALL_PROVIDERS : visibleProviders;
  ...
  const availableModels: readonly { id: string; displayName: string }[] = useMemo(() => {
    ...
    return merged;
  }, [provider, resolvedProviderConnection, availableConnectionsForProvider]);
