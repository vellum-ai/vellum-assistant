...
import { PROVIDER_DISPLAY_NAMES as CATALOG_PROVIDER_DISPLAY_NAMES } from "@/lib/llm-model-catalog.js";
...
export interface ProviderConnection {
   ...
   * serialize time from its `MANAGED_CONNECTION_NAMES` set; the UI uses
   * it to render the read-only badge + view-only editor and disable the
   * trash button without mirroring the canonical name list locally.
   *
   * Optional for graceful degradation against older daemons that predate
   * the field. A missing value is treated as `false` (user-managed).
   */
  baseUrl: string | null;
...
// Narrowed view of the shared catalog map for the editor + manager consumers
// here. Source of truth: `@/lib/llm-model-catalog#PROVIDER_DISPLAY_NAMES`.
//
// The object literal is exhaustively type-checked against `ConnectionProvider`
// via `satisfies`: a future `ConnectionProvider` literal added without a
// matching entry will fail typecheck here at compile time. The runtime guard
// backstops the case where the upstream catalog is missing a `displayName`
// for a known provider; we fail at module load rather than render `undefined`
// in the UI.
    ...
    if (label === undefined) {
      throw new Error(
        `provider-connections-client: catalog missing displayName for ` +
          `ConnectionProvider "${provider}". Run ` +
          "`bun run sync:llm-model-catalog` and confirm the upstream " +
          "meta/llm-provider-catalog.json carries the entry.",
      );
    }
    out[provider] = label;
  }
...
// via Django's generic RuntimeProxyView. Each wrapper calls without throwOnError
// and throws an error with .status set to the HTTP status on failure, so callers
// can branch on e.g. 409/404 without inspecting the daemon error envelope.
// ---------------------------------------------------------------------------
...
// Defensive normalization: the daemon may not yet return status/label if the
// backend hasn't rolled out yet. Apply safe defaults so this client can land
// independently of the assistant daemon deploy.
...
// ---------------------------------------------------------------------------
// Secrets API — proxied through Django's RuntimeProxyWildcardView to the daemon.
// ---------------------------------------------------------------------------
