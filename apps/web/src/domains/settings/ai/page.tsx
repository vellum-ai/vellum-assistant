...
} from "@/lib/generated/web-search-provider-catalog.gen.js";
import { routes } from "@/lib/routes.js";
import { assistantDaemonConfigQueryKey } from "@/lib/sync/query-tags.js";
import { synthesizeTTS } from "@/lib/voice/tts-synthesize.js";
import { getLocalSetting, removeLocalSetting, setLocalSetting } from "@/domains/settings/_lib/local-settings.js";
import { CallSiteOverridesModal, type CallSiteOverrideDraft } from "@/domains/settings/ai/CallSiteOverridesModal.js";
...
// API key localStorage keys (matching service-keys page)
const LS_IMAGE_GEN_API_KEY = "vellum_gemini_key";
...
function EmailServiceCard({ assistantId }: EmailServiceCardProps) {
  ...
  const handleSaveMode = useCallback(async () => {
    setSavingMode(true);
    try {
      setLocalSetting(LS_EMAIL_MODE, mode);
      if (mode === "your-own") {
    ...
    >
      {mode === "managed" ? (
        <div className="space-y-4">
          ...
          {subscriptionQuery.isLoading ? (
            ...
            <Notice
              ...
              actions={
                <Button
                  size="compact"
                  onClick={() => push(routes.settings.billing)}
                >
                  Upgrade to Pro
                </Button>
              }
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={
                    registerDomain.isPending || !subdomainDraft.trim()
                  }
                >
          ...
          {/* Save persists `LS_EMAIL_MODE` so toggling back to Managed
              from Your Own is sticky across reloads. Mirrors the Web
              Search card pattern. */}
          {!isExplicitlyNotPro && !subscriptionQuery.isLoading && (
            <div className="flex items-center gap-2">
...
export default function ModelsServicesPage() {
  ...
  const handleImageGenSave = async () => {
    ...
    try {
      ...
      if (hasUserKey) {
        setLocalSetting(LS_IMAGE_GEN_API_KEY, trimmed);
        setImageGenApiKey("");
      }
  ...
  const handleImageGenReset = () => {
    removeLocalSetting(LS_IMAGE_GEN_API_KEY);
    setImageGenApiKey("");
