...
import { installDock, paintDefaultDockIcon } from "./dock";
...
app
  ...
  .then(async () => {
    ...
    installDock();
    // Dev-only: the Electron dev binary ships the atom Dock icon, so paint the
    // branded per-environment icon over it at startup to match a packaged
    // build. Relies on `build/icon.png` (generated by scripts/dev.ts before
    // launch); no-op if it's missing. Packaged builds get the icon from the
    // bundle natively.
    if (isDev) paintDefaultDockIcon();
    installPowerEvents();
    installTray({
