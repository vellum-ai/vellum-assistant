...
- **Linear tickets**: every PR opened by a Vellum team member or AI agent must reference a Linear issue (external community contributors are not expected to use Linear — follow [`CONTRIBUTING.md`](./CONTRIBUTING.md)). Search Illuminati (`LUM-`), Atlas (`ATL-`), and Jarvis (`JARVIS-`) for an existing ticket before creating a new one. If no match, create the issue retroactively and link it from the PR — every piece of internal work should be tracked.
  - **Branch name**: include the identifier, e.g. `do/jarvis-nnn-fix-stale-approvals`. Linear auto-links branches that contain the issue ID.
  - **1 PR = 1 issue is the default.** By default, every PR uses `Closes JARVIS-NNN` (or `Fixes` / `Resolves`) in its body and commit message so Linear auto-closes that issue on merge. Each PR represents a distinct unit of work with its own scope, lifecycle, and review surface, and the resulting Linear timeline is easier to read when one ticket maps to one merged change. For multi-step efforts, structure the work as either sub-issues under a parent (one coherent effort, phased sequential steps) per [Linear's parent and sub-issues docs](https://linear.app/docs/parent-and-sub-issues), or sibling issues at the project level (several independent efforts sharing a goal). When all sub-issues complete, close the parent — depending on the team's workflow settings ([Parent auto-close](https://linear.app/docs/parent-and-sub-issues#status-automation)), Linear may close it automatically; if not, close it manually. Err on the side of closing when there's no manual follow-up needed. If unsure, ask.
  - **Chained `Part of` is fine when the requester asks for it.** Linear's [GitHub integration](https://linear.app/docs/github#link-using-magic-words) explicitly supports non-closing magic words (`ref`, `refs`, `references`, `part of`, `related to`, `contributes to`, `toward`, `towards`) which advance an issue's status to In Progress / In Review per workflow settings but skip the Done transition on merge. This is the documented path for chaining multiple PRs against one Linear ticket without auto-closing it on the first merge. If the requester (or a slash command like `/run-plan` / `/blitz`) prefers that pattern, follow it — it's a supported Linear workflow, not a workaround. Err on the side of the 1-PR-1-issue default when the requester hasn't specified.
