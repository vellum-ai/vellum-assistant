...
## Persistence Layer vs. Pluggable Memory

Persistence is a layer **below** memory. Core storage infrastructure — the SQLite DB and migrations, conversation/message stores, the generic job queue, delivery/media/llm-log stores, and the embeddings + Qdrant infra — lives in `assistant/src/persistence/`. The memory *feature* (graph/v2/v3, retrieval, retrospective) lives in `assistant/src/memory/` and consumes `persistence/`. The dependency is one-way: `persistence/` must **not** import from `memory/` (guard: `assistant/src/__tests__/persistence-layering-guard.test.ts`, which allowlists only documented residual back-imports).

The memory system is **pluggable**. The built-in graph/v2/v3 systems are interchangeable implementations of an internal `MemoryProvider` interface (`assistant/src/memory/provider/`), selected by the `memory.provider` config (`auto | graph | v2 | v3 | none`) via `resolveMemoryProvider(config)` — not by scattered hard-coded gates. Memory's tools, routes, injection, and post-turn consolidation are all owned by the resolved provider. A third party can supply an external memory plugin against the public `@vellumai/plugin-api` `InitContext.host` facet contract (store/embeddings/vectorStore/history/jobs/providers) without importing `assistant/` source; a plugin declaring `provides: "memory"` can stand in for the built-in system, gated by the default-off `memory-plugin-provider` flag (one active memory plugin at a time). See `assistant/docs/memory-plugins.md` and the `ARCHITECTURE.md` § Persistence and Pluggable Memory. When adding storage infra, put it in `persistence/`; when adding memory behavior, keep it in `memory/` or a memory plugin behind the provider/facet contract.
