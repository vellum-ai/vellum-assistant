...
- **Windows subprocesses**: Every production, client, skill, build, or packaging subprocess that can run on Windows must set `windowsHide: true`. This applies to Node child process APIs and `Bun.spawn` / `Bun.spawnSync`. The terminal-facing launcher in `clients/windows/scripts/launch-cli.ts` is the exception because it must preserve normal CLI console behavior.
