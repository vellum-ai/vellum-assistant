...
- **Linear tickets**: every PR opened by a Vellum team member or AI agent must reference a Linear issue (external community contributors are not expected to use Linear — follow [`CONTRIBUTING.md`](./CONTRIBUTING.md)). Search Illuminati (`LUM-`), Atlas, and Jarvis (`JARVIS-`) for an existing ticket before creating a new one. If no match, create the issue retroactively and link it from the PR — every piece of internal work should be tracked.
