...
- `apps/` — End-user app surfaces. Currently hosts `apps/web/` (Vite + React Router v7 SPA, mid-migration from `vellum-assistant-platform/web/`), `apps/ios/` (Capacitor iOS shell that loads the web app in a WKWebView), and `apps/electron/` (Electron desktop shell for macOS that wraps `apps/web/` and supervises the bundled Bun daemon; distribution and auto-update wiring still to come, and the directory will be renamed `apps/macos/` once `clients/macos/` retires — tracked at [LUM-1908](https://linear.app/vellum/issue/LUM-1908)). The Chrome extension at `clients/chrome-extension/` will also move here (as `apps/chrome-extension/`) in a follow-up PR; see [`apps/README.md`](apps/README.md) and [`apps/AGENTS.md`](apps/AGENTS.md).
