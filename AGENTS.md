...
- **Linear tickets**: every PR opened by a Vellum team member or AI agent must reference a Linear issue (external community contributors are not expected to use Linear — follow [`CONTRIBUTING.md`](./CONTRIBUTING.md)). Search Illuminati (`LUM-`), Atlas (`ATL-`), and Jarvis (`JARVIS-`) for an existing ticket before creating a new one. If no match, create the issue retroactively and link it from the PR — every piece of internal work should be tracked.
  ...
  - **1 PR = 1 issue.** Every PR uses `Closes JARVIS-NNN` (or `Fixes` / `Resolves`) in its body and commit message so Linear auto-closes that issue on merge.
  - **Multi-PR efforts get multiple issues.** Structure multi-step work as either sub-issues under a parent (one coherent effort, phased sequential steps) per [Linear's sub-issues guidance](https://linear.app/docs/sub-issues), or sibling issues at the project level (several independent efforts sharing a goal). Each PR closes its own issue. When all sub-issues complete, close the parent manually — Linear doesn't do this automatically, but err on the side of closing when there's no manual follow-up needed. If unsure, ask.
  - **Why 1 PR = 1 issue (not because of Linear technical limits).** Linear's [GitHub integration docs](https://linear.app/docs/github#link-using-magic-words) recognize non-closing magic words (`ref`, `refs`, `references`, `part of`, `related to`, `contributes to`, `toward`, `towards`) which advance status to In Progress / In Review without auto-closing on merge — so chaining `Part of JARVIS-NNN` across multiple PRs does sync status correctly until the final closing PR. The 1-PR-1-issue rule exists for conceptual clarity, not technical necessity: each PR represents a distinct unit of work with its own scope, lifecycle, and review surface, and the resulting Linear timeline is easier to read when one ticket maps to one merged change. Use sub-issues / sibling issues to express multi-PR efforts.
