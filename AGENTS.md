...
- **Linear tickets**: every PR opened by a Vellum team member or AI agent must reference a Linear issue (external community contributors are not expected to use Linear — follow [`CONTRIBUTING.md`](./CONTRIBUTING.md)). Search Illuminati (`LUM-`), Atlas (`ATL-`), and Jarvis (`JARVIS-`) for an existing ticket before creating a new one. If no match, create the issue retroactively and link it from the PR — every piece of internal work should be tracked.
  ...
  - **Multi-PR efforts get multiple issues.** Do not chain N PRs against a single ticket via `Part of JARVIS-NNN` — that breaks Linear's auto-status sync. Structure multi-step work as either sub-issues under a parent (one coherent effort, phased sequential steps) per [Linear's sub-issues guidance](https://linear.app/docs/sub-issues), or sibling issues at the project level (several independent efforts sharing a goal). Each PR closes its own issue. When all sub-issues complete, close the parent manually — Linear doesn't do this automatically, but err on the side of closing when there's no manual follow-up needed. If unsure, ask.
