...
- **Credentials** are owned by the CES. Secret values live in `keys.enc`. Identity and policy records live in `<cesDataRoot>/credential-records.json`. The assistant and gateway access credentials via the CES HTTP API (`CES_CREDENTIAL_URL`) or CES RPC. Neither has filesystem access to `keys.enc` / `store.key`.
