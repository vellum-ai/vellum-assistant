---
name: "DoorDash"
description: "Order food, groceries, and convenience items from DoorDash using local scripts"
user-invocable: true
metadata:
  {
    "vellum":
...
You can order food from DoorDash for the user using the `doordash` CLI, which is backed by the scripts in `scripts/`.
...
```bash
doordash <subcommand> [options] --json
```

## Setup

**IMPORTANT: Always use `host_bash` (not `bash`) for all DoorDash commands.** The CLI needs host access for Chrome CDP and session cookies — none of which are available inside the sandbox.

`doordash` should be available on PATH. If not, prepend `PATH="$HOME/.vellum/bin:$PATH"` to commands.
Do NOT search for scripts or inspect wrapper internals. Just run the commands as documented below.
