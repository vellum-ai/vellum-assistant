...
import { extractRequests, saveRequests } from "./lib/request-extractor.js";
import {
  clearSession,
  importFromRecording,
  loadSession,
...
import { loadRecording } from "./lib/shared/recording-store.js";
...
Examples:
  $ amazon status
  $ amazon refresh
  $ amazon search "AA batteries" --limit 5
  $ amazon cart add --asin B07XXXXX --quantity 2
  $ amazon order place --payment-method-id pm_abc123`,
  );
...
Examples:
  $ amazon login --recording /path/to/recording.json
  $ amazon login --recording ~/recordings/amazon-2024-01-15.json`,
    )
...
Examples:
  $ amazon refresh-headless
  $ amazon refresh-headless --json`,
    )
...
Examples:
  $ amazon search "AA batteries"
  $ amazon search "whole milk" --fresh --limit 10
  $ amazon search "USB-C cable" --limit 5 --json`,
    )
    .action(
      async (
        query: string,
...
Examples:
  $ amazon product B07XXXXX
  $ amazon product B08YYYYY --fresh
...
Examples:
  $ amazon variations B07XXXXX
  $ amazon variations B07XXXXX --json`,
    )
...
Examples:
  $ amazon cart view
  $ amazon cart add --asin B07XXXXX --quantity 2
  $ amazon cart remove --cart-item-id CXYZ123`,
  );
...
Examples:
  $ amazon cart add --asin B07XXXXX
  $ amazon cart add --asin B08YYYYY --quantity 3 --fresh
  $ amazon cart add --asin B07XXXXX --verbose`,
    )
    .action(
      async (
        opts: {
...
Examples:
  $ amazon cart remove --cart-item-id CXYZ123
  $ amazon cart remove --cart-item-id CXYZ123 --json`,
    )
...
Fresh items must also be added to cart with the --fresh flag:
  $ amazon cart add --asin B08YYYYY --fresh

Examples:
  $ amazon fresh delivery-slots
  $ amazon fresh select-slot --slot-id slot_abc123`,
  );
...
Examples:
  $ amazon fresh delivery-slots
  $ amazon fresh delivery-slots --json`,
    )
    .action(async (_opts: unknown, cmd: Command) => {
      await run(cmd, async () => {
...
Examples:
  $ amazon fresh select-slot --slot-id slot_abc123
  $ amazon fresh select-slot --slot-id slot_abc123 --json`,
    )
...
Recommended workflow before placing an order:
  1. amazon cart view          (verify cart contents)
  2. amazon checkout           (review totals and shipping)
  3. Confirm with the user
  4. amazon order place
 ...
 */
async function extractSessionFromChromeCookies(): Promise<AmazonSession> {
  ...
  const cookies: import("./lib/shared/recording-types.js").ExtractedCredential[] =
    [];
