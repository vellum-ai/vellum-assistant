...
- **Missing `secret` guardrail**: Every flow returns a `secret` except Telegram's handle bootstrap, which returns a deep link first and sends the code once the user taps it. Naming the exception rather than listing the channels keeps this true as channels are added. If `secret` is unexpectedly absent from a start or resend response that otherwise indicates success, treat this as a control-plane error. Do NOT fabricate a code or tell the user to proceed without one. Instead, tell the user something went wrong and ask them to retry the start (Step 3) or resend (Step 4).
