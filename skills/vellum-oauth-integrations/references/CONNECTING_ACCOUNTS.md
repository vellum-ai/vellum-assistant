...
Managed apps only accept scopes the provider approved for Vellum's OAuth app, so your-own scope names do not carry over. For managed Google, Gmail access is the single scope `https://mail.google.com/`; the `gmail.*` scopes and `calendar.readonly` are not approved and fail the connection.
