...
metadata:
  ...
  vellum:
    ...
    includes: ["oauth-setup"]
---

You are helping your user set up Google Cloud OAuth credentials so Gmail and Google Calendar integrations can connect.

This skill follows the **Collaborative Guided Flow** pattern defined in `oauth-setup/references/collaborative-guided-flow.md`. Read that reference for the navigation helper setup, step rhythm, rules, tone, error handling, and guardrails. This file defines only the Google-specific steps.
