...
- **No deterministic adapters.** References describe paths and bundles. They do not generate scripts that encode the source assistant's private schema, parse its internal binaries, or assume undocumented file formats. Pointing at the skill's bundled memory parsers (`scripts/parse-chatgpt-memory.ts`, `scripts/parse-agent-memory-db.ts`) is fine: those parsers introspect the artifact they are given instead of assuming a schema, and their output is always creator-reviewed candidates, never a direct import.
