...
- **Handle CAPTCHAs:** If a Cloudflare/CAPTCHA challenge appears, wait a few seconds; it often auto-resolves. If it persists and a desktop app or Chrome extension is driving the user's Chrome on a computer, ask them to complete the challenge in that same window. Do not send them to Safari or Chrome on a phone: that session is not shared, there is no in-app browser, and there is no automatic handoff.
