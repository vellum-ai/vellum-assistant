...
- `connections` has an entry with `"status": "active"`: use the [proxy preamble](#proxy-preamble) on every `link-cli` call and skip `auth login` entirely. Several active entries: ask the user which account to use and pass it as `--account` in the preamble.
...
- Non-zero exit mentioning `Unknown provider`: Stripe Link OAuth is not enabled for this assistant. Use the device login path only.
...
| Link API call returns 401 or 403 on the connection path | Re-run the preamble for a fresh grant. If it persists, the connection lacks scopes: `assistant oauth disconnect stripe_link`, then reconnect |
