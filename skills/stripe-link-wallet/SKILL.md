...
| Link API call returns 401 or 403 on the connection path | Re-run the preamble for a fresh grant. If it persists, the connection lacks scopes and must be replaced: gate it with `assistant ui confirm`, and only on confirmation run `assistant oauth disconnect stripe_link` and show the connect surface again |
