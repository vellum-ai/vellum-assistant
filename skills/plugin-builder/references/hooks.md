...
| `exitReason`     | `AgentLoopExitReason`    | Read-only | Which terminal state the turn reached (for example `no_tool_calls`, `max_tokens_reached`, `error`). A hook that should act only on a particular ending guards on it.                                              |
