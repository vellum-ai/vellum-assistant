...
Each surface can also be dropped straight into the workspace at `/workspace/<surface>/<name>/` without wrapping it in a plugin. Workspace MCP servers live in `/workspace/mcp.json` (the same Agent Plugins document a plugin ships). A plugin is what lets you ship several surfaces together as one installable unit.
...
| `doesSupportVision`     | value | Check whether a profile's resolved model can process image input. Takes a `ModelProfileInfo` entry from `getModelProfiles()` and resolves the effective (provider, model) by merging the profile over the workspace default and inferring the provider for model-only profiles. Handles mix profiles (true if any arm supports vision). A profile `inputModalities.image` override wins over the catalog. Unknown models default to false (fail-close) so a fallback captions rather than shipping a raw image. Use this to gate image-processing logic on capability rather than model name strings. |
...
| Export                      | Kind  | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `resolveCredential`         | value | Resolve a stored credential to its plaintext value (the same value `assistant credentials reveal` prints) from a UUID or a `service/field` reference. When a plugin is in context (hook, plugin tool, or plugin route via AsyncLocalStorage), resolution is scoped to credentials whose service matches the plugin's runtime install name. A bash or skill-sandbox child inherits `VELLUM_PLUGIN_NAME` when exactly one plugin skill is active. A standalone process whose entry path sits under `plugins/<service>/skills/<skill>/{scripts,tools}/` is scoped the same way. Outside any plugin context the resolver is unscoped. A child with plugin context consults the live credential catalog on a cache miss. Throws `CredentialResolutionError` on failure. Do not call `assistant credentials reveal` from plugin code. |
