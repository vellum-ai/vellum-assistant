...
| Export                      | Kind  | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `resolveCredential`         | value | Resolve a stored credential to its plaintext value (the same value `assistant credentials reveal` prints) from a UUID or a `service/field` reference. When a plugin is in context (hook, plugin tool, or plugin route), resolution is scoped to credentials whose service matches the plugin's runtime install name. A plugin-resident skill script started with `assistant plugin-skill run`, or a plugin skill tool from `TOOLS.json`, presents a daemon-issued grant and is scoped the same way. Outside any plugin context or grant the resolver is unscoped. Throws `CredentialResolutionError` on failure. Do not call `assistant credentials reveal` from plugin code. |
