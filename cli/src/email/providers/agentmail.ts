 ...
 *
 * Pure transport/integration logic. No guardrails here.
 */
...
export class AgentMailProvider implements EmailProvider {
  ...
  async sendDraft(
    ...
    const inbox = inboxId ?? (await this.resolveDefaultInbox());
    const result = await this.client.inboxes.drafts.send(
      inbox,
      draftId,
      {},
    );
...
function mapDomain(
  d: import("agentmail").AgentMail.Domain,
): EmailDomain {
  return {
    id: d.domainId,
    domain: d.domainId,
...
function mapInbox(
  i: import("agentmail").AgentMail.inboxes.Inbox,
): EmailInbox {
  return {
    id: i.inboxId,
    address: i.inboxId,
...
function mapDraftItem(
  d: import("agentmail").AgentMail.DraftItem,
): EmailDraft {
  return {
    id: d.draftId,
    inboxId: d.inboxId,
...
function mapMessage(
  m: import("agentmail").AgentMail.Message,
): EmailMessage {
  return {
    id: m.messageId,
    threadId: m.threadId,
...
function mapThreadFull(
  t: import("agentmail").AgentMail.Thread,
): EmailThread {
  return {
    id: t.threadId,
    subject: t.subject,
