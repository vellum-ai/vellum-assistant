...
Examples: `hatch`, `wake`, `sleep`, `retire`, `ps`, `ssh` belong here. `config`, `memory` belong in `assistant/src/cli/`. Contact ACL writes belong on `vellum gateway contacts` (and the `gateway contacts` binary in the gateway package), not on `assistant contacts`.
