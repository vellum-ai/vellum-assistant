...
const COMPOSE = `function compose(S: any, key: string) {
  ...
  // Publish the composed args so the useArgs() stub can hand them back. Every
  // controlled story in this DS follows storybook's documented pattern —
  // \`const [{ value }] = useArgs(); <C {...args} value={value} />\` — which
  // overrides the correctly-merged arg with whatever useArgs returns. An empty
  // stub therefore renders every controlled input as undefined (unchecked
  // boxes, empty selects, valueless sliders). Set immediately before each
  // render call: story render fns run synchronously here, so the stub reads
  // the args of the story currently rendering.
  const publish = (a: any) => { (globalThis as any).__dsStoryArgs = a; };
  let render: (() => any) | null = null;
  if (st && typeof st.render === 'function') render = () => { publish(args); return st.render(args, ctx); };
  else if (typeof st === 'function') render = () => { publish(args); return st(args, ctx); };
  else if (typeof meta.render === 'function') render = () => { publish(args); return meta.render(args, ctx); };
  else {
