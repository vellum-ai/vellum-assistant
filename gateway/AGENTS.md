...
- **Schema notes.** There is no `source_type` column: the wire DTO's `sourceType` is derived from `source_channel` (`phone` → voice, `vellum` → desktop, else channel), and the list route translates a `sourceType` filter into `source_channel` predicates. The requester-side conversation is `source_conversation_id`. Request ids are caller-supplied and load-bearing: interaction-bound kinds (`tool_approval`, `pending_question`) reuse the pending-interaction id as PK, and every other kind mints a UUID per create. The insert is strict, so a caller that can create concurrently (the escalation path runs once per sibling tool call in a turn) must mint an id unique per call.
