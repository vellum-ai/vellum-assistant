...
- **Decision provenance is two columns, and neither is `updated_at`.** `decided_at` and `decided_via` are stamped by the decision CAS only, gated on a status that means a decision was made, so the direction-agnostic CAS cannot leave a decision timestamp on a row that is decidable again. Read them for "how long did the guardian take, and from where": `updated_at` also moves for delivery-row and followup writes and again when the expiry sweep confirms, so `updated_at - created_at` measures the last write of any kind, not a time-to-decision. `decided_via` is an open string set carrying a channel id for a person's decision or `system` for a machine-driven terminal transition, and it is the answer to a different question than `source_channel`: that one says where the request was RAISED, this one where it was ANSWERED. A caller that cannot name a surface omits it, leaving NULL rather than guessing.
