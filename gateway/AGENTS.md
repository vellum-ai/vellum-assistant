...
- **Decision provenance is two columns, and neither is `updated_at`.** `decided_at` and `decided_via` are written by the decision CAS only, and they track the status in both directions: a transition to a decided status stamps them, and a transition back to a decidable one clears them, because the CAS is direction-agnostic and a pending row must never report a decision that no longer stands. Read them for "how long did the guardian take, and from where": `updated_at` also moves for delivery-row and followup writes and again when the expiry sweep confirms, so `updated_at - created_at` measures the last write of any kind, not a time-to-decision. `decided_via` is an open string set carrying a channel id for a person's decision or `system` for a machine-driven terminal transition, and it is the answer to a different question than `source_channel`: that one says where the request was RAISED, this one where it was ANSWERED. A caller that cannot name a surface omits it, leaving NULL rather than guessing: the shared confirm route serves the web app, the IPC adapter and the CLI alike, and the trusted header set names a principal rather than a client interface, so that rail records no surface at all.
