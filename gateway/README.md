...
| `/webhook/a2a`                             | POST            | A2A inbound webhook — receives inter-assistant envelopes (pairing + messages). Auth varies per envelope type: `message` and `pairing_finalize` require Bearer token; `pairing_request` and `pairing_accepted` are unauthenticated. Gated by `feature_flags.assistant-a2a.enabled`.                      |
| `/deliver/a2a`                             | POST            | Internal endpoint for the assistant runtime to deliver outbound A2A messages. Accepts standard `ChannelReplyPayload`, wraps in `A2AMessageEnvelope`, resolves outbound token from credential store, and POSTs to target gateway's `/webhook/a2a`. Requires `gatewayUrl` and `assistantId` query params. |
