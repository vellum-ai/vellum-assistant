...
4. **Forward with assistantId** — When routing succeeds, the gateway forwards the voice webhook to the runtime at `POST /v1/internal/twilio/voice-webhook` with a JSON body containing `{ params, originalUrl, assistantId }`. The runtime calls `createInboundVoiceSession()` to bootstrap a session keyed by CallSid, then returns TwiML pointing Twilio to the media-stream WebSocket (`<Connect><Stream>`).
...
Caller → Twilio → Gateway /webhooks/twilio/voice (no callSessionId)
  ...
  → runtime returns TwiML (media-stream `<Connect><Stream>`)
  → Twilio opens WebSocket → Gateway /webhooks/twilio/media-stream/<callSessionId>/<token> → Runtime /v1/calls/media-stream
  → media-stream session detects inbound (`initiatedFromConversationId == null`), optional guardian verification gate, then receptionist-style LLM greeting
```
...
| `/webhooks/twilio/voice-verify`            | POST            | Gateway-owned `<Gather>` voice-verify callback for the inbound guardian DTMF/spoken-code gate (validated via HMAC-SHA1 signature)             |
| `/webhooks/twilio/media-stream/...`        | WS              | Twilio Media Streams WebSocket (bidirectional proxy to runtime; `callSessionId` and auth `token` encoded as URL path segments)                |
