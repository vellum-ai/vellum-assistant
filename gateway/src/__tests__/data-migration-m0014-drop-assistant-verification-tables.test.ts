/**
 * Tests for m0014-drop-assistant-verification-tables.
 *
 * Verifies that the drops are gated on m0013's one_time_migrations checkpoint
 ...
 * the migration is idempotent, and that it is registered after m0013. Uses
...
import {
  up as m0014Up,
  down as m0014Down,
  M0013_CHECKPOINT_KEY,
} from "../db/data-migrations/m0014-drop-assistant-verification-tables.js";
...
function checkpointM0013(): void {
  getGatewayDb()
    .insert(oneTimeMigrations)
    .values({ key: M0013_CHECKPOINT_KEY, ranAt: 1_000 })
    .run();
}

function uncheckpointM0013(): void {
  getGatewayDb().delete(oneTimeMigrations).run();
}

// ── Tests ───────────────────────────────────────────────────────────────────

describe("m0014-drop-assistant-verification-tables", () => {
  test("skips entirely (no drops) when m0013 is not checkpointed", async () => {
    uncheckpointM0013();

    const result = await m0014Up();
    ...
    checkpointM0013();
    expect(await m0014Up()).toBe("done");
    expect(fakeAssistantDb.tables.size).toBe(0);
  });

  test("drops both assistant tables once m0013 is checkpointed", async () => {
    const result = await m0014Up();
  ...
  test("is registered after m0013 and gates on m0013's registered checkpoint key", () => {
    const keys = MIGRATIONS.map((m) => m.key);
    const backfillIndex = keys.indexOf(M0013_CHECKPOINT_KEY);
    const dropIndex = keys.indexOf("m0014-drop-assistant-verification-tables");
