...
describe("Twilio webhook signature with canonical ingress base URL", () => {
  ...
  test("uses platform callback assistant ID for Twilio websocket URL", async () => {
      ...
      async () =>
        new Response(
          '<?xml version="1.0" encoding="UTF-8"?><Response><Connect>' +
            '<Stream url="wss://__VELLUM_PUBLIC_BASE_URL__/webhooks/twilio/media-stream/cs/__VELLUM_RELAY_TOKEN__"/>' +
            "</Connect></Response>",
          {
            status: 200,
            headers: { "Content-Type": "text/xml" },
    ...
    expect(body).toContain(
      `url="wss://velay-staging.vellum.ai/${assistantId}/webhooks/twilio/media-stream/cs/`,
    );
