/**
 * Tests for m0012-verification-sessions-backfill.
 *
...
mock.module("../db/assistant-db-proxy.js", () => ({
  ...
  assistantDbRun: mock(async () => {
    throw new Error("m0012 must not write to the assistant DB");
  }),
...
import {
  up as m0012Up,
  down as m0012Down,
} from "../db/data-migrations/m0012-verification-sessions-backfill.js";
...
describe("m0012-verification-sessions-backfill", () => {
  ...
  test("returns skip on IPC failure, then completes on retry", async () => {
    ...
    expect(await m0012Up()).toBe("skip");
    expect(gatewaySessionIds()).toEqual([]);
  ...
  test("is registered after m0011", () => {
    const keys = MIGRATIONS.map((m) => m.key);
    const m0011Index = keys.indexOf("m0011-drop-gw-verification-sessions");
    const m0012Index = keys.indexOf("m0012-verification-sessions-backfill");

    expect(m0011Index).toBeGreaterThanOrEqual(0);
    expect(m0012Index).toBe(m0011Index + 1);
  });

  test("down is a no-op (returns done)", () => {
    expect(m0012Down()).toBe("done");
  });
});
