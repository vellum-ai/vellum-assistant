...
const log = getLogger("m0013-verification-sessions-backfill");
...
export async function up(): Promise<MigrationResult> {
  ...
  try {
    ...
    log.info(
      ...
      "m0013: backfilled verification sessions + guardian rate limits into gateway",
    );
    ...
    log.error(
      { err },
      "m0013: verification sessions backfill failed — will retry on next startup",
    );
    return "skip";
  }
}
