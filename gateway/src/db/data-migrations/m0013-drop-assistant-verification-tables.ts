 ...
 *
 * Gated on m0012's `one_time_migrations` checkpoint: registration order alone
 * only guarantees m0012 ran *before* this migration, not that it succeeded
...
const log = getLogger("m0013-drop-assistant-verification-tables");

function getRawGatewayDb(): Database {
  return (getGatewayDb() as unknown as { $client: Database }).$client;
}

/** `one_time_migrations` key the runner records when m0012 completes. */
export const M0012_CHECKPOINT_KEY = "m0012-verification-sessions-backfill";
...
export async function up(): Promise<MigrationResult> {
  ...
  const backfillDone = gwDb
    ...
    .get(M0012_CHECKPOINT_KEY);
  if (!backfillDone) {
    log.info(
      "m0013: m0012 checkpoint absent — skipping drops until the backfill completes",
    );
    return "skip";
  }
