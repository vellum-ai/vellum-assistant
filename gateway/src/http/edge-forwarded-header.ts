 ...
 *
 * Loopback gates (`isLoopbackPeer`, and the loopback-only endpoints
 * `/v1/guardian/init` and `/v1/pair`) treat its presence as proof that the
 * request did NOT arrive directly from a local loopback client: it was
 * forwarded by the edge proxy, possibly from a remote browser over a tunnel
 * (e.g. ngrok). Because every hop in that chain is loopback
 ...
 * config generated by cli/src/lib/nginx-ingress.ts
 * (`proxy_set_header X-Vellum-Edge-Forwarded`). Keep the two in sync.
 */
