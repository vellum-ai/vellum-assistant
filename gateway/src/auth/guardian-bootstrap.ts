 ...
 * Uses INSERT OR REPLACE on the assistant side so re-verification on the
 * same channel updates the existing binding rather than failing on unique
 * constraints.
 *
 * Persona-file seeding (`users/<slug>.md`) and trust-rule cache
 * invalidation are assistant-side concerns — the assistant handles them
 * independently when it detects a guardian contact.
 */
export function createGuardianBinding(
  ...
  // Check if a channel binding already exists for this type + address
  const existingChannel = db
    .query<{ id: string }, [string, string]>(
      `SELECT id FROM contact_channels
       WHERE contact_id = ? AND type = ? LIMIT 1`,
    )
  ...
  log.info(
    { contactId, channelId, channel: params.channel, guardianPrincipalId: params.guardianPrincipalId },
