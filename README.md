...
| `/swarm [workers] [max-tasks]` | Parallel execution — spawns a pool of agents (default: one worker per task) that work through `.private/TODO.md` concurrently, each in its own worktree. PRs are auto-assigned to the current user. |
