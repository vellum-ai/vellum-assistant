...
**Shared utility:** `ActorCredentialRefresher` is a shared utility used by both platforms. It calls `GatewayHTTPClient.post(path:json:skipRetry:true)` so URL resolution and header construction are handled by the standard client path, while the `skipRetry` flag ensures the 401 retry interceptor is bypassed. `ActorTokenManager` on each platform delegates to this refresher for both proactive and reactive (401-recovery) refresh flows.
