...
> & {
  getClientId: () => string;
  /**
   * Computer-use and app-control executors contributed by the
   * `computer-use-actions` capability module. Absent when the native helper
   * feature is not installed, in which case `host_cu` and `host_app_control`
   * are reported as unavailable to the daemon.
   */
