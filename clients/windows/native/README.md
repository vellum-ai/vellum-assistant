...
The helper targets .NET 10 and `windows10.0.17763.0`. Supported deployments
must also remain within Microsoft's active .NET 10 Windows support matrix.

The Electron main process starts the helper as a same-user, non-elevated child
and exchanges newline-delimited JSON-RPC 2.0 frames over standard input and
...
Native coordinates use physical pixels in the Windows virtual desktop space,
including its possible negative origin. A capability that consumes logical or
