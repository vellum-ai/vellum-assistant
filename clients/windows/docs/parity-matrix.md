...
| `helper.hotkey` (voice mode chord, registration state)                | `preload/features/dictation.ts`, `main/features/voice-mode-chord.ts`, `native/.../VoiceModeChordService.cs`    | `native/mac-helper` (Fn)                    | `main/voice-mode-chord.test.ts`, `native/Vellum.WindowsHelper.Tests`                |
...
| `helper.hotkey.fnPushToTalk` (Fn voice mode tap)        | `helper.hotkey.setVoiceModeChord`: the voice mode shortcut's bare-modifier chord, registered with the helper's keyboard hook (`main/features/voice-mode-chord.ts`). |
