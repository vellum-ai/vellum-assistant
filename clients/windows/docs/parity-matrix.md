...
| `helper` (ping, state, restart, dictation partials)           | `preload/features/dictation.ts`, `main/features/dictation.ts`, `main/windows-helper.ts`                | `native/mac-helper`                      | `main/dictation.test.ts`, `native/Vellum.WindowsHelper.Tests`                   |
| `helper.hotkey` (push-to-talk chord, registration state)      | `preload/features/push-to-talk.ts`, `main/features/push-to-talk.ts`, `native/.../PushToTalkService.cs` | `native/mac-helper` (Fn)                 | `main/push-to-talk.test.ts`, `native/Vellum.WindowsHelper.Tests`                |
...
| `helper.dictation.transcribe`, `onTranscribed` (offline whole-utterance recognizer) | None. Windows dictation streams partials plus a finalized transcript.                                                              |
