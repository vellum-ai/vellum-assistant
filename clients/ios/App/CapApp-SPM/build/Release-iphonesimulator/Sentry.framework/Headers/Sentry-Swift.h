...
/// <code>HTTPURLResponse</code> extension method above, so this thin static wrapper exposes it. We can’t call
/// <code>-[NSHTTPURLResponse valueForHTTPHeaderField:]</code> from Objective-C directly because it isn’t
...
/// We build it ourselves instead of using <code>String(describing:)</code>, because the default <code>UIView</code>
/// description embeds <code>frame = (x y; w h)</code>. Those coordinates can leak which key a user tapped in
/// custom PIN/passcode views, so we never include the frame.
+ (NSString * _Nonnull)getSanitizedViewDescription:(UIView * _Nonnull)view SWIFT_WARN_UNUSED_RESULT;
