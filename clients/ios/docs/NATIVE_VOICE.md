...
| Compact vs minimal during a call | **The simulator cannot answer this.** It shows the compact presentation, because its mocked microphone raises no privacy indicator and so never shares the island. A device does: the indicator is up for the whole call, muted included, and iOS falls back to the minimal presentation. Treat a simulator screenshot of the island as void here, the same way it is for anything else about audio |
