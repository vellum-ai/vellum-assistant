...
| Which presentation a call gets | Measured 2026-08-04 on an iPhone 17 Pro simulator: a running session renders the **compact** presentation, avatar leading and phase glyph trailing. It does not fall back to `minimal`, so the compact slots are what a call is actually seen in. `minimal` is for a shared island (a second activity), which is why that slot carries identity rather than phase |
