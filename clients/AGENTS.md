...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  1. **Alternating `executeScrollToBottom`** — scroll-to-bottom alternates between edge-based (`scrollToEdge(.bottom)`) and ID-based (`scrollTo(id: lastMessageId, .bottom)`) strategies on each call via a `recoveryAlternator` toggle. `ScrollPosition` is a value type — repeated writes of the same value are silently deduped by SwiftUI's binding update mechanism. By alternating, each recovery attempt produces a structurally different `ScrollPosition` value that SwiftUI is guaranteed to process. The two strategies also use different estimation paths (total-content-height vs per-item cumulative), which may land the viewport at slightly different positions and help LazyVStack converge faster. User-initiated (CTA) path fires edge-based first (breaks any stale position), then ID-based for precision, wrapped in `withAnimation(VAnimation.spring)`. Auto-follow/recovery paths use `VAnimation.fast` or no animation. Falls back to edge-only for empty conversations (`lastMessageId == nil`). All scrolls are synchronous — no deferred Tasks. See [SO#79884780](https://stackoverflow.com/q/79884780).
