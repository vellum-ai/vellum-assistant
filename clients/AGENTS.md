...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  1. **Dual `executeScrollToBottom`** — calls `scrollToEdge(.bottom)` immediately (edge-based, targets estimated height) then `scrollTo(id:)` on the next run-loop pass (ID-based, forces view materialization). The ID-based scroll prefers `lastMessageId` (a ForEach item, always indexable by SwiftUI even when not materialized) over the standalone `"scroll-bottom-anchor"` view (outside ForEach, only locatable when materialized). The two calls run in separate transactions so they don't overwrite each other. The ID-based Task is tracked via `pendingIdScrollTask` and cancelled on user scroll-up, `reset()`, and `cancelAll()` to prevent accumulation across rapid switches. The Task also checks `mode.allowsAutoScroll` before executing, so stale scrolls don't fight user intent. See [SO#79884780](https://stackoverflow.com/q/79884780).
  ...
  7. **`phaseAllowsAutoFollow` guard** — content-height auto-follow, persistent recovery, AND `handleReachedBottom()` in the geometry handler all share the same phase guard: `!scrollPhase.isScrolling || scrollPhase == .animating` (i.e., `.idle` or `.animating`). This blocks during ALL user-initiated scroll phases: `.tracking` (finger down), `.interacting` (dragging), and `.decelerating` (momentum after release). Apple's `ScrollPhase.isScrolling` is `phase != .idle` ([docs](https://developer.apple.com/documentation/swiftui/scrollphase/isscrolling)); the `.animating` exception allows our own programmatic scrolls to proceed. Without the deceleration guard, auto-follow fires during scroll momentum and fights the user's fling, causing snap-back flicker. The phase guard on `handleReachedBottom()` prevents mode oscillation between `.freeBrowsing` and `.followingBottom` within the 16ms UI-sync debounce window — without it, the "Scroll to latest" CTA can never appear because the mode flips back to `.followingBottom` before the debounced sync fires. The `onScrollPhaseChange` idle handler provides deferred reattach when the scroll settles. User scroll-up detection (`handleUserScrollUp`) remains gated on `.interacting` only — intentional drag, not momentum.
  8. **CTA edge-based scroll with spring animation** — user-initiated CTA taps use `scrollToEdge(.bottom)` as the primary scroll (reaches the actual content bottom, not just the last message — there's padding/anchor below). Wrapped in `withAnimation(VAnimation.spring)` (0.3s response, 0.8 damping) for a smooth, visible animation matching industry standard (ChatGPT, Claude, Slack, iMessage). An ID-based correction fires on the next run loop via Task to fix edge overshoot into blank LazyVStack estimated space. The same `withAnimation` block also drives the CTA's `.move(edge: .bottom).combined(with: .opacity)` exit transition via `syncUIImmediately()`. Auto-follow/recovery paths still use the dual edge+ID strategy with `VAnimation.fast`.
  8a. **Resize recovery window** — `handleContainerWidthChanged` always re-pins after a width change when `isFollowingBottom` (no `isAtBottom` check). After a resize, LazyVStack re-estimates content heights and the viewport can be at the *estimated* bottom (blank space) where `distanceFromBottom ≈ 0` → `isAtBottom = true`. A fresh 2-second recovery window ensures persistent recovery fires unconditionally until the actual bottom materializes.
