...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  ...
  8. **CTA ID-based scroll with spring animation** — user-initiated CTA taps use `scrollTo(id: lastMessageId, anchor: .bottom)` synchronously (NOT `scrollToEdge(.bottom)`). Edge-based scroll targets the *estimated* content bottom, which overshoots into blank LazyVStack space. ID-based targets actual ForEach content — SwiftUI locates it in the data source even when not materialized. The scroll is wrapped in `withAnimation(VAnimation.spring)` (0.3s response, 0.8 damping) for a smooth, visible animation matching industry standard (ChatGPT, Claude, Slack, iMessage all use visible animated scroll-to-bottom). The same `withAnimation` block also drives the CTA's `.move(edge: .bottom).combined(with: .opacity)` exit transition via `syncUIImmediately()`. Auto-follow/recovery paths still use the dual edge+ID strategy (fast, near-bottom adjustments don't need visible animation).
