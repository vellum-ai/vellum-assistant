...
- **`MainWindowView.coreLayoutBase` animation-scope policy is intentionally asymmetric.** `coreLayoutBase` watches two values that change the sidebar/chat frame — `sidebarExpanded` and `isSettingsOpen` — and they are handled differently on purpose:
  - **`sidebarExpanded` — keep the ambient `.animation(VAnimation.panel, value: sidebarExpanded)` on BOTH the sidebar column and `chatContentView`.** They must transition in sync; removing it from either side causes the other to snap instantly. The sidebar-toggle keyboard shortcut in `AppDelegate+InputMonitors.swift` writes `UserDefaults.standard` directly from an `NSEvent` monitor (not a SwiftUI update path), so the ambient modifier is the only thing that animates that path without requiring the monitor to hop onto the main actor with a `withAnimation` wrapper. This has regressed before — when refactoring `MainWindowView` layout, verify both columns retain their `sidebarExpanded` animation modifiers.
  - **`isSettingsOpen` — do NOT add an ambient `.animation(_:value: isSettingsOpen)` on either column.** The chat subtree contains a conditional `.flipped()` (rotation + scale) transform on typing-indicator rows (`MessageListContentView.latestEdgeActivityRow`); [per Apple](https://developer.apple.com/documentation/swiftui/view/animation(_:value:)), an ambient `.animation(_:value:)` interpolates every animatable modifier inside its scope when the watched value changes, which made the thinking dots sweep through 90°–135° when a panel opened/closed during streaming (LUM-821, PR #27254). Instead, drive transitions into/out of `.panel(.settings)` and `.panel(.logsAndUsage)` from the state-mutation sites with an explicit `withAnimation(VAnimation.panel) { … }` wrapper. `MainWindowState.showPanel(_:)` wraps its own body so any caller outside SwiftUI (command-palette actions, menu items, `NSEvent` monitors) gets the transition for free. See also [WWDC23 — Wind your way through advanced animations in SwiftUI](https://developer.apple.com/videos/play/wwdc2023/10156/).
