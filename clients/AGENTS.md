...
Programmatic scrolling uses a hybrid approach: item-targeted scrolls (`scrollTo`) go through `ScrollViewReader`'s `ScrollViewProxy.scrollTo(_:anchor:)` because `ScrollPosition.scrollTo(id:anchor:)` has known reliability issues with `LazyVStack` on macOS 15 (scrolls silently fail). Edge scrolls (`scrollToEdge`) continue to use `ScrollPosition.scrollTo(edge:)` which works reliably. The scroll state stores `scrollTo: (any Hashable, UnitPoint?) -> Void` (capturing the `ScrollViewProxy`) and `scrollToEdge: (Edge) -> Void` (capturing the `ScrollPosition` binding) closures configured during view setup. When entering push-to-top mode, the `$scrollPosition` binding is also updated to `ScrollPosition(id:anchor:)` so SwiftUI maintains the anchor during content-size changes (streaming). Ref: [ScrollViewProxy](https://developer.apple.com/documentation/swiftui/scrollviewproxy), [ScrollPosition](https://developer.apple.com/documentation/swiftui/scrollposition).
