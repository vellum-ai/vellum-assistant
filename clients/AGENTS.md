...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  ...
  7. **`scrollPhase != .interacting` guard** — both content-height auto-follow and persistent recovery skip when the user is actively touching the trackpad/mouse. Prevents a race where `onScrollGeometryChange` fires before `onScrollPhaseChange` updates the phase, which could cause a single recovery fire before `handleUserScrollUp()` transitions to `freeBrowsing`.
