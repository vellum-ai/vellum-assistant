...
Apply `@MainActor` only to types whose stored properties drive SwiftUI view rendering. Everything else should be `nonisolated` so it can run on the cooperative thread pool. This is the single most important rule for preventing app hangs. (Ref: [WWDC25 — Embracing Swift concurrency](https://developer.apple.com/videos/play/wwdc2025/232/), [Apple — Swift concurrency documentation](https://developer.apple.com/documentation/swift/concurrency))

> **⚠️ Migration in progress.** The network layer (`clients/shared/Network/`) currently has `@MainActor` on ~46 files — this is being migrated to nonisolated (see LUM-492). The table below describes the **target state**. For new code, follow these rules. For existing code, follow the migration plan in the linked Linear issues.

> **Note on executor semantics:** Making a type `nonisolated` does not guarantee its code runs off the main thread. Nonisolated *synchronous* code called from a `@MainActor` context still runs on the caller's (main) executor until the first suspension point. In this codebase, network client methods are `async` and suspend at `URLSession.data(for:)`, so after the first `await` the runtime moves execution to the cooperative thread pool. The key benefit of `nonisolated` is that it *allows* the runtime to schedule work off the main thread — `@MainActor` *prevents* it. (Ref: [Swift Forums — Nonisolated async functions](https://forums.swift.org/t/se-0338-clarify-the-execution-of-non-actor-isolated-async-functions/54444))
...
- **CPU-bound work belongs off the main thread.** Image resizing, compression, base64 encoding, and JSON serialization should use `Task.detached` or `nonisolated async` functions to avoid blocking the UI. Note: `nonisolated` alone is not sufficient for CPU-bound *synchronous* work called from `@MainActor` — use `Task.detached` to explicitly escape actor isolation. (Ref: [SE-0338 — Clarify the Execution of Non-Actor-Isolated Async Functions](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0338-clarify-execution-non-actor-async.md))
...
- **Beware async cascading.** Making a synchronous function `async` forces all callers to become `async` too. Each `await` point introduces an actor hop (context-switch overhead) and an **actor reentrancy window** — while suspended, other tasks can run on the main actor and mutate shared state, leading to subtle ordering bugs. Unnecessary `async` propagation also increases latency and makes call chains harder to reason about. Keep functions synchronous unless they genuinely need to suspend. (Ref: [WWDC21 — Protect mutable state with Swift actors](https://developer.apple.com/videos/play/wwdc2021/10133/), [Swift — Actor reentrancy](https://docs.swift.org/swift-book/documentation/the-swift-programming-language/concurrency/#Actors))
...
5. Network client structs and protocols should be `nonisolated` (no `@MainActor`). `GatewayHTTPClient` is a stateless enum with static methods — it and its client wrappers are being migrated off `@MainActor` (see LUM-492). **For new network clients, do not add `@MainActor`** — they should be nonisolated so their work runs on the cooperative thread pool. See § "@MainActor Isolation Boundaries" above.
