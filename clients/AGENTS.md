...
- **Bottom detection:** Use `.onScrollGeometryChange(for:of:action:)` to compute `effectiveContentHeight - contentOffsetY - visibleRectHeight` and set `isAtBottom` using asymmetric hysteresis (10pt enter threshold, 50pt leave threshold) to prevent oscillation during streaming. Do not use `ScrollPosition.viewID` for bottom detection — `viewID` becomes `nil` on user-initiated scroll, making it unreliable for continuous tracking.
