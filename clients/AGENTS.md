...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  1. **Imperative scroll methods, ID-only strategy** — all scroll closures (`scrollToEdge`, `scrollTo`) use Apple's imperative `.scrollTo()` mutating methods on the `ScrollPosition` binding (e.g. `binding.wrappedValue.scrollTo(id: target, anchor: .bottom)`), NOT value replacement (`binding.wrappedValue = ScrollPosition(edge: .bottom)`). This is Apple's recommended pattern for programmatic scrolling — a command ("scroll to X now") rather than a state declaration ("scroll state is X"). Value replacement was empirically unreliable: repeated writes of the same struct value were silently deduped. **All paths** (CTA, recovery, auto-follow) use **ID-based scroll only** — targeting real ForEach items that SwiftUI can locate even when not materialized. Within `executeScrollToBottom()`, edge-based scroll (`scrollToEdge(.bottom)`) is NEVER used when `lastMessageId` is available — it computes total content height from LazyVStack estimates, which overshoot into blank space for long conversations with unmaterialized items. (Note: conversation-switch positioning in section 6 uses a separate edge-based `scrollPosition.scrollTo(edge: .bottom)` call directly on the binding — this is intentional because `lastMessageId` may point to a stale conversation's message at that point.) ID-based scroll lands short (showing messages) rather than overshooting (showing blank space). Each recovery attempt materializes views near the target, improving estimates, converging monotonically to the actual bottom. Edge-based scroll is only used as a fallback for empty conversations (`lastMessageId == nil`). All scrolls are synchronous. See [SO#79884780](https://stackoverflow.com/q/79884780).
  ...
  3. **Recovery-window bottom-recovery** — during the recovery window (section 4), re-pins on every geometry update (throttled to 100ms) when mode allows auto-scroll, using ID-based scroll only. Outside the recovery window, the content-height auto-follow (section 2) handles ongoing scroll tracking. If the viewport is still stuck after the deadline, the CTA ("Scroll to latest") provides manual recovery.
  ...
  6. **Imperative edge-based conversation-switch positioning** — on conversation switch, `handleConversationSwitched()` calls `scrollPosition.scrollTo(edge: .bottom)` (imperative method) to give SwiftUI a direct "scroll to bottom now" command for the newly created ScrollView (via `.id(conversationId)`). `.defaultScrollAnchor(.bottom, for: .initialOffset)` is still set but scoped to initial offset (not `.sizeChanges`) because the `.sizeChanges` anchor role fights user scroll-up during streaming. An explicit `ScrollPosition(id: lastId, anchor: .bottom)` must NOT be used here — for long conversations, cumulative height-estimate error across hundreds of unmaterialized items places the viewport in blank LazyVStack space. `scrollPosition` is NOT cleared in `onDisappear` — the old ScrollView's `onDisappear` fires AFTER the new ScrollView's `onAppear` (confirmed by diagnostic logs: 10-225ms delay), so clearing would overwrite the `scrollTo(edge: .bottom)` that `handleConversationSwitched()` just issued. Stale-dedup is addressed by: imperative `.scrollTo()` methods, the `restoreScrollToBottom()` 100ms fallback, content-height auto-follow on message load, and the recovery window. An `onChange(of: conversationId)` safety net guarantees `handleConversationSwitched()` runs even when SwiftUI coalesces `.id()` lifecycle events during rapid switching.
  7. **`phaseAllowsAutoFollow` guard** — content-height auto-follow, persistent recovery, AND `handleReachedBottom()` in the geometry handler all share the same phase guard: `!scrollPhase.isScrolling` (i.e., `.idle` only). This blocks during ALL non-idle scroll phases: `.tracking` (finger down), `.interacting` (dragging), `.decelerating` (momentum after release), AND `.animating` (programmatic scroll in flight). Apple's `ScrollPhase.isScrolling` is `phase != .idle` ([docs](https://developer.apple.com/documentation/swiftui/scrollphase/isscrolling)). No `.animating` exception — recovery calling non-animated `requestPinToBottom()` during an in-flight spring animation (e.g. CTA smooth scroll) would interrupt it with a jarring jump. Streaming auto-follow is unaffected because it uses non-animated ID-based scroll writes which don't trigger `.animating` — the position changes instantly and `scrollPhase` stays `.idle`. Animated pins (`handleSendingChanged`, `handleMessagesCountChanged`) briefly trigger `.animating` (~150ms with `VAnimation.fast`), during which auto-follow pauses; after completion, `.idle` resumes and auto-follow fires immediately. The phase guard on `handleReachedBottom()` prevents mode oscillation between `.freeBrowsing` and `.followingBottom` within the 16ms UI-sync debounce window — without it, the "Scroll to latest" CTA can never appear because the mode flips back to `.followingBottom` before the debounced sync fires. The `onScrollPhaseChange` idle handler provides deferred reattach when the scroll settles. User scroll-up detection (`handleUserScrollUp`) fires during both `.interacting` (direct trackpad contact) and `.decelerating` (momentum after finger lift). A fast trackpad flick has a very brief `.interacting` window — sometimes only 1-2 geometry updates fire before the phase transitions to `.decelerating`. Without `.decelerating` detection, `handleUserScrollUp()` can miss the scroll-up entirely and the CTA never appears. `.decelerating` is exclusively user-initiated momentum (programmatic scrolls use `.animating`), so this is safe. `scrollPhase` is reset to `.idle` in `reset()` to prevent stale phases from a previous conversation blocking recovery during the new conversation's materialization window.
  8. **CTA scroll with spring animation** — user-initiated CTA taps fire `scrollTo(id: lastMessageId, .bottom)` using the imperative `.scrollTo()` method. Wrapped in `withAnimation(VAnimation.spring)` (0.3s response, 0.8 damping) matching industry standard (ChatGPT, Claude, Slack, iMessage). The recovery window (set in `requestPinToBottom`) handles the gap between `lastMessageId` and the absolute content bottom (padding/anchor below) — recovery is phase-gated (`.idle` only), so it waits until the spring completes, then fires clean ID-based corrections. Each recovery attempt materializes views near the target, improving LazyVStack estimates for the next attempt — converging monotonically without ever overshooting into blank space. If the user scrolls up before the spring completes, they start from real content (not blank space). The same `withAnimation` block also drives the CTA's `.move(edge: .bottom).combined(with: .opacity)` exit transition via `syncUIImmediately()`. All paths (CTA, recovery, auto-follow) use ID-based scroll only (section 1). The 100ms recovery cycle handles convergence — each attempt materializes views near the bottom, correcting estimates, until the bottom anchor materializes and recovery stops.
...
Programmatic scrolling uses `scrollPosition.scrollTo(id:anchor:)` and `scrollPosition.scrollTo(edge:)` (imperative mutating methods on `ScrollPosition`) instead of `ScrollViewReader` + `proxy.scrollTo()`. The scroll state stores `scrollTo: (any Hashable, UnitPoint?) -> Void` and `scrollToEdge: (Edge) -> Void` closures configured during view setup that capture the view-owned `ScrollPosition` binding and call the imperative methods on `binding.wrappedValue`. Do NOT use value replacement (`binding.wrappedValue = ScrollPosition(...)`) — this is empirically unreliable due to SwiftUI silently deduplicating repeated writes of the same struct value.
