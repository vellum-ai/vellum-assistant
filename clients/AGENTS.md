...
- **Cache expensive derived values.** Never expose a frequently-read, O(n) property as a plain computed var. Convert it to a `@Published` stored var updated on write with a `throttle` or `debounce` (100 ms is a good default for high-frequency publishers like streaming; adjust for the specific use case). Move any heavy work (sorting, JSON sizing, filtering large collections) off the main thread before updating the stored var.
...
- **Allow a layout settle delay before restoring scroll position after pagination prepend.** After inserting new messages during pagination, wait at least **32 ms** before calling `proxy.scrollTo(anchor)` so SwiftUI can finish its layout pass. If any inserted content performs an animated height change (e.g., `InlineVideoEmbedCard` at 0.25 s), increase the delay to at least the animation duration (100 ms minimum) — restoring scroll mid-animation lands at the wrong position. This delay is specific to pagination scroll restoration; it is not needed for normal message appends or streaming updates.
...
| Mutable array in `DispatchGroup` callbacks | Race condition — callbacks may run on different threads | In legacy GCD code, wrap each append in `DispatchQueue.main.async`. For new code, prefer actor isolation or `@Sendable` closures |
