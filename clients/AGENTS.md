...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  1. **Imperative scroll methods + recovery-only alternation** — all scroll closures (`scrollToEdge`, `scrollTo`) use Apple's imperative `.scrollTo()` mutating methods on the `ScrollPosition` binding (e.g. `binding.wrappedValue.scrollTo(edge: .bottom)`), NOT value replacement (`binding.wrappedValue = ScrollPosition(edge: .bottom)`). This is Apple's recommended pattern for programmatic scrolling — a command ("scroll to X now") rather than a state declaration ("scroll state is X"). Value replacement was empirically unreliable: repeated writes of the same struct value were silently deduped. Three execution paths: **Auto-follow** (content-height changes, streaming) uses ID-based only — targets real ForEach items, never overshoots into blank space. **Recovery** (persistent bottom-recovery window) alternates edge/ID via `recoveryAlternator` to break potential dedup and converge from different estimation paths. **CTA** fires edge first (breaks stale position), then ID for precision. Falls back to edge-only for empty conversations. All scrolls are synchronous. See [SO#79884780](https://stackoverflow.com/q/79884780).
  ...
  8. **CTA two-step scroll with spring animation** — user-initiated CTA taps fire `scrollToEdge(.bottom)` first (breaks any stale position), then `scrollTo(id: lastMessageId, .bottom)` for precise targeting. Both use imperative `.scrollTo()` methods. SwiftUI coalesces both writes in the same transaction — the user sees one smooth animated scroll. Wrapped in `withAnimation(VAnimation.spring)` (0.3s response, 0.8 damping) matching industry standard (ChatGPT, Claude, Slack, iMessage). The recovery window (set in `requestPinToBottom`) handles the small gap between `lastMessageId` and the absolute content bottom (padding/anchor below) — recovery is phase-gated (`.idle` only), so it waits until the spring completes, then fires clean corrections using the alternating strategy. If the user scrolls up before the spring completes, they start from real content (not blank space). The same `withAnimation` block also drives the CTA's `.move(edge: .bottom).combined(with: .opacity)` exit transition via `syncUIImmediately()`. Content-height auto-follow (section 2) uses ID-based only (never edge-based, which can overshoot on long conversations). Persistent bottom-recovery (sections 3-4) uses the alternating edge/ID strategy via `requestPinToBottom(forRecovery: true)`. The 100ms recovery cycle handles convergence — each attempt materializes views near the bottom, correcting estimates, until the bottom anchor materializes and recovery stops.
