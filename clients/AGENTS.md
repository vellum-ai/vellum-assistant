...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  ...
  8. **CTA recovery window** — `requestPinToBottom(userInitiated: true)` (CTA tap) resets `bottomAnchorAppeared` and starts a fresh 2-second recovery window. Without this, the CTA tap can land in blank estimated space with no recovery because the initial recovery window has long expired.
  9. **CTA visibility during stabilization** — `.stabilizing(previousMode: .freeBrowsing)` preserves `showsScrollToLatest = true` so the CTA doesn't flash off during expansion/resize stabilization windows.
  10. **`restoreScrollToBottom` stabilization guard** — the 100ms restore fallback also skips in `.stabilizing` mode, preventing it from overriding the stabilization and permanently leaking `activeStabilizationCount` (because `endStabilization()` checks `guard case .stabilizing = mode`).
