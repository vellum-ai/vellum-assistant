...
- **Do not use `.textSelection(.enabled)` on macOS.** Each usage creates a private `SelectionOverlay` `NSViewRepresentable` that triggers an expensive [`NSTextFieldCell` font/bezel cascade](https://linear.app/vellum/issue/LUM-615) during view graph updates (~2 s hang with many instances in a `LazyVStack`). Use `.textSelectionIfAvailable()` instead — it applies `.textSelection(.enabled)` on iOS/visionOS but is a no-op on macOS. For macOS text that must be natively selectable, use `VSelectableTextView` (AppKit `NSTextView` wrapper in `DesignSystem/Components/Display/SelectableTextView.swift`).
