...
> The message list uses a simplified scroll architecture with no auto-follow.
> The user scrolls manually; a "Scroll to latest" CTA appears when scrolled away from the bottom.

### Architecture

All scroll state lives as `@State` properties on `MessageListView` — no external state machine or coordinator. Key properties:

- **`scrollPosition`** (`ScrollPosition`) — native SwiftUI scroll position ([docs](https://developer.apple.com/documentation/swiftui/scrollposition)).
- **`isAtBottom`** (`Bool`) — whether the viewport is near the content bottom. Driven by `onScrollGeometryChange` with asymmetric hysteresis (30pt leave, 10pt enter) via a `BottomDetection` struct to prevent oscillation. See [onScrollGeometryChange](https://developer.apple.com/documentation/swiftui/view/onscrollgeometrychange(for:of:action:)).
- **`viewportHeight`** (`CGFloat`) — measured viewport height, passed to `MessageListContentView` for the active-turn `minHeight` calculation.

Scroll-to-latest: `ScrollToLatestOverlayView` reads `isAtBottom` and fires `scrollPosition.scrollTo(id:anchor:)` on tap. No auto-follow — the user controls their scroll position at all times.

### Bottom Detection

Uses `onScrollGeometryChange(for: BottomDetection.self)` returning a struct with two Bool thresholds. The action closure applies asymmetric hysteresis:
- **Leave threshold (30pt):** `isAtBottom` stays `true` until distance exceeds 30pt.
- **Enter threshold (10pt):** `isAtBottom` becomes `true` only when distance ≤ 10pt.

This prevents oscillation during streaming. The `BottomDetection` struct ensures the transform closure does not read `@State`, so SwiftUI only fires the action when the Bool pair changes — not on every frame.

### Pagination

Pagination uses `onScrollGeometryChange(for: Bool.self)` to detect when a sentinel near the top enters the trigger band. A 500ms cooldown (`lastPaginationCompletedAt`) and an in-flight guard (`isPaginationInFlight`) prevent cascading loads. After loading, scroll position is preserved by anchoring to the previously-top-visible message ID.

### Conversation Switching

On conversation switch, scroll state resets: `scrollPosition.scrollTo(edge: .bottom)`, `isAtBottom = true`, and the projection cache clears. `.defaultScrollAnchor(.bottom, for: .initialOffset)` handles the initial layout. The `.id(conversationId)` modifier on the ScrollView ensures a fresh view instance per conversation.

### Upstream Observation Fixes

- **Body-level circuit breaker.** When `isThrottled` is true (>100 body evaluations in 2s), `derivedState` returns a cached projection instead of recomputing. This caps cost regardless of the loop source.
- **AssistantActivitySnapshot equality.** Captures only structural properties — not per-token text content — so per-token mutations don't cascade into body re-evaluation.

### Rules

- **No auto-follow.** Do not re-introduce auto-scroll-to-bottom on new messages or streaming content. The user scrolls manually; the CTA provides scroll-to-latest.
- **Use `scrollPosition.scrollTo(id:anchor:)` for programmatic scrolling.** Do not use `ScrollViewReader` or value replacement (`ScrollPosition(edge:)`). Ref: [ScrollPosition](https://developer.apple.com/documentation/swiftui/scrollposition).
- **Use an in-flight guard for pagination.** `isPaginationInFlight` prevents stacking concurrent loads from rapid scroll-to-top.
- **Forward scroll/wheel events from gesture-capturing subviews.** `WKWebView` and similar responders swallow scroll events — override `scrollWheel(_:)` to forward to `nextResponder`.
- **Route scroll reactions through dedicated methods in `MessageListView+Lifecycle.swift`.** Do not add inline `onChange` handlers that call `scrollTo` directly. Keep scroll logic centralized.
