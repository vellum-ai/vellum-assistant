...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  ...
  6. **`.defaultScrollAnchor(.bottom, for: .initialOffset)` handles conversation-switch positioning** — scoped to initial offset (not `.sizeChanges`) because the `.sizeChanges` anchor role fights user scroll-up during streaming. On conversation switch, `handleConversationSwitched()` sets `scrollPosition = ScrollPosition()` (empty — no target) so the newly created ScrollView (via `.id(conversationId)`) uses the default anchor to position content bottom-first, materializing the last items with correct heights. An explicit `ScrollPosition(id: lastId, anchor: .bottom)` must NOT be used here — for long conversations, cumulative height-estimate error across hundreds of unmaterialized items places the viewport in blank LazyVStack space. The 100ms `restoreScrollToBottom()` fallback fine-tunes with ID-based scroll after bottom items have materialized.
