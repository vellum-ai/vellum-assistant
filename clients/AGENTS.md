...
- **View bodies must be lightweight.** Never perform I/O, network calls, or heavy computation inside a SwiftUI view body. This includes filtering, sorting, and formatting data — do that work in the ViewModel (asynchronously), not inline during render. Defer work to `task {}`, `onAppear`, or background actors.
- **Lazy containers for large collections.** Use `LazyVStack`, `LazyHStack`, `LazyVGrid` instead of eager equivalents when the item count is unbounded or large. In particular, never use `VStack`/`HStack` inside a `ScrollView` for data-driven lists — eager loading kills scroll performance.
...
- **Profile before optimizing, but follow known patterns.** Use Instruments to validate — specifically Time Profiler for main-thread spikes, Core Animation instrument for frame drops, and Allocations for memory. Launch from Xcode → Product → Profile. However, the patterns above are established project standards — follow them proactively rather than waiting for a performance regression.
