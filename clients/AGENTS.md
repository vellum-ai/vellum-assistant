...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  ...
  4. **Recovery window** — 500ms after `reset()`/`handleAppear()` where recovery fires unconditionally, bypassing the `isAtBottom` guard which is unreliable during LazyVStack materialization.
