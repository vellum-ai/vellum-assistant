...
| Long-form text editing | `TextEditor` (acceptable for editor-like surfaces where the user expects a full text-editing experience, e.g., contact notes, skill editing, tool permission tester). Read-only syntax-highlighted code views use `HighlightedTextView` (pure SwiftUI, `AttributedString` + `Text`). **Editable** syntax-highlighted views use `SyntaxTextView` (NSTextView-backed `NSViewRepresentable`) — an intentional AppKit exception because SwiftUI's `TextEditor` does not support per-token syntax highlighting | Custom AppKit `NSScrollView` + `NSClipView` + `NSTextView` stack (exception: `SyntaxTextView` for editable syntax highlighting — see "Use this" column) |
