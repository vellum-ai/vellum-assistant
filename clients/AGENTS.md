...
> Follow these rules any time you touch scroll-position logic in the chat view.

### Architecture

Scroll state is managed by four `@State` properties on `MessageListView`:

- **`scrollPosition`** — Native [`ScrollPosition`](https://developer.apple.com/documentation/swiftui/scrollposition) (initialized to `.bottom`). Used for all programmatic scrolling via imperative `.scrollTo(id:anchor:)` and `.scrollTo(edge:)` methods. Do not reintroduce `ScrollViewReader`.
- **`isAtBottom`** — Bool tracking whether the viewport is within 30pt of the content bottom. Updated by [`onScrollGeometryChange(for: Bool.self)`](https://developer.apple.com/documentation/swiftui/view/onscrollgeometrychange(for:of:action:)), which fires only on Bool transitions (~2 fires per scroll session, not every geometry tick). The state change is wrapped in `withAnimation` so the CTA exit transition animates.
- **`topVisibleMessageId`** — Captured before pagination loads to restore scroll position after prepend.
- **`isLoadingMore`** — Guards against concurrent pagination loads.

Auxiliary state (`projectionCache`, `lastConversationId`, task handles for pagination/resize/highlight/anchor) lives as `@State` on `MessageListView`. Lifecycle methods are in `MessageListView+Lifecycle.swift`.

### Bottom Detection

Uses `onScrollGeometryChange(for: Bool.self)` with a 30pt threshold (`contentSize.height - contentOffset.y - visibleRect.height < 30`). The action fires only on value transitions, not every geometry tick. The "Scroll to latest" CTA is rendered in `ScrollToLatestOverlayView` using `.transition(.move(edge: .bottom).combined(with: .opacity))`.

Do not use `ScrollPosition.viewID` for bottom detection — it becomes `nil` on user-initiated scroll. See [`ScrollPosition` docs](https://developer.apple.com/documentation/swiftui/scrollposition).

### Conversation Switching

Uses `.id(conversationId)` to destroy and recreate the ScrollView, combined with [`.defaultScrollAnchor(.bottom, for: .initialOffset)`](https://developer.apple.com/documentation/swiftui/view/defaultscrollanchor(_:for:)) to position at the bottom. An explicit `scrollPosition.scrollTo(edge: .bottom)` in `handleConversationSwitched()` acts as a safety net in case the `@State` `ScrollPosition` retains a stale target.

`handleConversationSwitched()` resets all view-local state: cancels in-flight tasks (pagination, resize, highlight, anchor timeout), resets `isAtBottom = true` (prevents CTA flash), resets `isLoadingMore`, clears `topVisibleMessageId`, and calls `projectionCache.reset()`.

Do NOT clear `scrollPosition` in `onDisappear` — the old ScrollView's `onDisappear` fires AFTER the new ScrollView's `onAppear`, so clearing would overwrite the new positioning.

### Pagination

`handlePaginationTrigger()` stores its task in `@State var paginationTask` for cancellation. The task is scoped to its starting conversation via `guard !Task.isCancelled, conversationId == startConversationId`. After the page load, a 100ms delay + `scrollPosition.scrollTo(id: anchorId, anchor: .top)` restores reading position. Add `guard !Task.isCancelled` after every `Task.sleep` per the rules below.
...
- **Add `guard !Task.isCancelled else { return }` after every `Task.sleep` in scroll or animation tasks.** Prevents stale scrolls after task cancellation.
- **Store all unstructured `Task { }` handles in `@State` properties.** Cancel them on conversation switch and `onDisappear`. Fire-and-forget tasks cannot be cancelled and will mutate state in the wrong context.
- **Cancel tasks synchronously on user-initiated scroll.** Do not rely on the task noticing cancellation at its next sleep boundary.
- **Use in-flight guards to prevent concurrent pagination loads.** `isLoadingMore` gates `handlePaginationTrigger()`.
- **Route scroll reactions through dedicated methods in `MessageListView+Lifecycle.swift`.** Do not add inline `onChange` handlers that call `scrollTo` directly.
- **Reset `projectionCache` on conversation switch.** Prevents stale `TranscriptRenderModel` from the previous conversation persisting in memory and stale `lastKnownMessagesRevision` interfering with cache invalidation.

<details>
<summary><strong>Layout timing and scroll forwarding caveats</strong></summary>

- **Allow a layout settle delay before restoring scroll position.** After inserting new messages (pagination prepend), wait at least 100ms before calling `scrollTo(anchor)` so SwiftUI can finish its layout pass.
- **Forward scroll/wheel events from gesture-capturing subviews.** `WKWebView` (and other `NSResponder` subclasses that capture scroll events) swallow all scroll-wheel input. Override `scrollWheel(_:)` to forward the event to `nextResponder`.
- **`ScrollPosition` is the project standard for programmatic scroll tracking.** Do not reintroduce `ScrollViewReader`.
...
| `.scrollPosition(id:)` with unmanaged binding | Nil binding fights SwiftUI's internal tracking, crashes on re-layout | Use `ScrollPosition()` (no idType constraint) for programmatic scrolling; use `onScrollGeometryChange(for: Bool.self)` for bottom detection |
