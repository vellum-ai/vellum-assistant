...
- **`MessageListScrollState.swift`** — Core state: a `ScrollMode` enum with 6 cases (`initialLoad`, `followingBottom`, `freeBrowsing`, `pushToTop`, `programmaticScroll`, `stabilizing`) and a centralized `transition(to:)` method. UI-facing properties (`showScrollToLatest`, `showTailSpacer`, `scrollIndicatorsHidden`) are individually tracked by the Observation framework; all other state (geometry, phases, layout cache, mode, `uiVersion`) is `@ObservationIgnored`. Key methods: `requestPinToBottom()`, `enterPushToTop()`, `consumePendingPushToTop()`, `exitPushToTop()`, `beginStabilization()`/`endStabilization()`, `handleUserScrollUp()`, `handleReachedBottom()`. Lifecycle: `reset(for:)`, `cancelAll()`.

**Observation strategy:** Each UI-facing property is tracked independently so SwiftUI only re-evaluates views that read the specific property that changed. Scroll-dependent UI is rendered in isolated child views (`TailSpacerView`, `ScrollToLatestOverlayView`) that create their own observation boundaries. The debounced `scheduleUISync()` / `syncUISnapshots()` mechanism coalesces rapid changes and updates each property independently. See [WWDC23: Discover Observation in SwiftUI](https://developer.apple.com/videos/play/wwdc2023/10149/) and the [Observation framework docs](https://developer.apple.com/documentation/observation).

Bottom detection uses `.onScrollGeometryChange` (macOS 15+) to compute the distance from the bottom of the content. The handler computes `effectiveContentHeight - contentOffsetY - visibleRectHeight` and uses asymmetric hysteresis to set `isAtBottom`: when already at bottom, `isAtBottom` stays `true` until `distanceFromBottom` exceeds 30pt (leave threshold); when not at bottom, `isAtBottom` becomes `true` only when `distanceFromBottom` is within 10pt (enter threshold). This prevents oscillation during streaming where content-height growth transiently exceeds a single threshold. This approach is reliable because it uses continuous geometry measurements rather than `ScrollPosition.viewID`, which per Apple documentation becomes `nil` on any user-initiated scroll and is therefore unsuitable for continuous bottom-detection. The reattach-on-idle logic in `onScrollPhaseChange` is the primary consumer of `isAtBottom`. The "Scroll to latest" CTA is rendered in `ScrollToLatestOverlayView`, which directly observes `showScrollToLatest`.
...
Programmatic scrolling uses a hybrid approach: item-targeted scrolls (`scrollTo`) go through `ScrollViewReader`'s `ScrollViewProxy.scrollTo(_:anchor:)` because `ScrollPosition.scrollTo(id:anchor:)` has known reliability issues with `LazyVStack` on macOS 15 (scrolls silently fail). Edge scrolls (`scrollToEdge`) continue to use `ScrollPosition.scrollTo(edge:)` which works reliably. The scroll state stores `scrollTo: (any Hashable, UnitPoint?) -> Void` (capturing the `ScrollViewProxy`) and `scrollToEdge: (Edge) -> Void` (capturing the `ScrollPosition` binding) closures configured during view setup. When entering push-to-top mode, the `$scrollPosition` binding is cleared to `ScrollPosition()` (empty) so the stale `.bottom` edge anchor doesn't fight the proxy scroll during streaming. An empty binding lets SwiftUI maintain the current offset naturally. On push-to-top exit, the binding is reset to `ScrollPosition(edge: .bottom)`. Do not use `ScrollPosition(id:anchor:)` for push-to-top — its scroll-to-item behavior silently fails with `LazyVStack` and competes with the proxy scroll. Ref: [ScrollViewProxy](https://developer.apple.com/documentation/swiftui/scrollviewproxy), [ScrollPosition](https://developer.apple.com/documentation/swiftui/scrollposition).
...
- **Use `requestPinToBottom()` instead of direct scroll calls.** `requestPinToBottom()` checks `mode.allowsAutoScroll` before scrolling, which returns `false` during stabilization, free browsing, push-to-top, and programmatic scroll. This prevents auto-scroll from racing layout mutations. For layout-sensitive operations (resize, pagination, expansion), wrap the window with `beginStabilization(_:)` / `endStabilization()`.
...
- **`ScrollPosition` + `ScrollViewReader` hybrid for programmatic scroll.** The message list uses `@State private var scrollPosition = ScrollPosition()` with `.scrollPosition($scrollPosition)` on the ScrollView. A `ScrollViewReader` wraps the scroll content and provides a `ScrollViewProxy` for item-targeted scrolls (the `scrollTo` closure on `MessageListScrollState`). `ScrollPosition.scrollTo(id:anchor:)` silently fails with `LazyVStack` on macOS 15, so `ScrollViewProxy.scrollTo(_:anchor:)` is used instead for item navigation. Edge scrolls still use `ScrollPosition.scrollTo(edge:)` via the `scrollToEdge` closure. During push-to-top, the `$scrollPosition` binding is cleared to `ScrollPosition()` (empty) so the stale `.bottom` edge anchor doesn't fight the proxy scroll during streaming. An empty binding lets SwiftUI maintain the current offset naturally. On push-to-top exit, the binding is reset to `ScrollPosition(edge: .bottom)`. Do not use `ScrollPosition(id:anchor:)` — its scroll-to-item behavior silently fails with `LazyVStack` and competes with the proxy scroll.
