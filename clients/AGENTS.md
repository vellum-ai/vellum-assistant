...
**Bottom-pinning strategy:** Uses a multi-layer approach to handle `LazyVStack` height estimation unreliability:
  ...
  7. **`phaseAllowsAutoFollow` guard** — both content-height auto-follow and persistent recovery only fire when `!scrollPhase.isScrolling || scrollPhase == .animating` (i.e., `.idle` or `.animating`). This blocks during ALL user-initiated scroll phases: `.tracking` (finger down), `.interacting` (dragging), and `.decelerating` (momentum after release). Apple's `ScrollPhase.isScrolling` is `phase != .idle` ([docs](https://developer.apple.com/documentation/swiftui/scrollphase/isscrolling)); the `.animating` exception allows our own programmatic scrolls to proceed. Without the deceleration guard, auto-follow fires during scroll momentum and fights the user's fling, causing snap-back flicker. User scroll-up detection (`handleUserScrollUp`) remains gated on `.interacting` only — intentional drag, not momentum.
