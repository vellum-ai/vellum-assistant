...
| `default-profile-disable.ts`                 | `0.11.4`                          | Managed profiles are enable-only (the daemon rejects `status: "disabled"` on a managed-source entry), so the Profiles rows offer Disable for custom profiles only                                                                                                                                                                     | Disable renders on managed rows too; it hides the default from every picker and stops the resolver selecting it at any rung. The daemon refuses a disable on a code-owned profile, and the row surfaces that message verbatim. Enable is never gated                    |
