...
| `use-supports-sight-frames.ts`               | `0.11.7-dev.0`                            | No `attach_frame` handler, so a parked camera frame is refused with the `unknown_type` the transport reads as an `update_config` rejection. The voice room samples no frames at all: the viewfinder is a shutter and nothing else, exactly as it was. 0.11.7 stable is excluded, having been cut three days before the frame landed                                                                                                          | The frame is understood; while the viewfinder is open the gate's keeps upload speculatively and the freshest one is parked at each utterance end so the turn carries it. Scoped to the assistant the session is bound to, so a version fetched for the outgoing assistant cannot authorize a frame against the incoming one                                                                                                                          |
