...
| `use-supports-sight-stream.ts`               | `0.11.7-dev.202609010135`                 | No `sight_frame` handler, so a kept camera frame is refused with the `unknown_type` the transport reads as an `update_config` rejection. The voice room samples no frames at all: the viewfinder is a shutter and nothing else. The floor is a pinned dev-build timestamp because `main` carries 0.11.7 as its base, so a build with the handler and one from before it are both named `0.11.7-dev.*` and only the stamp separates them; it is the minute after the daemon change's squash merge (3251f98402, 2026-09-01T01:34:30Z), rounded up so no pre-merge build can share it | The frame is understood; while the viewfinder is open every frame the gate keeps is uploaded and sent at once, and the daemon persists each one into the conversation as its own camera-frame message that runs no turn. Closing the viewfinder stops them. Scoped to the assistant the session is bound to, so a version fetched for the outgoing assistant cannot authorize a frame against the incoming one                                                      |
