...
| `use-supports-attachment-list.ts`            | `0.11.10`                                 | No `GET /v1/attachments?conversationId=` route, so the request 404s. The Chat Info panel lists attachments from the loaded transcript instead: it cannot tell a camera frame from a photo the user attached, and its per-category counts cover the loaded pages rather than the conversation                                                                                                                                                                                                                                                                                                                                                                                          | The route exists; the panel lists every attachment from the daemon, with exact totals and a Camera Frames category of its own. Pinned to `0.11.10`, the first release expected to carry the route: a 0.11.10 build cut before it landed answers 404, which the consumer reads as unsupported and falls back to the transcript path                                                                             |
