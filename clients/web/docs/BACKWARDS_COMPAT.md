...
- **`local` builds count as dev builds.** A `vel up` image reports
  `0.11.8-local.20260902150000.1fb7b91c12`; it is commits on top of `0.11.8`
  the same way a dev build is, and against a dev `minVersion` the two
  compare by their stamp, read to the minute.
...
### Optional mode-session presentation metadata

The optional `modeSession`, `modeSessionActivity`, and response-level
...
| -------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `batched-question-submit.ts`                 | `0.8.2`                                     | `POST /v1/question-response` knows only the single-question `{ kind: "option" \| "free_text" }` body, so a one-entry submission is downgraded to it. That shape cannot say "skipped", so a skipped question is sent as blank free text and both the persisted record and the tool result read as an empty typed answer                                                                                                                                                                                                                                                                                                                                                                | The batched `{ kind: "submit", responses }` body is posted for every submission, so a skip stays a skip. Assistant-scoped, so a version held for the outgoing assistant cannot pick the wire shape for the incoming one                                                                                                                                                                                                                                                               |
...
| `use-supports-discord-channel.ts`            | `0.11.5-dev.202608242314.38e0513`           | No Discord readiness probe; asked about Discord the daemon answers with its unsupported snapshot, so the Channels page omits the row entirely                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | The probe exists; Discord renders on the Channels page with readiness, a trust-floor control, and guided setup                                                                                                                                                                                                                                                                                                                                                                        |
| `use-supports-discord-config.ts`             | `0.11.5-dev.202608252008` (config routes)   | The Discord row renders but the manual credential form is withheld: the config routes do not exist and a save would 404. Guided setup through the assistant still works                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | The routes exist; "Connect manually" renders the three-step wizard and saves over HTTP                                                                                                                                                                                                                                                                                                                                                                                                |
| `use-supports-new-chat-plugins.ts`           | `0.14.0`                                    | New-chat plugin picker hidden; the send path omits the per-chat plugin set (older daemons ignore it)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Picker renders and the send path includes the per-chat plugin set the daemon applies                                                                                                                                                                                                                                                                                                                                                                                                  |
| `use-supports-inchat-plugin-edit.ts`         | `0.14.0`                                    | In-chat plugin pill hidden; the conversation GET omits `enabledPlugins` so per-chat scope is unreadable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Pill renders the conversation's plugin scope and edits it via `PUT /conversations/:id/enabledplugins`                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `daemon-app-pins.ts`                         | healthz `appPins` capability                | The daemon has no pin route and reports no `pinSortPosition`/`pinColor`, so pins live in the browser-wide `vellum:pinnedApps` key shared by every assistant: pins from one assistant appear under another (LUM-3452), and a pin keeps the name and icon copied at pin time                                                                                                                                                                                                                                                                                                                                                                                                            | Pins ride on the app list and are set via `POST apps/:id/pin`, so they are scoped to the assistant owning the app, render its live name and icon, and follow the user across browsers. The legacy key is drained once per assistant                                                                                                                                                                                                                                                   |
...
| `use-supports-sight-stream.ts`               | `0.11.7-dev.202609010224.44cd29e`           | No `sight_frame` handler, so a kept camera frame is refused with the `unknown_type` the transport reads as an `update_config` rejection. The voice room samples no frames at all: the viewfinder is a shutter and nothing else. The floor is one published build's whole version string because `main` carries 0.11.7 as its base, so a build with the handler and one from before it are both named `0.11.7-dev.*`; it names dev-release run 33462421058 on head 44cd29e199, the first success after the daemon merge (3251f98402), rather than a computed minute, since versions are stamped when a run computes them and a run queued for a pre-merge sha can emerge stamped later | The frame is understood; while the viewfinder is open every frame the gate keeps is uploaded and sent at once, and the daemon persists each one into the conversation as its own camera-frame message that runs no turn. Closing the viewfinder stops them. Scoped to the assistant the session is bound to, so a version fetched for the outgoing assistant cannot authorize a frame against the incoming one                                                                        |
| `sight-sessions.ts`                          | live-voice `ready.sightSessions` capability | Camera keeps retain the existing `sight_frame` shape. No lifecycle start/end frames or camera epoch are sent, so older assistants preserve flat camera-frame history.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | The connected voice session accepts ordered `sight_start` / `sight_end` frames and epoch-stamped keeps. The client allocates a fresh epoch for each sampler run and carries it through asynchronous upload. This is negotiated per connection rather than inferred from a release version.                                                                                                                                                                                            |
| `watch-capture-target.ts`                    | `0.11.8-dev.202609031252.8641564`           | `/v1/watch/stream` ignores `captureDisplayId` and `captureWindowId`, so a session started on a pick would read the whole main display while the companion framed one window. The picker is not offered, Teach starts the whole-screen session, and a target that reaches the controller is dropped before the dial so the session reports none and the frame follows the read                                                                                                                                                                                                                                                                                                         | The parameters scope every screen read of the session to that display or window; the pick rides the stream URL and comes back as the session's `captureTarget`, which is what the frame is drawn from. Scoped to the assistant the session is bound to, for the reason `watch-sessions.ts` is                                                                                                                                                                                         |
...
| `profile-input-modalities.ts`                | `0.11.9-dev.202609071659.2168636`           | Profile PATCH silently drops `inputModalities`, so the Modalities accordion is hidden and the save omits the field                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | The field persists; image/audio overrides reach vision and inline-audio send. Scoped to the write-target assistant so a version held for another assistant cannot authorize the write                                                                                                                                                                                                                                                                                                 |
| `selection-rewrite.ts`                       | `0.11.8-dev.202609021456.e7361b7`           | A hold over an editable selection is a question for the assistant; the selection is never sent to `POST /v1/dictation`, whose command mode rewrites a question too                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | An editable selection is sent with the words; an edit is pasted over it, and a `question` answer goes to the assistant                                                                                                                                                                                                                                                                                                                                                                |
| `credential-verification.ts`                 | `0.11.8-dev.202609022016.7bd8f87`           | No `POST /v1/platform/verify-credential`, so the repair for a rejected managed credential cannot confirm the replacement it stored. It skips verification and treats the stored replacement as the repair, which is what a repair meant on those daemons; asking anyway would turn the 404 into a reported failure after a successful rotation and invite another                                                                                                                                                                                                                                                                                                                     | The route exists; the repair confirms the replacement authenticates before reporting success, and a replacement the platform rejects in turn reads as the failure it is. Resolved against a hydrated version, since the decision is whether to skip a check                                                                                                                                                                                                                           |
| `use-supports-attachment-list.ts`            | `0.11.11`                                   | No `GET /v1/attachments?conversationId=` route, so the request 404s. The Chat Info panel lists attachments from the loaded transcript instead: it cannot tell a camera frame from a photo the user attached, and its per-category counts cover the loaded pages rather than the conversation                                                                                                                                                                                                                                                                                                                                                                                          | The route exists; the panel lists every attachment from the daemon, with exact totals and a Camera Frames category of its own. A build at or above the pin that still answers 404 reads as unsupported at the call site and falls back to the transcript path. A dev build stamps the `package.json` base version, so the daemon path stays off on dev builds until that base reaches the pin                                                                                         |
| `use-supports-activation-progress.ts`        | `0.11.8-dev.202609030107.d5d996d`           | No `/v1/activation/*` routes, so the progress read 404s and a launched task could never be linked to its conversation. Every activation surface (welcome modal, pill, list page) stays hidden and the progress query stays disabled. No local-only fallback on purpose: progress has to converge across desktop, web and mobile, and a client-side copy would strand a checklist on one device and re-offer finished tasks on the next                                                                                                                                                                                                                                                | Routes exist; the surfaces render, a launch links its conversation, and a completed turn flips a row through the `activation:progress` sync tag. The floor is the minute the route commit (`d5d996d`) merged, the earliest a build can carry it, so dogfood dev builds and a same-source local build light up while `assistant/package.json` still reads 0.11.8. Assistant-scoped, so a version held for the assistant the user just left cannot enable the read against an older one |
| `subagent-status-conversation-id.ts`         | envelope `conversationId`                   | `subagent_status_changed` is left unscoped by the transport, so a status for a subagent the store has not seen (its spawn missed, or the store reset by a conversation switch) is filed under the conversation on screen. A subagent still running in the conversation the user left is filed under the one they opened: its card surfaces in the wrong transcript, and the reconcile that follows asks about that conversation, finds no such subagent, and settles it as interrupted while it runs (LUM-3505)                                                                                                                                                                       | The conversation's sink scopes the envelope, so the handler reads the parent from the transport and the reconcile asks about it. Not a version read: the envelope answers per event, which keeps a same-source self-hosted build on the new path                                                                                                                                                                                                                                      |
| `document-conversation-link.ts`              | `0.8.4`                                     | No document-conversation link endpoint; a validated cached edit conversation remains browser-local and entry skips the link write                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Waits for the owning assistant's version, then persists the link before navigation; link errors stay actionable                                                                                                                                                                                                                                                                                                                                                                       |
| `guardian-notification-targeting.ts`         | `0.12.2-dev.202609181913.1108ac3`           | The assistant broadcasts a guardian-scoped `notification_intent` to every SSE connection, so the client drops any intent carrying `targetGuardianPrincipalId` and acks it without a banner                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | The assistant delivers guardian-scoped intents only to connections authenticated as the guardian, so the client shows them like any other notification. A `-local.` build always takes the old path, since its version cannot say whether its checkout carries the targeting                                                                                                                                                                                                          |
| `use-supports-document-create.ts`            | `0.12.2-dev.202609211450.3151b18`           | No `POST /v1/documents/create`; every "New Document" action is hidden, and documents are created only by the assistant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | The route exists; "New Document" creates a blank document owned by a conversation and opens it                                                                                                                                                                                                                                                                                                                                                                                        |
| `auto-profile-preview.ts`                    | `0.12.3-dev.202609230022.f2df2fb`           | No `POST /v1/auto-profile/preview`; on the Auto profile the composer pill reads plain "Auto" while typing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | The route exists; after a typing pause the pill reads "Auto · <profile>" for the current draft, and the sending turn reuses that pick                                                                                                                                                                                                                                                                                                                                                 |
