...
| `use-supports-activation-progress.ts`        | `0.12.1-dev.202609141911.5e16607`           | No `/v1/activation/*` routes, so the progress read 404s and a launched task could never be linked to its conversation. Every activation surface (welcome modal, pill, list page) stays hidden and the progress query stays disabled. No local-only fallback on purpose: progress has to converge across desktop, web and mobile, and a client-side copy would strand a checklist on one device and re-offer finished tasks on the next                                                                                                                                                                                                                                                | Routes exist; the surfaces render, a launch links its conversation, and a completed turn flips a row through the `activation:progress` sync tag. Releases 0.12.0 and 0.12.1 lack the routes and 0.12.2 has them, so the floor is the minute `main` took the 0.12.1 base (`5e16607`): `main` dev and local builds from then on light up, both releases stay dark. Assistant-scoped, so a version held for the assistant the user just left cannot enable the read against an older one |
