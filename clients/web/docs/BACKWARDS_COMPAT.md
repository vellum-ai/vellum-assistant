...
| `hosted-qwen-personality-steering.ts`        | `0.11.9-dev.202609071544.b018770`         | Hosted Qwen (`vellum` + `qwen/qwen3-8b`) can already be the active profile, but the daemon does not attach `directions` from `data/personality-sliders.json`. The personality page still runs the identity rewrite so IDENTITY.md / SOUL.md change. Unknown or mismatched assistant identity also stays on this path.                                                                                                                                                                                                                                                                                                                                                              | The daemon consumes the sidecar on the next chat turn, so the page persists the dials and skips the rewrite. Owner-scoped write gate: a version held for a different assistant cannot authorize the skip.                                                                                                                                                                                                    |
