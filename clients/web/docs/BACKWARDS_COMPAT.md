...
| `use-supports-sight-frames.ts`               | `0.11.7-dev.0`                            | No `attach_frame` handler, so a parked camera frame is refused with the `unknown_type` the transport reads as an `update_config` rejection. The voice room samples no frames at all: the viewfinder is a shutter and nothing else, exactly as it was. 0.11.7 stable is excluded, having been cut three days before the frame landed                                                                                                          | The frame is understood; while the viewfinder is open every frame the gate keeps is uploaded and parked at once, so the session's latest-wins slot always holds the current view and whichever turn launches next carries it. Closing the viewfinder unparks. Scoped to the assistant the session is bound to, so a version fetched for the outgoing assistant cannot authorize a frame against the incoming one                                     |
...
| `use-supports-contact-voiceprints.ts`        | `0.11.7-dev.0`                            | No voiceprint routes, so `GET contacts/:id/voiceprints` 404s. The contact detail view renders exactly as it did before the feature: no voice profile card, no query issued. Gated rather than left to degrade because the card's empty state invites the user to record two clips, and the enroll POST that follows would 404 after the work was done. 0.11.7 stable is excluded, having been cut before the routes landed | The card mounts, lists the contact's enrolled profiles, and can enroll, relabel, and delete them. Scoped to the assistant that owns the contact, so a version fetched for the outgoing assistant cannot light the card up against the incoming one |
