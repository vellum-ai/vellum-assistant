...
| `acp-model-switching.ts`                     | `0.11.10-dev.202609090534.a9ef179`        | No `POST acp/:id/set-model` route, so a live switch 404s. `AcpConfigSchema` strips an unknown `acp.defaultModel`, so a config PATCH answers 200, writes the key, and drops it on the next read, which is the quiet failure the gate exists for. `GET /v1/acp/sessions` reports neither `model` nor `availableModels`, so a picker has nothing to list. The Coding Agents settings card and the ACP run panel's MODEL card render nothing and a session runs on whatever the adapter picks | The route, the config key, and the listing fields all exist: the settings card saves the default the daemon applies at spawn, and the MODEL card lists the adapter's models and switches a running session. The floor names the feature branch's last daemon commit rather than a predicted release |
