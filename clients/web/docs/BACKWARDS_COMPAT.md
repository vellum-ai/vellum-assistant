...
| `use-supports-sight-frames.ts`               | `0.11.7-dev.0`                            | No `attach_frame` handler, so parking a camera frame for the next turn to carry is refused with the `unknown_type` the transport reads as an `update_config` rejection. 0.11.7 stable is excluded, having been cut three days before the frame landed                                                                                                                                                                                                            | The frame is understood, so a client may park one id at a time on the session's latest-wins slot and unpark it with `null`. The voice room does not use it: its viewfinder streams keeps through `sight_frame` instead (see the row below), so this floor governs only the transport capability. Scoped to the assistant the session is bound to                                                                                                                    |
