...
| `use-supports-sight-persist.ts`              | `(pinned at release: first published dev build containing #41863)` | No `POST /v1/assistants/{assistant_id}/conversations/{id}/sight-frame` route, so a kept camera frame answers 404 and the upload behind it is stranded as a row nothing collects. The composer's camera is a shutter and nothing else: the frame the gate is holding rides an outgoing send, and everything the camera saw between two messages is lost. Separate from the sight-stream floor because the socket frame and this route landed in different builds, so neither can speak for the other | The route exists; while the Eyes viewfinder is open every frame the gate keeps is uploaded and persisted into the open conversation as its own camera-frame message that runs no turn, and a persisted keep is dropped from the store so it does not also ride the next send. Closing the viewfinder stops them. Scoped to the assistant owning the conversation, so a version fetched for the outgoing assistant cannot authorize a persist against the incoming one |
