...
| `use-supports-sight-stream.ts`               | `0.11.8`                                  | No `sight_frame` handler, so a kept camera frame is refused with the `unknown_type` the transport reads as an `update_config` rejection. The voice room samples no frames at all: the viewfinder is a shutter and nothing else. Every 0.11.7 build is excluded, dev ones included: `main` carries 0.11.7 as its base, so a build with the handler and one without are named alike                                                                                   | The frame is understood; while the viewfinder is open every frame the gate keeps is uploaded and sent at once, and the daemon persists each one into the conversation as its own camera-frame message that runs no turn. Closing the viewfinder stops them. Scoped to the assistant the session is bound to, so a version fetched for the outgoing assistant cannot authorize a frame against the incoming one                                                      |
