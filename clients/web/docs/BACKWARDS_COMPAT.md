...
| `subagent-status-conversation-id.ts`         | `0.11.12`                                 | `subagent_status_changed` names no conversation, so a status for a subagent the store has not seen (its spawn missed, or the store reset by a conversation switch) is scoped to the conversation on screen. A subagent still running in the conversation the user left is filed under the one they opened: its card surfaces in the wrong transcript, and the reconcile that follows asks about that conversation, finds no such subagent, and settles it as interrupted while it runs (LUM-3505)                                                                                                                                                                                     | The event carries the parent `conversationId`; the stub is scoped to it and the reconcile asks about that parent. An id-less status from such an assistant is left unscoped rather than guessed                                                                                                                                                                                                                |
