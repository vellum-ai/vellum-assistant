...
| `deeplink.connect`                 | `{ url: string \| null, bundle: string \| null }`                                             | Electron host only: inbound `<scheme>://connect` URL from the SPA pair page's "Open in the Vellum app" button or a `vellum pair --app` QR code. `bundle` is a pairing bundle that prefills the Connect a Remote Assistant dialog; it is secret material, never logged or breadcrumbed (the device `code` a url+code link carries is dropped at the bridge for the same reason). `useGlobalDeepLinkConsumer` parks the request in the connect-dialog store and navigates to the assistant chooser; bundle-less links get guidance copy naming `url`'s host.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
