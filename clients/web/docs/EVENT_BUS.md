...
| `app.hidden`                       | `{ signal: "visibility" \| "app_state" \| "window_attention" }`                               | Page hidden or app backgrounded. Deduped per physical edge exactly like `app.resume`. Only repeats of the same edge collapse, so a hide landing between two foregrounds is always delivered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `app.attention`                    | `{ attended: boolean }`                                                                       | Electron host only: the window this renderer runs in gained or lost the user's attention, meaning on screen, unminimized, and holding keyboard focus. Separate from `app.resume` / `app.hidden`, which report only whether the window is on screen: a window sitting visible behind another app is still showing the transcript, so the consumers that release the camera hardware and tear down the SSE stream must not act on a focus change. Published for every attention change, including one a minimize or a hide reports as a lifecycle edge too, so a consumer reading this channel alone never believes an off-screen window is watched. `hooks/use-web-presence-report.ts` consumes it and posts `visible: false` the moment focus leaves, so a reply to the conversation that window was showing is not suppressed on the strength of a report that stays fresh for the daemon's whole TTL. Off Electron never fires: `document.hasFocus()` is window-level and false for a visible tab in an unfocused browser window, so visibility stays the browser's contract for whether a conversation is on screen. |
