...
| `clients/web/src/runtime/event-sources/lifecycle-edge.ts` | Not a source: the shared seam the DOM-visibility and Capacitor app-state sources publish through. Collapses two reports of one physical foreground / background edge into a single `app.resume` / `app.hidden`.                                                                          |
...
| `app.resume`                       | `{ signal: "visibility" \| "app_state" \| "online" }`                                        | Page visible, app foregrounded, or network came back online. At most one per physical foreground edge: iOS reports the same edge twice (`visibilitychange` and Capacitor `appStateChange`, milliseconds apart) and `runtime/event-sources/lifecycle-edge.ts` collapses the pair, keeping the label of whichever source arrived first. `signal: "online"` is outside that dedup and always fires. No desktop source publishes this or `app.hidden`: the DOM source is registered under Electron but cannot fire, since Vellum windows disable background throttling and the Page Visibility API with it, and the desktop's own window state rides `app.attention` instead, because a minimized desktop app has to keep the SSE stream that delivers its notifications.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `app.hidden`                       | `{ signal: "visibility" \| "app_state" }`                                                     | Page hidden or app backgrounded. Deduped per physical edge exactly like `app.resume`. Only repeats of the same edge collapse, so a hide landing between two foregrounds is always delivered. Backgrounded is what it means to consumers: they release the camera, drop live capture consent, and tear the SSE stream down behind a grace window.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `app.attention`                    | `{ attended: boolean }`                                                                       | Electron host only, and the only window signal the desktop renderer publishes: the window this renderer runs in gained or lost the user's attention, meaning on screen, unminimized, and holding keyboard focus. Deliberately not a lifecycle edge, because `app.hidden` means backgrounded to the consumers that release the camera hardware and tear the SSE stream down, and a minimized desktop app has to keep the stream that delivers its notifications. Published for every attention change including the first payload, so a consumer that read attention before the host reported any is corrected rather than left waiting for the next edge. `hooks/use-web-presence-report.ts` consumes it and posts `visible: false` the moment focus leaves, so a reply to the conversation that window was showing is not suppressed on the strength of a report that stays fresh for the daemon's whole TTL. Off Electron never fires: `document.hasFocus()` is window-level and false for a visible tab in an unfocused browser window, so visibility stays the browser's contract for whether a conversation is on screen. `runtime/window-attention.ts` exposes the same fact synchronously as `isWindowAttended()`, and `isVisibleToUser()` is the cross-platform predicate consumers should ask. |
...
| `deeplink.openConversations`       | `{ provenance: "intent" \| null }`                                                            | Capacitor iOS: inbound `vellum-assistant://conversations` URL (`runtime/event-sources/capacitor-deep-links.ts`), produced by the unread chip on the Quick Actions widget and the unread line on the Status widget (`OpenConversationsIntent`). No parameters. `useGlobalDeepLinkConsumer` lands on the chat unless the route already names a conversation, and parks the request; `ChatLayout` drains it into the conversation list, opening the drawer on mobile and expanding the sidebar on a wider window. The park opens nothing until the route names a conversation, so the drawer arrives on the settled route rather than under the replace-navigation an `/assistant` landing runs on arrival; it is spent by that single open, and ages out after a minute as the camera park does. `provenance` as on `deeplink.openCamera`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `deeplink.connect`                 | `{ url: string \| null, code: string \| null, legacy: boolean }`                              | Electron host only: inbound `<scheme>://connect` URL from the SPA pair page's "Open in the Vellum app" button or a `vellum pair --app` QR code. `url` is the validated https server base and `code` the device code the link carried; the two are recomposed into the pairing link that prefills the Connect a Remote Assistant dialog, so one click completes the pair. `code` is credential material, never logged or breadcrumbed. `legacy` marks a link that carried a `bundle` param from an app version whose connect dialog took a pasted pairing bundle; the payload is dropped in the main-process parser and only the flag crosses the bridge. `useGlobalDeepLinkConsumer` parks the request in the connect-dialog store and navigates to the assistant chooser; a link with no usable base gets guidance instead of a prefill, naming its legacy pairing bundle when it carried one. The dialog never submits on its own: a URL scheme carries no caller identity, and pairing is an authority grant.                                                                                                                                                                                                                                                                                                                                                                                    |
...
A _signal source_ is the bridge between a host-environment event
...
Only subscribe to `app.resume` when you need _domain-specific_
...
useBusSubscription("sse.event", (event) => {
  ...
  queryClient.setQueryData(["interactions", event.requestId], {
    state: event.state,
  });
});
```
