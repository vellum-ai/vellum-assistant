...
| `clients/web/src/runtime/event-sources/*`                 | One file per host-environment signal (DOM visibility, network online/offline, Capacitor app state, Electron `powerMonitor`, Electron main-window attention, Electron deep links). Each calls `publish` directly and returns an unsubscribe.                                                                                    |
