...
| `deeplink.share`                   | `{ inboxId: string \| null }`                                                                 | Capacitor iOS: inbound `vellum-assistant://share/<id>` URL from the Share Sheet extension (`runtime/event-sources/capacitor-deep-links.ts`), or a Darwin / resume fallback with `inboxId: null` (`runtime/share-inbox.ts`). The URL is a pointer; text and files live in the App Group inbox. `useGlobalDeepLinkConsumer` consumes the item (destructive) and parks a send that `useShareInboxSend` fulfils after uploads finish. Inbox existence is the send-authorization: only this app's share extension can write the container, so a forged id is a no-op. A new-conversation destination mints a draft; an existing thread uses the same confirmation as `deeplink.sendToThread`. Failed uploads demote to a composer pre-fill. |
...
2. **No `bus` parameter.** Sources call `publish` directly from
   ...
   The lifecycle pair is the one exception: the DOM-visibility,
   Capacitor app-state, and Electron window-attention sources call
   `publishLifecycleEdge` instead, so the edge they observe reaches the
   bus once.
