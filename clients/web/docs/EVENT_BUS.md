...
| `deeplink.billingCheckoutComplete` | `{ status, sessionId }` | Electron host only: inbound `vellum://billing/checkout-complete?status=…&session_id=…` URL. The platform bounces a native-initiated Stripe Checkout here once the user finishes in the system browser. `useGlobalDeepLinkConsumer` navigates to billing with the `session_id` (opening the Pro onboarding wizard), or to the upgrade-cancel page on `status: "cancel"`. |
| `deeplink.startVoice` | `{ mode: "new" \| "resume" }` | Capacitor iOS: inbound `vellum-assistant://voice?mode=…` URL (`runtime/event-sources/capacitor-deep-links.ts`). The single native→SPA voice command channel — Siri and the Action Button (App Intents), the Dynamic Island Live Activity's tap-to-return `widgetURL`, and manual test links all arrive here. `useGlobalDeepLinkConsumer` navigates and hands the request to the live-voice starter, parking it while the layout-scoped session controller is unmounted (cold launch). `mode: "resume"` only navigates back to a running session. |
