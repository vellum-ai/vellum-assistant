...
| `deeplink.openConversations`       | `{ provenance: "intent" \| null }`                                                            | Capacitor iOS: inbound `vellum-assistant://conversations` URL (`runtime/event-sources/capacitor-deep-links.ts`), produced by the unread chip on the Quick Actions widget and the unread line on the Status widget (`OpenConversationsIntent`). No parameters. `useGlobalDeepLinkConsumer` lands on the chat unless the route already names a conversation, and parks the request; `ChatLayout` drains it into the conversation list, opening the drawer on mobile and expanding the sidebar on a wider window. The park is held across the replace-navigation an `/assistant` landing runs on arrival and spent only once the route names a conversation, so the drawer is not closed by the navigation it was waiting on; it ages out after a minute, as the camera park does. `provenance` as on `deeplink.openCamera`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
