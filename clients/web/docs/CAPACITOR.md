...
| `AppIcon` | [`src/runtime/app-icon.ts`](../src/runtime/app-icon.ts) | Reads the alternate home-screen icons the build ships (`CFBundleAlternateIcons`) and swaps between them. Backs the App icon picker in Settings -> General, where a user cycles eyes and color or matches their assistant's avatar in one tap. iOS shows a system alert on every icon change, so `set` runs only from a press and never on its own |
