...
- **Protected (app) routes**: route protection middleware (see [`CONVENTIONS.md` § Route protection via middleware](./CONVENTIONS.md#route-protection-via-middleware)) redirects unauthenticated users to `/account/login?returnTo=…`. Individual pages should **not** render inline sign-in gates. Return `null` when `!isLoggedIn` and let the middleware handle the redirect. The branded login page (`/account/login`) renders a native login form on Capacitor iOS and a web login form on web. Both wear the same shell, from [`src/components/auth-welcome-screen.tsx`](../src/components/auth-welcome-screen.tsx): `WelcomeScreenShell` supplies the layout and the avatar wave, `WelcomeScreenCopy` the heading and the action column, and each caller passes only its own buttons.
...
- **`<scheme>://camera`, `<scheme>://new-chat` and `<scheme>://conversations`** are the Home Screen widgets' command contracts (`parseOpenCameraDeepLink` → `deeplink.openCamera`, `parseNewChatDeepLink` → `deeplink.newChat`, `parseOpenConversationsDeepLink` → `deeplink.openConversations`). Neither takes a parameter: the host is the whole request, so a URL carrying a path is rejected, while extra query items are ignored rather than rejected, so a producer that grows one later degrades to the plain command on an older bundle. The camera command parks in `usePendingDeepLinkStore` and is drained by the composer's attachment layer (`useCameraDeepLink`), which owns the 60s age bound and spends the request unopened while a live-voice call holds the camera layer. The conversations command parks the same way and is drained by `ChatLayout`, which owns its 60s age bound and holds the park, opening nothing, until the route names a conversation: the layout closes the drawer on every navigation and an `/assistant` landing replace-navigates off itself a beat after arrival, so a drawer opened on the index would be shut by the navigation it was waiting on, and one that reopened after each navigation could not be got out of.
...
The iOS shell registers **eleven app-local** Capacitor plugins in [`MyViewController.capacitorDidLoad()`](../../../clients/ios/App/App/MyViewController.swift) (count them there, not from prose). The table also includes Android-only bridges. `CameraPreview` is an external SPM/Gradle dependency that Capacitor discovers automatically, so it is not registered in that method.
...
| `RecentChats` | [`src/runtime/recent-chats.ts`](../src/runtime/recent-chats.ts) | Mirrors the sidebar conversation list (ids + titles) into an App Group UserDefaults cache that backs the Shortcuts app's chat picker (`ChatEntityQuery`) and the share extension's destination list; synced from `ChatLayout` once the list query has resolved |
| `ShareInbox` | [`src/runtime/share-inbox.ts`](../src/runtime/share-inbox.ts) | Drains the App Group inbox the iOS Share Sheet extension writes (`consume({ id? })`) and emits `inboxReady` when a write lands while the host is already in memory. `useGlobalDeepLinkConsumer` consumes the item and `useShareInboxSend` uploads files then sends. A missing plugin is a no-op |
...
| `SenderNotification` | `src/runtime/sender-notification.ts` | iOS process-local owner for local sender notifications. Contract version 1 exposes `getCapabilities`, bounded scoped `prepare`/`reset`, full-key `post`, and read-only `status`. After `post` accepts a delivery key, native code alone races the Communication Notification rewrite against its deadline and submits the original or rewritten content once. Results are `posted`, `duplicate`, `blocked`, `failed`, `unknown`, or `unavailable`; an uncertain or late bridge result is never permission to schedule a second banner. Prepared identities and completed results live only in bounded RAM, while validated PNG bytes may warm the existing hash-addressed App Group cache. The plugin preserves Capacitor's numeric request id, `cap_extra`, category, delegate/action callbacks, and nil sound. App presentation ignores prepared and inline sender data. Assistant presentation requires the exact scope, assistant, and native sender identity. |
| `AndroidSenderNotification` | `src/runtime/android-sender-notification.ts` | Android process-local owner for local notification requests. Contract version 1 exposes capability-gated scoped `prepare`/`reset`, full-key `post`, and `status`. After the separate versioned and page-generation-bound `AndroidPushRegistration.setNotificationOwnership` handshake, both local requests and data-only FCM use the same coordinator. The coordinator key is the first semantically present full trimmed correlation id, delivery id, or request key, bounded to 512 UTF-16 code units and separate from the numeric Android notification id. Blank candidates are absent, while an overlength candidate fails closed without falling through to a lower-precedence key. A bridge timeout, malformed response, blocked result, or uncertain post never schedules a second banner or sound. Both coordinator posts and the Android Local Notifications fallback own channel sound, so the web layer never adds a second sound to either route. A page load, renderer loss, or bridge destruction clears live ownership without clearing retained coordinator results; only a process restart clears those bounded RAM results. Old web bundles keep the foreground-handler route, and old shells keep the Local Notifications route. Notification-block FCM remains on Capacitor's existing route. |

The independent `push-avatar-sender` and `local-notification-avatar` matrix,
required compatibility and ownership cases, current manual statuses, and
rollout gates are maintained in the canonical
