...
The shell registers **ten app-local** Capacitor plugins in [`MyViewController.capacitorDidLoad()`](../../../clients/ios/App/App/MyViewController.swift) (count them there, not from prose). `CameraPreview` is an external SPM/Gradle dependency that Capacitor discovers automatically, so it is not registered in that method.
...
**Pair every long-lived stream (SSE, chunked fetch, WebSocket-equivalents) with a timer that resets on every received byte (including SSE comment frames, which most SDKs expose through `onSseEvent` even when they don't yield through the iterator) and force-reconnects after a bounded window of silence.** The shared timer is [`@vellumai/sse-idle-watchdog`](../../../packages/sse-idle-watchdog/src/index.ts). The web wrapper in [`src/lib/streaming/stream-watchdog.ts`](../src/lib/streaming/stream-watchdog.ts) adds Sentry/lifecycle telemetry; [`src/lib/streaming/stream-transport.ts`](../src/lib/streaming/stream-transport.ts) arms it per frame (comment frames included). The Chrome extension uses the same package from `clients/chrome-extension/background/sse-connection.ts`.
