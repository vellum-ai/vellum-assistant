...
**No interruption reason identifies backgrounding on iOS.** Worth knowing before reaching for one: `AVAudioSession.InterruptionReason` looks like it has the answer, but `sceneWasBackgrounded` is declared inside `#if TARGET_OS_VISION` (visionOS only) and `appWasSuspended` has not been delivered since iOS 16. What remains on iOS is `default`, `builtInMicMuted` and `routeDisconnected`. So a background transition either raises no interruption at all, which is the intended behavior given `UIBackgroundModes: audio` and an active session, or arrives indistinguishable from `default`. `subscribeVoiceAudioInterruptions` carries the reason so callers can spare a session that was merely re-routed, but it cannot answer the background question. Only a handset can.
