...
- **[`docs/PLATFORM_ADAPTATION.md`](./docs/PLATFORM_ADAPTATION.md)**: Which signal to branch on when a surface differs across desktop, iOS, and Android: window size (`useIsMobile()`) vs input capability (`useTouchMobile()`) vs platform (`data-native-platform`), and where the branch belongs. Read this before writing any `isMobile ? … : …`.
