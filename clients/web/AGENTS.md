...
- **Server data has one owner: its query cache — except the chat transcript, which is a materialized snapshot.** For ordinary server state, read directly from the TanStack Query cache; never copy it into a Zustand store or `useState` via an effect and then reconcile the two. A function that merges server snapshots into client state (named `reconcile*`, `sanitize*`, `merge*Snapshot`, or similar) is the tell that you've double-sourced: the two copies drift, and you end up re-sorting or de-duping on every render to hide it. **The one sanctioned exception is the chat transcript.** It is owned by the chat-session store's `snapshot` — a rolling materialized view *seeded* from the `/messages` query cache and *advanced* by folding stream events through the pure, idempotent reducer (`transcript/rolling-base.ts`). The query cache stays authoritative: every committed history fetch *reseeds* the snapshot (replaying the buffered event tail), so there is no ad-hoc merge and no drift — the fold is the single write path. The transcript renders from `snapshot ⊕ optimisticSends` via `useTranscriptMessages`; nothing else may hold a parallel copy of transcript rows. See [`docs/STATE_MANAGEMENT.md` — Server state vs client state](./docs/STATE_MANAGEMENT.md) and `transcript/rolling-base.ts`.
