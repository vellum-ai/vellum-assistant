...
- **App viewer is a route**: `/assistant/conversations/:conversationId/app/:appId` names the open app, so browser Back closes it. Open an app by navigating to `routes.conversation(conversationId, appId)`; close it through `closeAppRoute` (`utils/conversation-navigation.ts`), which drops the segment and closes the viewer. The one store-only open left is the Chat Info panel's app of another assistant, which no conversation URL can name. `useAppRouteSync`, mounted in `active-chat-view.tsx`, is the one place that reads that segment into `useViewerStore`. Split view, the minimized state, and the bound edit conversation stay in memory, so a reload of an app URL lands full width.
