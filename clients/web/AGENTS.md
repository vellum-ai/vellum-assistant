...
- **App viewer is a route**: `/assistant/conversations/:conversationId/app/:appId` names the open app, so browser Back closes it. Open one by navigating to `routes.conversation(conversationId, appId)` and close it through `closeAppRoute` (`utils/conversation-navigation.ts`, which owns every other move on that segment); `useAppRouteSync`, mounted in `active-chat-view.tsx`, is the one place that reads the segment into `useViewerStore`.
