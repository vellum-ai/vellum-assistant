 ...
 * contract — the runtime collapse of `medium`/`high` cells to Conservative
 * and the Conservative default for cell-less rooms — which ships in the
 * pinned version below. The web bundle always serves latest while assistants
 * update on the user's schedule; the picker renders exactly two levels and
 * shows a stored `medium`/`high` cell as the level it behaves as, which is
 * only truthful against an assistant that applies the same collapse at the
 * resolve point. Rendering it against an older assistant would display
 * Conservative for a cell that backend still treats as its raw threshold.
 * When unsupported, the channel list renders without access controls
 * instead (read-path degrade); channels remain visible and the rest of the
 * tab works.
 *
