 ...
 *
 * A client uploads a sampled frame over the ordinary attachment route and then
 * tells the daemon about it with an `attach_frame` live-voice frame, which
 * parks the id so the next turn carries it. The upload half works against any
 * assistant; the frame does not.
 *
 * The floor for the transport capability, not for a surface. The voice room's
 * viewfinder streams its keeps through `sight_frame`, which persists each one
 * on arrival and has a floor of its own (`use-supports-sight-stream.ts`); this
 * gate governs `LiveVoiceChannelClient.attachFrame` for anything that parks.
 *
 * A WRITE gate, so it withholds the behavior rather than letting it fail (see
 * docs/BACKWARDS_COMPAT.md): below `MIN_VERSION` nothing is sent. An assistant
 * that predates the frame answers `unknown_type`, the same code an old
 * assistant returns for `update_config`, and speculatively
 ...
 *
 * Delete this gate, and the `MIN_VERSION` branch at every caller of
 * `attachFrame`, once the minimum supported assistant is >= MIN_VERSION.
 */
 ...
 * (`sessionAssistantId`) understands the `attach_frame` frame, so a caller may
 * park frames for the turns that follow.
 *
 * On the `false` branch (below `MIN_VERSION`, or any of
 * `useAssistantScopedSupports`'s conservative unknown/mismatch cases) nothing
 * is parked and the behavior is simply absent.
 */
