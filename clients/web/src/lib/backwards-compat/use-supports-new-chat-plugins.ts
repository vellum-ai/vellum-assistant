 ...
 *
 * MIN_VERSION targets 0.14.0. The manage-plugins surface (this new-chat
 * picker plus the in-chat plugin pill) ships in that release, so the gate
 * holds until the active assistant is on 0.14.0 or newer. That keeps the
 * picker and send-path inclusion hidden on every older assistant until
 * the surface lands, including on 0.12.x and 0.13.x, which cut without it.
 */
import {
  assistantSupports,
