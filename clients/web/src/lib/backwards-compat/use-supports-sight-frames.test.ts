 ...
 *
 * The constant is `0.11.7-dev.0`, a pre-release, which is not the obvious shape
 * for a minimum-version gate and is easy to "tidy" into a plain `0.11.7`. That
 * edit would light the sampler up against every assistant on the released
 * 0.11.7, where the daemon has no `attach_frame` handler and refuses every
 * frame with the code the transport reads as an `update_config` rejection.
 *
...
import { MIN_VERSION } from "./use-supports-sight-frames";

describe("sight frame version gate", () => {
  test("admits a dev build of the release it was cut against", () => {
    ...
    // commits on top of 0.11.7, including the frame, so it must pass before
    // 0.11.8 exists.
    expect(
      versionSupports("0.11.7-dev.202608301412.b432fb7", MIN_VERSION),
    ).toBe(true);
  });

  test("excludes the 0.11.7 stable release", () => {
    // Cut 2026-08-27, three days before the frame landed. This is the row that
    // makes the pre-release suffix necessary rather than decorative.
  ...
  test("excludes everything below 0.11.7", () => {
    ...
    expect(
      versionSupports("0.11.6-dev.202608301412.b432fb7", MIN_VERSION),
    ).toBe(false);
  });
