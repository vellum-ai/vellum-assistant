...
// `utils.test.ts`. Here we verify the boundary on each side of 0.14.0
...
describe("useSupportsNewChatPlugins", () => {
  ...
  test("reads false for assistants below 0.14.0 (incl. 0.12.0, 0.13.0)", () => {
    expect(readGateViaHook("0.13.0")).toBe(false);
    expect(readGateViaHook("0.12.0")).toBe(false);
    expect(readGateViaHook("0.11.1")).toBe(false);
    expect(readGateViaHook("0.11.0")).toBe(false);
  });

  test("reads true for assistants on 0.14.0+", () => {
    expect(readGateViaHook("0.14.0")).toBe(true);
    expect(readGateViaHook("0.14.1")).toBe(true);
