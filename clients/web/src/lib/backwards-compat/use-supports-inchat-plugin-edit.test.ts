...
// boundary on each side of MIN_VERSION (0.14.0, the release that ships the
...
describe("useSupportsInchatPluginEdit", () => {
  ...
  test("reads false below 0.14.0 (incl. 0.12.0, 0.13.0)", () => {
    expect(readGate("0.13.0")).toBe(false);
    expect(readGate("0.12.0")).toBe(false);
    expect(readGate("0.11.1")).toBe(false);
    expect(readGate("0.11.0")).toBe(false);
  });

  test("reads true for assistants on 0.14.0+", () => {
    expect(readGate("0.14.0")).toBe(true);
    expect(readGate("0.14.1")).toBe(true);
