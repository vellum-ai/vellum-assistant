 ...
 *
 * MIN_VERSION targets 0.14.0. The manage-plugins surface (this in-chat pill
 * plus the new-chat plugin picker) ships in that release, so the gate holds
 * until the active assistant is on 0.14.0 or newer, keeping the pill hidden on
 * every older assistant until the surface lands, including on 0.12.x and
 * 0.13.x, which cut without it.
 */
import { useAssistantSupports } from "./utils";

export const MIN_VERSION = "0.14.0";
