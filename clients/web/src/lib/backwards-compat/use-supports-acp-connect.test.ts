...
describe("useSupportsAcpConnect", () => {
  ...
  test("returns true at exactly MIN_VERSION (0.10.11)", () => {
    expect(check("0.10.11")).toBe(true);
  });

  test("returns true for dev builds ahead of MIN_VERSION", () => {
    expect(check("0.10.11-dev.202607151252.abc1234")).toBe(true);
  });
  ...
  test("returns false for versions below MIN_VERSION", () => {
    // 0.10.10 was cut before the Connect routes exist, so it must NOT pass —
    // the gate boundary is exactly what this bump raised.
    expect(check("0.10.10")).toBe(false);
  ...
  test("returns false when the supported version belongs to a DIFFERENT assistant (version-skew scope)", () => {
    ...
    expect(check("0.10.11", "other-asst")).toBe(false);
  });
