...
// boundary on each side of MIN_VERSION (0.11.10, the first release expected to
...
describe("useSupportsAttachmentList", () => {
  ...
  test("reads false below 0.11.10", () => {
    expect(readGate("0.11.9")).toBe(false);
    expect(readGate("0.11.2")).toBe(false);
    expect(readGate("0.10.10")).toBe(false);
  });

  test("reads false for a dev build of a base below 0.11.10", () => {
    expect(readGate("0.11.9-dev.202609010000.abcdef0")).toBe(false);
  });

  test("reads true at 0.11.10 and for its dev builds", () => {
    expect(readGate("0.11.10")).toBe(true);
    expect(readGate("0.11.10-dev.202609090000.abcdef0")).toBe(true);
  });
