 ...
 *   full-screen overlay). On a wide viewport `"split"` uses the open
 *   conversation, and starts a fresh one when none is open.
 *
...
/**
 * Select `conversationId` and land on it, keeping an open app beside it in
 * the side-by-side layout rather than dismissing the app for the chat.
 */
...
function setView(
  ...
  switch (data?.view) {
    ...
    case "split": {
      ...
      // put there; with nothing selected, starting one is what the request
      // asks for. Ignoring it would leave the app full-width with no way to
      // talk to the assistant about it.
