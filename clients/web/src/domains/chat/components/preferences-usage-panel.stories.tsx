/**
 * The preferences menu's usage reading.
 *
 * Unlike the billing tile's `UsageBalancePanel`, this one composes itself: the
 * subscription arrives through TanStack Query, and the wallet status and the
 * usage-grant figures arrive through `useBillingBalanceStatus`. Every one of
  ...
  return (
    <>
      ...
      {showBillingRows && balance !== null && showCredits ? (
        <div className="my-2">
          <CreditsCard
            balance={displayedCreditsUsd(balance, availableUsageBalance)}
            onAddCredits={() => {}}
          />
        </div>
      ) : null}
...
/**
 * Seeds the three gate stores and the two billing reads, then hands every one
 * of them back on unmount so a story cannot leak its billing state into the
 * next one. Written in an effect rather than during render because the stores
 * are subscribed by the tree being rendered.
 */
...
const meta: Meta<UsagePanelStoryArgs> = {
  ...
  // gate for all of them. Isolating the seed per instance is not possible
  // while the source of truth is a module singleton, so the variants are
  // canvas-only, where exactly one is mounted at a time.
  tags: ["!autodocs"],
 ...
 * row only appears when there is no usage reading to hide the dollar balance
 * behind.
 */
