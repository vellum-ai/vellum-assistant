...
export function SurfaceContainer({
  ...
  const completionTone =
    surface.completionTone ?? inferCompletionTone(surface.completionSummary);
  ...
  return (
    <div
      className={cn(
        "rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-lift)] p-4",
        className,
      )}
    >
      ...
      {surface.completed
        ? surface.completionSummary && (
            <div className="mt-4 flex justify-end">
              <span
                className={cn(
                  "flex items-center gap-1.5 text-body-medium-default",
