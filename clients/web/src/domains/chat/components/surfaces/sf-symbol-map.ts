/**
 * Maps SF Symbol names (used by the macOS/iOS daemon) to Lucide icon components
 * so the web client can render them without pulling the entire icon set into
 * the main bundle.
 *
 * Mirrors `SFSymbolMapping.swift` in the native client.
 *
 * Icons are imported explicitly (not via `lucide-react`'s `icons` namespace)
 * because the namespace import re-exports every icon (~1700) and prevents
 * tree-shaking. Adding a new SF Symbol means adding both the map entry and
 * the corresponding named import below.
 */
...
const SF_SYMBOL_TO_LUCIDE_ICON: Record<string, LucideIcon> = {
  // Navigation / Arrows
...
/**
 * Look up the Lucide icon component for an SF Symbol string.
 * Returns `undefined` when the symbol has no mapping; callers should fall
 * back to rendering the raw string or a placeholder.
 */
export function sfSymbolToLucideIcon(sfSymbol: string): LucideIcon | undefined {
  return SF_SYMBOL_TO_LUCIDE_ICON[sfSymbol];
}
