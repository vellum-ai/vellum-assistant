...
describe("empty custom groups", () => {
  const conversationGroups = [
    { id: "grp-full", name: "Full", sortPosition: 0, isSystemGroup: false },
    { id: "grp-empty", name: "Empty", sortPosition: 1, isSystemGroup: false },
  ];

  /* A group with nothing in it has nothing to show, in the list or as a rail
     tile, so it waits for its first conversation exactly as Pinned does. */
  test("a group with no conversations gets no section", () => {
    const { result } = renderHook(() =>
      useSidebarState({
        assistantId: "asst-1",
        conversations: [
          makeConversation(0, { conversationId: "c1", groupId: "grp-full" }),
        ],
        conversationGroups,
      }),
    );
  ...
  /* Keeps the assertion above honest: the group is filtered for being empty,
     not for existing. */
  test("the same group gets a section once it holds a conversation", () => {
    const { result } = renderHook(() =>
      useSidebarState({
        assistantId: "asst-1",
        conversations: [
          makeConversation(0, { conversationId: "c1", groupId: "grp-full" }),
          makeConversation(1, { conversationId: "c2", groupId: "grp-empty" }),
        ],
        conversationGroups,
      }),
    );

    expect(result.current.sections.map((section) => section.key)).toContain(
      "grp-empty",
    );
  });
});
