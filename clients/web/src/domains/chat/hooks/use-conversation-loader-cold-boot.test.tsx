...
/* The drained foreground list never resolves in these tests. */
...
/* Not a native shell: no new-chat draft is minted on cold launch. */
  ...
  daemonClient.get = mock(
    async (options: {
      ...
      if (url.endsWith("/conversations")) {
        ...
        const body = {
          conversations: listRows.slice(0, limit).map(raw),
...
describe("useConversationLoader cold-boot landing", () => {
  ...
  test("with nothing stored, asks for page one with limit 1", async () => {
    ...
    const listCall = (
      daemonClient.get as unknown as {
        mock: { calls: Array<[{ query?: Record<string, unknown> }]> };
      }
    ).mock.calls.find((call) => call[0].query?.limit !== undefined);
    expect(listCall?.[0].query?.limit).toBe(1);
  });

  test("reads the newest row from a warm drained cache instead of the server", async () => {
