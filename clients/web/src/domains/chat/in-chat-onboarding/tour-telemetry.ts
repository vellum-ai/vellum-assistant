 ...
 * - `tour_started`   — the tour began; `screen` stays `auto` (the
 *                      post-onboarding hand-off is the only trigger since
 *                      the header replay button was removed).
...
export function emitInChatTourStarted(): void {
  emitOnboardingFunnelStepCompleted(IN_CHAT_TOUR_FUNNEL_STEPS.started, {
    funnelVersion: IN_CHAT_TOUR_FUNNEL_VERSION,
    variant: readInChatTourVariant(),
    // The hand-off is the only trigger; `auto` keeps the rows comparable
    // with those recorded while the replay button existed.
    screen: "auto",
    outcome: "completed",
  });
}
