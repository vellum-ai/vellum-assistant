 ...
 * as the final key is pressed. Only fires while the Vellum window has focus.
 *
 * Fn is not handled here: the host's Fn channel belongs to the voice mode
 * toggle (`use-voice-mode-hotkey` / `use-native-fn-registration`), so a
 * stored Fn activator falls back to Ctrl on read.
 *
  ...
  useEffect(() => {
    ...
    const readActivator = () => {
      ...
      activatorRef.current = raw
        ? parseActivator(raw, { preserveFunction: false })
        : { kind: "off" };
    };
    readActivator();
    ...
    const handleBlur = () => {
      ...
      // DOM keyup can be lost when the page blurs, so end the session.
      if (activeRef.current) {
        stopActiveTarget();
      }
    };
