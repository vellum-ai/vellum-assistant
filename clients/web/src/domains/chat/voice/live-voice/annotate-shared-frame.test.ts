...
/** The colour the frame's window drew the marks in, sent alongside them. */
const INK = "#a78bfa";
...
describe("drawing the marks onto a shared frame", () => {
  test("places a fraction of the surface at that fraction of the frame", () => {
    ...
    draw(context, [stroke([0.25, 0.5], [0.75, 0.5])], 800, 400, INK);
  ...
  test("draws the ink thinner than the halo under it", () => {
    ...
    draw(watched, [stroke([0, 0], [1, 1])], 100, 100, INK);
  ...
  test("weighs the line against the smaller side", () => {
    const wide = recorder();
    draw(wide.context, [stroke([0, 0], [1, 1])], 1600, 400, INK);
    const tall = recorder();
    draw(tall.context, [stroke([0, 0], [1, 1])], 400, 1600, INK);
    expect(wide.context.lineWidth).toBe(tall.context.lineWidth);
  });

  test("a press that never moved is a dot rather than nothing", () => {
    const { calls, context } = recorder();
    draw(context, [stroke([0.5, 0.5])], 200, 200, INK);
    expect(
  ...
  /**
   * The colour comes with the marks rather than being resolved again here.
   * The window that drew them resolved the assistant's accent from state this
   * one does not have, and a second resolution that differed would put a
   * drawing in the transcript in a colour the user never saw.
   */
  test("inks the marks in the colour they were drawn in", () => {
    const inks: unknown[] = [];
    const { context } = recorder();
    const watched = {
      ...context,
      stroke: () => inks.push(watched.strokeStyle),
    };
    draw(watched, [stroke([0, 0], [1, 1])], 100, 100, INK);
    // The halo goes down first and the assistant's colour over it.
    expect(inks.at(-1)).toBe(INK);
  });

  test("draws nothing for a mark with no points at all", () => {
    const { calls, context } = recorder();
    draw(context, [{ points: [] }], 200, 200, INK);
  ...
  test("hands back the plain frame when there is nothing to draw", async () => {
    ...
    expect(await annotateSharedFrame(frame, [], INK)).toBe(frame);
  });
  ...
  test("hands back the plain frame when the drawing cannot be done", async () => {
    ...
    expect(
      await annotateSharedFrame(frame, [stroke([0, 0], [1, 1])], INK),
    ).toBe(frame);
  });
});
