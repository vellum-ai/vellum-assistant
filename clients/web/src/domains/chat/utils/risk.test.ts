...
describe("getProvenanceText", () => {
  test("maps known reasons to display text", () => {
    expect(getProvenanceText("trust_rule_allowed")).toBe(
      "· Auto-approved · Trust rule matched",
    );
    expect(getProvenanceText("platform_auto_approve")).toBe(
      "· Auto-approved · Platform session",
    );
  });
