...
import { buildIntegrationItems, filterIntegrationItems, type IntegrationFilter } from "../integration-items";
...
function IntegrationsPanelInner({ assistantId }: { assistantId: string }) {
  ...
  const [selectedFilter, setSelectedFilter] = useState<IntegrationFilter>("all");
  const [selectedProviderKey, setSelectedProviderKey] = useState<string | null>(searchParams.get("provider"));
  const mcp = useMcpConnections(assistantId);

  const { platformAssistantId, isLoading: platformIdLoading, error: platformIdError } =
  ...
  const connections = useQuery({
    ...assistantsOauthConnectionsListOptions({ path: { assistant_id: platformAssistantId ?? "" } }),
    enabled: Boolean(platformAssistantId) && platformGate === "full" && orgReady,
  });
  ...
  useEffect(() => {
    ...
    if (oauthStatus === "connected") {
      ...
      toast.error(oauthErrorMessage(t, searchParams.get("oauth_code") ?? "unknown") ??
        (providerLabel ? t("integrationsPage.connectFailedToast", { providerLabel })
          : t("integrationsPage.connectFailedToastGeneric")));
    }
    navigate(routes.settings.integrations, { replace: true });
  }, [searchParams, navigate, t]);

  const items = useMemo(() => filterIntegrationItems(
    buildIntegrationItems(providers.data ?? [], connections.data ?? [], mcp.list.data?.servers ?? []),
    searchText, selectedFilter,
  ), [providers.data, connections.data, mcp.list.data, searchText, selectedFilter]);
  const selectedProvider = providers.data?.find((provider) => provider.provider_key === selectedProviderKey);
  const loading = providers.isLoading || connections.isLoading || platformIdLoading || mcp.list.isLoading;
  const oauthError = providers.isError || connections.isError || Boolean(platformIdError);
  const authBusy = Boolean(mcp.auth.attempt && mcp.auth.attempt.phase !== "error");
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
        ...
        <Select<IntegrationFilter>
          options={[
            ...
            { value: "connected", label: t("integrationsPage.filterConnected") },
            { value: "available", label: t("integrationsPage.filterAvailable") },
          ]}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <Button variant="outlined" className="min-h-11" leftIcon={<Plus />} onClick={addCustom} disabled={!orgReady || !flagsHydrated || authBusy}>
          {t("integrationsPage.addCustom")}
        </Button>
        <Button
          variant="ghost"
          className="min-h-11 min-w-11"
          iconOnly={<RefreshCw className={mcp.reload.isPending ? "animate-spin" : undefined} />}
      ...
      {oauthError ? <Notice tone="warning">{t("integrationsPage.oauthUnavailable")}</Notice> : null}
      {mcp.list.isError ? (
        <Notice tone="warning" actions={<Button variant="ghost" onClick={() => void mcp.list.refetch()}>{t("mcpConnect.retry")}</Button>}>
          {t("integrationsPage.mcpUnavailable")}
        </Notice>
      ) : null}
      <McpConnectionDialogs connections={mcp} allowAdd={allowAdd} />
      {loading ? (
        <div role="status" className="flex items-center gap-2 text-body-medium-lighter text-[var(--content-tertiary)]">
          <Loader2 className="size-4 animate-spin" />
          {t("integrationsPage.loading")}
        </div>
      ) : null}
      <div className="space-y-2">
        {items.map((item) => item.kind === "oauth" ? (
          <IntegrationRow
            key={item.id}
            ...
            disabled={!orgReady || (platformGate === "full" && !platformAssistantId)}
            onConfigure={() => setSelectedProviderKey(item.provider.provider_key)}
          />
        ) : (
          <McpServerCard
            key={item.id}
            ...
            onAuthenticate={mcp.auth.connect}
            onManagePlugin={(pluginName) => navigate(pluginName ? `${routes.plugins}/${encodeURIComponent(pluginName)}` : routes.plugins)}
            isAuthenticating={mcp.auth.attempt?.serverId === item.server.id && authBusy}
      ...
      {!loading && items.length === 0 ? (
        <p className="py-8 text-center text-body-medium-default text-[var(--content-tertiary)]">
          {searchText.trim()
            ? t("integrationsPage.emptySearchSubtitle", { query: searchText.trim() })
      ...
      {selectedProvider && (platformGate !== "full" || platformAssistantId) ? (
        <IntegrationDetailModal
          ...
          displayName={selectedProvider.display_name ?? selectedProvider.provider_key}
...
export function IntegrationsPage() {
  ...
  return <Suspense><IntegrationsPanelInner key={assistantId} assistantId={assistantId} /></Suspense>;
}
