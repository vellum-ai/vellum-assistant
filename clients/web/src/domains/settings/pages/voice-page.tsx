...
/**
 * Hold-to-dictate presets. Bare modifiers on purpose: a held modifier types
 * nothing, so dictation can start while the composer is focused. Fn is not
 * offered here; the host's Fn channel belongs to the voice mode toggle.
 */
...
/**
 * Hold-to-dictate: hold the saved modifier chord to record, release to stop.
 * Separate from the voice mode shortcut above on purpose: that binding is a
 * toggle and rejects bare modifiers, while holding Ctrl or Alt to dictate is
 * the whole point here, so the two never claim the same input.
 */
function PushToTalkCard() {
  ...
  const showFocusedTabNote = pttEnabled && !isElectron();
  ...
  const handleCaptureKeyUp = useCallback(
    (event: ReactKeyboardEvent<HTMLDivElement>) => {
      ...
      // Every modifier released with no key pressed in between: the chord is
      // the modifiers themselves, i.e. hold-to-talk on a bare modifier.
      const remaining = collectModifiers(event);
      if (remaining.length === 0 && pendingModifiers.length > 0) {
        selectActivator({
          kind: "modifierOnly",
          ...
          onChange={(next: boolean) => {
            if (next) {
              if (activator.kind === "off") {
                selectActivator(CTRL_PTT_ACTIVATOR);
              }
            } else {
