 ...
 * served from the query cache and the panel's heavier siblings are stubbed, so
 * the tile's own formatting is the only moving part.
 */
...
describe("BillingPanel balance tile", () => {
  ...
  test("keeps the negative rendering", () => {
    // No grant figure to net out, so the overdrawn wallet reaches the tile as
    // it is: the sign is the tile's own rendering, not a credit rule.
    const { getByTestId } = renderPanel(
