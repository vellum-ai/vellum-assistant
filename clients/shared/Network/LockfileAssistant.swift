...
public struct LockfileAssistant {
    ...
    // MARK: - Apple Container Entry

    /// Creates or refreshes a lockfile entry for an Apple Container assistant.
    ///
    /// Follows the same read-modify-write pattern as `ensureManagedEntry` but
    /// writes `cloud: "apple-container"` instead of `"vellum"`. Existing
    /// entries keep their original `hatchedAt` value.
    ///
    /// - Parameters:
    ///   - assistantId: The assistant name assigned during hatch.
    ///   - hatchedAt: ISO-8601 timestamp of when the container was created.
    ///   - lockfilePath: Override for tests; defaults to `LockfilePaths.primaryPath`.
    @discardableResult
    public static func ensureAppleContainerEntry(
        assistantId: String,
        hatchedAt: String,
        lockfilePath: String? = nil
    ) -> Bool {
