            ...
            VCard {
                VStack(alignment: .leading, spacing: VSpacing.lg) {
                    ...
                    Text("AssistantProgressView — macOS only (clients/macos/)")
                        .font(VFont.caption)
                        .foregroundColor(VColor.textMuted)
                    Text("Unified progress indicator for all assistant response lifecycle states. Handles tool running, thinking, processing, complete, and error states. Not available in the shared gallery because it depends on macOS-only imports.")
                        .font(VFont.small)
                        .foregroundColor(VColor.textMuted)
