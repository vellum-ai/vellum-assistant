...
///
/// Extracted from `ChatViewModel+MessageHandling` to isolate the
/// `handleServerMessage()` switch and its per-case helpers into a focused
/// class. Holds a weak reference back to the owning `ChatViewModel` so it
/// can read/write VM state and call streaming/surface helper methods that
/// remain as ChatViewModel extensions.
@MainActor
