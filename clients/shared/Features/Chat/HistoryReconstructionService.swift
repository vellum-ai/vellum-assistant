...
/// from daemon history-response payloads. Extracted from `ChatViewModel` so the
/// heavy work (JSON size estimation, tool-input formatting, surface mapping,
/// image decoding) is decoupled from the view model and can be called from any
/// isolation context.
