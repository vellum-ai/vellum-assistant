...
final class CachedRemoteImageTests: XCTestCase {
    ...
    /// The shared `RemoteImageCache.session` must be configured with a
    ...
    func testSessionHasURLCache() {
        let session = RemoteImageCache.session
        XCTAssertNotNil(
            session.configuration.urlCache,
            "RemoteImageCache.session must have a urlCache configured"
        )
    }
    ...
    func testSessionUsesReturnCacheDataElseLoadPolicy() {
        ...
        XCTAssertEqual(
            ...
            "RemoteImageCache.session should prefer cached data when available"
        )
    }

    /// `RemoteImageCache` is expected to place its on-disk cache under
    ...
    /// With a `nil` URL, constructing `CachedRemoteImage` must not crash
