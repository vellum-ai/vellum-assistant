...
describe("host-proxy-router", () => {
  ...
  describe("presence reporting", () => {
    ...
    test("seeds a paired assistant as its stream goes live", async () => {
      const presencePosts = capturePresencePosts();
      ...
      // A paired connect resolves its token before opening SSE, so its entry
      // has to be registered ahead of connect() for the seed to find it.
      lockfileListener?.({
        assistants: [
          { assistantId: "paired-1", cloud: "paired", runtimeUrl: "https://tunnel.example.com" },
        ],
      ...
      expect(presencePosts.map((post) => post.url)).toEqual([
        "https://tunnel.example.com/v1/clients/presence",
      ]);
      expect(JSON.parse(presencePosts[0]!.body)).toEqual({ state: "idle" });
    });
