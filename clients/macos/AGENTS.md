...
- **`AlignmentBarrierLayout` for LazyVStack protection**: Wrap the `ScrollView` (from the **outside**) with `AlignmentBarrierLayout { ScrollView { ... } }` to block `explicitAlignment` queries from cascading into the lazy container. The barrier returns `nil` for all alignment queries while passing through sizing and placement unchanged. **CRITICAL: The barrier MUST wrap the ScrollView, not its content.** Placing a custom `Layout` _inside_ a `ScrollView` (between the `ScrollView` and a `LazyVStack`) breaks `LazyVStack`'s viewport tracking — cells fail to materialize on resize, send, or content changes until the user scrolls. This is because custom `Layout` containers disrupt the internal communication channel `ScrollView` uses to tell `LazyVStack` which cells are visible. See [`Layout.explicitAlignment`](https://developer.apple.com/documentation/swiftui/layout/explicitalignment(of:in:proposal:subviews:cache:)-3iqmu) and `clients/shared/DesignSystem/Layout/AlignmentBarrierLayout.swift`.
