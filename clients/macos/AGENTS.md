...
- **No `.frame(maxWidth:, alignment:)` inside LazyVStack cell hierarchy**: `.frame(maxWidth: X, alignment: Y)` compiles to `_FlexFrameLayout`, whose `placement()` method queries `ViewDimensions.subscript` for the child's explicit alignment. If children also have `.frame(maxWidth:, alignment:)`, this recurses through the entire subtree — O(depth × children) per layout pass. With nested FlexFrames, a single hover or window resize triggers 3000+ alignment queries and 58+ second hangs. **This includes `.frame(maxWidth: X)` with no explicit alignment parameter** — it defaults to `.center` which still triggers the query. Safe alternatives:
  - `.frame(width: exactWidth)` — compiles to `_FrameLayout`, no alignment query.
  - `HStack { content; Spacer(minLength: 0) }` — leading alignment without alignment queries.
