...
- **No `.transition(.move(edge:))` inside `LazyVStack`**: Never use `.transition(.move(edge:))`, `.transition(.slide)`, or any combined transition containing `.move` on items inside a `LazyVStack`. The `.move` component requires SwiftUI to compute the total content height via `motionVectors` → `measureEstimates`, which iterates every child and defeats lazy loading — causing 100s+ second hangs on long conversations. Use `.transition(.opacity)` instead, or remove the transition entirely. See [WWDC23: Demystify SwiftUI performance](https://developer.apple.com/videos/play/wwdc2023/10160/).
