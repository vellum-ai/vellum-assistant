...
final class SettingsStoreChannelVerificationTests: XCTestCase {
    ...
    // MARK: - Submit Outbound Code

    func testSubmitOutboundCodeSendsCorrectIPCPayload() {
        // Set up outbound session state
        store.smsOutboundSessionId = "sess-submit-1"

        let guardianMessagesBefore = sentMessages.compactMap { $0 as? GuardianVerificationRequestMessage }
        let submitCountBefore = guardianMessagesBefore.filter { $0.action == "submit_outbound_code" }.count

        store.submitOutboundGuardianCode(channel: "sms", code: "123456")

        let guardianMessages = sentMessages.compactMap { $0 as? GuardianVerificationRequestMessage }
        let submitMessages = guardianMessages.filter { $0.action == "submit_outbound_code" }
        XCTAssertEqual(submitMessages.count, submitCountBefore + 1)
        let msg = submitMessages.last!
        XCTAssertEqual(msg.channel, "sms")
        XCTAssertEqual(msg.sessionId, "sess-submit-1")
        XCTAssertEqual(msg.verificationCode, "123456")
        XCTAssertEqual(msg.assistantId, testAssistantId)
    }

    func testFailedSubmitKeepsOutboundSessionAndSurfacesError() {
        // Set up outbound session state
        store.smsOutboundSessionId = "sess-submit-2"
        store.smsOutboundExpiresAt = Date().addingTimeInterval(600)
        store.smsOutboundSendCount = 1

        // Simulate error response for invalid code
        daemonClient.onGuardianVerificationResponse?(GuardianVerificationResponseMessage(
            type: "guardian_verification_response",
            success: false,
            channel: "sms",
        ...
        let predicate = NSPredicate { _, _ in self.store.smsOutboundSubmitError != nil }
        let expectation = XCTNSPredicateExpectation(predicate: predicate, object: nil)
        wait(for: [expectation], timeout: 2.0)

        // Session should still be active
        XCTAssertEqual(store.smsOutboundSessionId, "sess-submit-2")
        XCTAssertNotNil(store.smsOutboundExpiresAt)
        XCTAssertEqual(store.smsOutboundSendCount, 1)

        // Error should be surfaced
        XCTAssertNotNil(store.smsOutboundSubmitError)
        XCTAssertTrue(store.smsOutboundSubmitError!.contains("incorrect"))
    }

    func testSuccessfulSubmitClearsOutboundStateAndMarksVerified() {
        // Set up outbound session state
        store.smsOutboundSessionId = "sess-submit-3"
        store.smsOutboundExpiresAt = Date().addingTimeInterval(600)
        store.smsOutboundSendCount = 1
        store.smsGuardianVerificationInProgress = true

        // Simulate success response with bound: true
        daemonClient.onGuardianVerificationResponse?(GuardianVerificationResponseMessage(
            type: "guardian_verification_response",
            success: true,
            bound: true,
        ...
        // Outbound state should be cleared
        XCTAssertNil(store.smsOutboundSessionId)
        XCTAssertNil(store.smsOutboundExpiresAt)
        XCTAssertNil(store.smsOutboundSubmitError)

        // Guardian should be verified
        XCTAssertTrue(store.smsGuardianVerified)
        XCTAssertEqual(store.smsGuardianIdentity, "+15551234567")
    }

    func testPendingBootstrapDoesNotEnableVerifySubmit() {
        // Set up Telegram outbound session in pending_bootstrap state
        store.telegramOutboundSessionId = "tg-sess-bootstrap"
        store.telegramOutboundExpiresAt = Date().addingTimeInterval(600)
        store.telegramBootstrapUrl = "https://t.me/MyBot?start=verify_abc"

        // Simulate status response with pending_bootstrap
        daemonClient.onGuardianVerificationResponse?(GuardianVerificationResponseMessage(
            type: "guardian_verification_response",
            success: true,
            channel: "telegram",
            verificationSessionId: "tg-sess-bootstrap",
            pendingBootstrap: true
        ))

        let predicate = NSPredicate { _, _ in self.store.telegramOutboundSessionId == "tg-sess-bootstrap" }
        let expectation = XCTNSPredicateExpectation(predicate: predicate, object: nil)
        wait(for: [expectation], timeout: 2.0)

        // Bootstrap URL should still be present (UI uses this to show bootstrap link instead of code entry)
        XCTAssertNotNil(store.telegramBootstrapUrl)
        // No submit error should be present
        XCTAssertNil(store.telegramOutboundSubmitError)
    }
}
