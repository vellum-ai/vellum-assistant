...
final class FeatureFlagManagerTests: XCTestCase {
    ...
    /// Tests that the desktopApp flag is recognized from its environment variable.
    func testDesktopAppFlagFromEnv() {
        // GIVEN an environment with VELLUM_FLAG_DESKTOP_APP set to "true"
        let env = ["VELLUM_FLAG_DESKTOP_APP": "true"]

        // WHEN we create a manager from that environment
        let manager = FeatureFlagManager(environment: env)

        // THEN the flag is enabled via the enum case
        XCTAssertTrue(manager.isEnabled(.desktopApp))

        // AND via the raw string
        XCTAssertTrue(manager.isEnabled("desktop_app"))
    }

    /// Tests that the desktopApp flag defaults to disabled when not set.
    func testDesktopAppFlagDefaultsToDisabled() {
        // GIVEN an empty environment
        let env: [String: String] = [:]
        ...
        // THEN the desktopApp flag is disabled
        XCTAssertFalse(manager.isEnabled(.desktopApp))
    }

    /// Tests that the desktopApp flag can be toggled via setOverride.
    func testDesktopAppFlagOverride() {
        // GIVEN a manager with no flags
        let manager = FeatureFlagManager(environment: [:])

        // WHEN we override desktopApp to true
        manager.setOverride(.desktopApp, enabled: true)

        // THEN the flag is enabled
        XCTAssertTrue(manager.isEnabled(.desktopApp))

        // AND when we remove the override
        manager.removeOverride(.desktopApp)
