...
(function () {
   ...
   * @param {string|Element} container - Element or selector
   * @param {object} [options]
   * @param {string} [options.transition='fade'] - 'fade' | 'slide' | 'none'
   * @param {boolean} [options.showDots=true]
   * @param {boolean} [options.showArrows=true]
   * @param {boolean} [options.showCounter=true]
   * @param {boolean} [options.showProgress=false]
   * @param {boolean} [options.keyboard=true]
   * @param {boolean} [options.loop=true]
   * @param {boolean} [options.autoPlay=false]
   * @param {number} [options.interval=5000]
   * @returns {{ goTo: function, next: function, prev: function }}
   */
  widgets.slideshow = function (container, options) {
    var el = resolveContainer(container);
    if (!el) return;
    ...
    var slides = el.querySelectorAll('.v-slide');
    ...
    if (opts.transition !== 'fade') {
      el.setAttribute('data-transition', opts.transition);
    }

    // Inject navigation elements
    var nav = document.createElement('div');
    nav.className = 'v-slideshow-nav';

    var dotsContainer = null;
    if (opts.showDots) {
      dotsContainer = document.createElement('div');
      dotsContainer.className = 'v-slideshow-dots';
      for (var i = 0; i < slides.length; i++) {
        var dot = document.createElement('button');
        dot.className = 'v-slideshow-dot';
    ...
    if (opts.showCounter) {
      ...
      counterEl.className = 'v-slideshow-counter';
    ...
    if (opts.showArrows) {
      ...
      arrowsContainer.className = 'v-slideshow-arrows';

      var prevBtn = document.createElement('button');
      prevBtn.className = 'v-slideshow-prev';
      prevBtn.setAttribute('aria-label', 'Previous slide');
      prevBtn.innerHTML = '\u2039';
      prevBtn.addEventListener('click', function () { prev(); });

      var nextBtn = document.createElement('button');
      nextBtn.className = 'v-slideshow-next';
    ...
    if (opts.showProgress) {
      ...
      progressBar.className = 'v-slideshow-progress';
      progressFill = document.createElement('div');
      progressFill.className = 'v-slideshow-progress-fill';
      progressBar.appendChild(progressFill);
      el.appendChild(progressBar);
    }

    // Measure tallest slide for min-height
    requestAnimationFrame(function () {
      var maxH = 0;
      for (var s = 0; s < slides.length; s++) {
        slides[s].style.position = 'relative';
        slides[s].style.opacity = '1';
        var h = slides[s].offsetHeight;
        if (h > maxH) maxH = h;
        if (s !== current) {
          slides[s].style.position = '';
          slides[s].style.opacity = '';
        }
      }
    ...
    function goTo(index) {
      if (index < 0 || index >= slides.length) {
        if (!opts.loop) return;
        index = index < 0 ? slides.length - 1 : 0;
      }

      for (var s = 0; s < slides.length; s++) {
        slides[s].classList.remove('active', 'prev');
      }
      if (current !== index) {
      ...
      // Update dots
      if (dotsContainer) {
        var dots = dotsContainer.querySelectorAll('.v-slideshow-dot');
      ...
      // Update counter
      if (counterEl) {
        var cur = String(current + 1);
        var total = String(slides.length);
        if (cur.length < 2) cur = '0' + cur;
      ...
      if (opts.showArrows && !opts.loop) {
        var pBtn = el.querySelector('.v-slideshow-prev');
        var nBtn = el.querySelector('.v-slideshow-next');
        if (pBtn) pBtn.classList.toggle('disabled', current === 0);
        if (nBtn) nBtn.classList.toggle('disabled', current === slides.length - 1);
      }
    ...
    function startAutoPlay() {
      autoTimer = setInterval(function () { next(); }, opts.interval);
    }
