...
extension MainWindowView {
    ...
    func nativePanelView(_ panelId: NativePanelId) -> some View {
        ...
        case .generated:
            GeneratedPanel(
                onClose: { sharingState.showSharePicker = false; windowState.closeDynamicPanel() },
                isExpanded: Binding(
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    HStack(spacing: VSpacing.sm) {
                        ...
                        if sharingState.isPublishing {
                            ProgressView()
                                .controlSize(.small)
                                .frame(height: 24)
                        } else if let url = sharingState.publishedUrl {
                ...
                if let error = sharingState.publishError {
                    HStack {
                        Spacer()
                        Text(error)
                            .font(VFont.caption)
