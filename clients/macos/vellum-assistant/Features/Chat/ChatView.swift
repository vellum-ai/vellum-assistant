...
=======
    ...
    /// Messages filtered for display — excludes subagent notification placeholders.
    /// Computed once per render rather than inside the LazyVStack body.
    private var displayMessages: [ChatMessage] {
        messages.filter { !$0.isSubagentNotification }
    }

    /// The ID of the last assistant message, used to decide whether to show the
    /// feedback/action row at the bottom of a bubble. Computed once per render
    /// rather than O(n) per visible cell inside the ForEach body.
    private var latestAssistantMessageId: UUID? {
        let last = displayMessages.last(where: { $0.role == .assistant })
        return last?.id
    }
                    ...
                    ForEach(Array(displayMessages.enumerated()), id: \.element.id) { index, message in
                        ...
                        if let confirmation = message.confirmation {
                            ...
                            ChatBubble(
                                ...
                                isLatestAssistantMessage: message.role == .assistant && message.id == latestAssistantMessageId
                            )
                                .id(message.id)
                                .transition(.opacity.combined(with: .move(edge: .bottom)))
                        }
