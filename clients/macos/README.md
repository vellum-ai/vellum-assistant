...
The macOS app is a frontend — all inference (chat, computer-use sessions, ambient analysis) goes through the **local assistant process** (internally called the daemon), a backend process that manages LLM API calls, conversation state, and tool execution. The app connects to it via HTTP+SSE (the runtime HTTP server port is resolved dynamically from the lockfile for multi-instance setups).
...
> **Multi-instance note:** The default data directory is `~/.vellum/`. When multiple instances are configured via the lockfile, paths resolve to the instance-specific directory instead. See `VellumPaths.resolveVellumDir()` for resolution logic.
