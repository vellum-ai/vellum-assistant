...
export const buildLinuxMenu = ({
  openAbout,
  checkForUpdates,
  installCli,
}: Omit<LinuxMenuOptions, "handle">): MenuItemConstructorOptions[] => [
  {
    id: "file",
    label: "File",
    submenu: [
...
export const installLinuxMenu = (options: LinuxMenuOptions): void => {
  const apply = (): void => {
    Menu.setApplicationMenu(Menu.buildFromTemplate(buildLinuxMenu(options)));
  };
  ...
  options.handle(MENU_TITLES, z.tuple([]), () =>
    buildLinuxMenu(options).map((item) => ({
      id: String(item.id),
  ...
  options.handle(
    ...
    ([id, x, y], event) => {
      const submenu = buildLinuxMenu(options).find(
        (item) => item.id === id,
      )?.submenu;
