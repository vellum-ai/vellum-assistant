...
if [ "$TARGET_ARCH" != "universal" ] && [ -x "$DEST" ]; then
  ...
  if [ "$CURRENT_VERSION" = "$BUN_VERSION" ]; then
    # Check platform and architecture to determine correct extraction method
...
# Determine the correct platform string for bun downloads
case "$(uname -s | tr '[:upper:]' '[:lower:]')" in
  linux*)
    case "$TARGET_ARCH" in
      aarch64)
        fetch_single_bun "linux-aarch64" "$DEST"
        ;;
      x64)
        fetch_single_bun "linux-x64" "$DEST"
        ;;
      *)
        echo "ERROR: unsupported target arch for Linux: $TARGET_ARCH (use aarch64|x64)" >&2
        exit 1
        ;;
    esac
    ;;
  *)
    # Default to darwin for non-Linux systems (macOS typically)
    case "$TARGET_ARCH" in
      aarch64)
