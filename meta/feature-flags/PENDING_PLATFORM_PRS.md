...
| `memory-v3-injection-gate` | 2026-06-30 | memory-v3 owner | Platform PR not yet opened (as of 2026-06-30) | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` provisioning the `memory-v3-injection-gate` assistant flag, which gates the memory v3 per-turn injection gate. See `../vellum-assistant-platform/.private/plans/memory-v3-injection-gate-flag.md`. |
