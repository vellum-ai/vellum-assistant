...
| `voice-duplex-handoff` | 2026-07-16 | live-voice owner | Platform PR not yet opened (as of 2026-07-16) | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` and `tags = ["assistant"]` provisioning the `voice-duplex-handoff` assistant flag, which gates keeping a barged-in live-voice turn running as a background subagent (true-duplex handoff, JARVIS-1249). Needed so managed assistants can receive a remote override; until then the flag resolves to the registry default (off). |
| `composer-secret-guard` | 2026-07-23 | composer secret guard owner | Platform PR open: [vellum-assistant-platform#9487](https://github.com/vellum-ai/vellum-assistant-platform/pull/9487) (opened 2026-07-24, awaiting merge) | Terraform entry in `../vellum-assistant-platform/terraform/gcp/env/prod/vellum-assistant/main.tf` (the `feature_flags` module `flags` map) with `tags = ["assistant"]` provisioning the `composer-secret-guard` assistant flag, which gates detecting API keys/secrets typed into the chat composer and offering secure credential storage before send. New flags serve false at creation; managed assistants get a remote override once merged and targeted in LaunchDarkly. Remove this row when #9487 merges. |
