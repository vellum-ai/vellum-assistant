...
| `voice-triage-escalate` | 2026-07-12 | voice routing owner | Platform PR opened: `vellum-assistant-platform#9025` (2026-07-12) | Terraform entry in `../vellum-assistant-platform/terraform/gcp/env/prod/vellum-assistant/main.tf` (feature-flags module `flags` map), serves false at creation, provisioning the `voice-triage-escalate` assistant flag, which gates the in-app Voice Mode triage-and-escalate routing (a fast model fronts each voice turn and escalates to the quality model on a tricky turn). Additionally gated behind `voice-mode`. Until provisioned the flag resolves to the registry default (off) and is invisible in the flag UI. Remove this row when `vellum-assistant-platform#9025` merges. |
