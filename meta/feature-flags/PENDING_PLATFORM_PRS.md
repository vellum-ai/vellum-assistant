...
| `composer-secret-guard` | 2026-07-23 | composer secret guard owner | Platform PR not yet opened (as of 2026-07-23) | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` and `tags = ["assistant"]` provisioning the `composer-secret-guard` assistant flag, which gates detecting API keys/secrets typed into the chat composer and offering secure credential storage before send. Needed so managed assistants can receive a remote override; until then the flag resolves to the registry default (off). |
