...
| `schedule-usage-ui` | 2026-06-04 | schedule usage owner | Platform PR not yet opened (as of 2026-06-04) | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` provisioning the `schedule-usage-ui` client flag before schedule usage UI is enabled remotely. |
