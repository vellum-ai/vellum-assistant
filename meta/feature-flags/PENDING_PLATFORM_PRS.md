...
| `memory-plugin-provider` | 2026-06-28 | memory-plugin owner | Platform PR not yet opened (as of 2026-06-28) | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` provisioning the `memory-plugin-provider` flag (scope `both`). Gates whether an external `provides: "memory"` plugin may take over from the built-in memory system; production is not green until the flag is provisioned. A one-PR companion plan should be added in `../vellum-assistant-platform/.private/plans/`. |
