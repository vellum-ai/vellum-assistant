...
| `skill-creation-card` | 2026-07-07 | skill surfacing owner | Platform PR planned (companion Terraform PR per `../vellum-assistant-platform/.private/plans/skill-surfacing-v1-flag.md`) | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` provisioning the `skill-creation-card` flag (scope `both`), which gates the in-chat skill-creation card inserted by the memory retrospective. Until it applies, the flag resolves from the registry default (fail-closed). |
