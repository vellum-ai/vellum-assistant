...
| `messages-search-backend` | 2026-06-30 | messages search owner | Companion platform PR open as `vellum-assistant-platform` #8742 (as of 2026-07-01); the registry `defaultEnabled` is flipped `false` → `true` in this repo's PR 9 | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) provisioning the `messages-search-backend` assistant-scope boolean flag (off = `fts5` SQLite full-text search, on = `qdrant` sparse lexical index). Platform PR #8742 provisions the flag **off-by-default** in LaunchDarkly; on-targeting for managed assistants is done later in LD once the flag exists. #8742 must land before managed assistants are flipped so they can receive the override. |
