...
| `ios-voice-audio-session` | 2026-07-29 | iOS voice owner | Platform PR not yet opened (as of 2026-07-29) | Terraform entry in `../vellum-assistant-platform` provisioning the client-scoped `ios-voice-audio-session` flag with `defaultEnabled: false`. Lets the iOS shell configure the shared `AVAudioSession` for background/lock-screen voice audio. Declared on the first-class-iOS-voice-mode branch (#39306) rather than `main`. Until the platform entry exists it resolves to the registry default (off), which is the intended state — it must not be enabled for anyone until measured on a physical device (the Simulator's mock audio device cannot evaluate it, and the same pattern shipped once as #39331 and killed capture outright). |
