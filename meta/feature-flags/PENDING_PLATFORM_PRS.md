...
| `summarize-up-to-here` | 2026-07-08 | summarize-up-to-here owner | Platform PR not yet opened (as of 2026-07-08) | Terraform entry in `../vellum-assistant-platform/terraform/launchdarkly.tf` (or equivalent) with `defaultEnabled: false` provisioning the both-scope `summarize-up-to-here` flag, which gates the per-message "Summarize up to here" web action and the `POST /v1/conversations/summarize` endpoint. |
