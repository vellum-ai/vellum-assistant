...
const MARKETPLACE_PATH = join(ROOT, "..", "plugins", "marketplace.json");
...
for (const { canonical, targets } of PAIRS) {
  if (PLUGINS_ONLY && canonical !== MARKETPLACE_PATH) {
    continue;
  }
...
if (CHECK) {
  ...
  console.log(
    `✓ All ${count} committed bundled copies match their canonical sources`,
  );
} else {
  console.log(`✓ Synced ${count} bundled copies`);
}
