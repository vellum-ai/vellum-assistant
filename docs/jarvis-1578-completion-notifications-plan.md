...
Background provenance reuses existing task/tool IDs and persisted trigger/result rows. The recipient is the assistant's active Vellum guardian, matching existing reply delivery; task records do not carry a separate launch-time recipient. The resolved principal is included in the notification's existing context payload and checked before local or mobile delivery. Recipient-owned background completions stay out of the assistant-wide Home feed and notification bell because those surfaces do not filter by recipient. Their results remain in the source conversation, with scoped desktop/browser and mobile alerts. No database or wire-format migration is introduced.
