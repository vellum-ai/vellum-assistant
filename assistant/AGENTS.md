...
- Per-conversation override (§8.3) may lift *or* tighten the type-level floor for a specific conversation. The client UI / CLI writes it to the gateway's `conversation_admission_override` table; `handle-inbound` looks it up and forwards it as `sourceMetadata.admissionConversationOverride`; the runtime resolves type-floor vs. override into the effective policy via `resolveEffectivePolicy` (shared in `@vellumai/gateway-client`) for both the ACL pre-pass and the floor stage. A conversation pinned to `no_one` is hard-denied pre-forward by the gateway kill switch, exactly like a type-level `no_one`.
