 ...
 * `src/plugin-api/bundle/index.js`. The bundle is what gets:
 *
 ...
 * `src/plugin-api/`. The committed `bundle/index.js` is the canonical
 ...
 *
 * NOTE: we name this directory `bundle/` rather than the conventional
 * `dist/` to dodge the root `.gitignore`'s `dist` pattern — committing
 * the artifact is intentional here so CI/Docker/macOS builds don't all
 * need a separate \`bun run build:plugin-api\` step before typecheck/test.
 */
...
const OUT_DIR = join(ASSISTANT_DIR, "src/plugin-api/bundle");
