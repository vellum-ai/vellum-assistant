...
export const graphRecallDefinition = {
  name: "recall",
  description:
    'Search local information the moment you feel uncertain. Use recall for memory, past conversations, and workspace files — before you guess, before you ask, before you hedge. Auto-injection is incomplete by design; it surfaces patterns, not the specifics you need to answer well. If you catch yourself reaching for "I think", "I believe", "if I remember", "didn\'t we", "last time" — that\'s the signal. Recall. If a turn references someone, a place, a decision, a document, or prior work you should be able to find locally — recall. Recall is a snapshot of what was known when it was written, not a window into live systems: for the current state of external resources (channel messages, inbox contents, calendar events, API data), query the source directly, and when recall disagrees with freshly fetched data, the live data wins. Call it multiple times per conversation if the turn warrants it. Be specific in your query for best results.',
  input_schema: {
    type: "object",
    properties: {
      query: {
