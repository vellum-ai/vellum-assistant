...
- **Procedure, every sight →** capture it as a CANDIDATE NOTE, not a skill: write a NEW concept article carrying \`kind: proc-candidate\` and a \`goal:\` frontmatter line stating the procedure's intent in one phrase (\`goal: "deploy the web app to staging"\`). The body holds this run's steps. **One candidate note = one observed trace, and each trace is what the distill pass counts toward recurrence — so each note must be DISTINCT.** This is APPEND-ONLY: never overwrite or update an existing \`proc-candidate\` note for a later sighting of the same procedure; write a fresh note with a UNIQUE slug instead (carry the same \`goal:\` so the distill pass clusters them, and disambiguate the slug with a short timestamp or run marker, e.g. \`deploy-web-staging-2026-06-22\`). Updating the prior note in place would make the recurrence count stall at one. **Do NOT scaffold a skill** — first sight is evidence of nothing yet; a separate pass distills candidates into skills once a procedure recurs.
- **Procedure, explicitly taught** ("from now on, always…", "the way to do X is…") **→** write the new candidate note (same append-only, unique-slug rule) AND set \`explicit: true\` in its frontmatter. An explicit instruction is itself evidence of reusability, so the distill pass promotes it immediately rather than waiting for recurrence.
