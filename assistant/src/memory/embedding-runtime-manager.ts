...
export class EmbeddingRuntimeManager {
  ...
  private async install(signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      // Step 3: Write a thin wrapper module that re-exports @huggingface/transformers.
      // Bun shares the module cache between CJS and ESM, so there is no
      // dual-instance issue — onnxruntime-node's backend registration is
      // visible to the ESM imports in @huggingface/transformers.
      ...
      // Step 5: Atomic swap — remove old install and rename temp to final
