 ...
 */
export function mergeSkippedConfigKeys(
  legacyPath: string,
  ...
  if (merged.length > 0) {
    try {
      ...
      migrationLog("info", "Merged legacy config keys into workspace config", {
        keys: merged,
      });
    } catch (err) {
      migrationLog("warn", "Failed to merge legacy config keys", {
        err: String(err),
        keys: merged,
      });
    }
