...
export class AgentMailProvider implements EmailProvider {
  ...
  async createInbox(opts: CreateInboxOpts): Promise<EmailInbox> {
    ...
    const inbox = await this.client.inboxes.create(
      params as Parameters<typeof this.client.inboxes.create>[0],
    );
  ...
  async setupWebhook(opts: SetupWebhookOpts): Promise<EmailWebhook> {
    const result = await this.client.webhooks.create({
      url: opts.url,
      eventTypes:
        (opts.events as import("agentmail").AgentMail.EventType[]) ?? [
          "message.received",
        ],
    });
    return {
  ...
  async createDraft(opts: CreateDraftOpts): Promise<EmailDraft> {
    ...
    const result = await this.client.inboxes.drafts.create(
      opts.inboxId,
      params,
    );
...
function mapMessageItem(
  m: import("agentmail").AgentMail.MessageItem,
): EmailMessage {
  return {
    id: m.messageId,
    threadId: m.threadId,
...
function mapThreadItem(
  t: import("agentmail").AgentMail.ThreadItem,
): EmailThread {
  return {
    id: t.threadId,
    subject: t.subject,
