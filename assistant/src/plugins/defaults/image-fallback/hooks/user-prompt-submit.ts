 ...
 *
 * 1. Self-gates on `isNonInteractive` — background/scheduled runs skip the
 *    added latency of captioning.
 * 2. Resolves the active profile from `modelProfileKey` (or the workspace's
 *    active profile when the key is `null`) and checks `doesSupportVision`.
 *    If the model already handles images, the hook is a no-op.
 * 3. Finds a vision-capable profile for captioning via `findVisionProfile`.
 *    If none exists, images are replaced with a fail-open placeholder so the
 *    model at least knows an image was present.
 * 4. Scans all messages for `ImageContent` blocks, captions each (with an
 *    in-memory content-hash cache to avoid re-captioning across turns), and
 *    replaces the block with `[Image: <caption>]`.
 *
 * Fail-open is the dominant error mode: a captioning failure leaves a
 * placeholder text block rather than the raw image (which would cause a
 * provider rejection on a text-only model) or dropping the image entirely
...
const userPromptSubmit: PluginHookFn<UserPromptSubmitContext> = async (ctx) => {
  // Background/scheduled runs skip the added latency of captioning.
  if (ctx.isNonInteractive) return;

  // Resolve the active profile. When modelProfileKey is null the workspace's
  // active profile is in use — find it via the isActive flag.
      ...
      if (visionProfileKey != null) {
        ...
        message.content[i] = {
          ...
          text:
            caption != null
              ? `[Image: ${caption}]`
              : "[Image: captioning failed — unable to describe]",
        };
      } else {
        // No vision profile configured at all — fail-open placeholder.
        message.content[i] = {
          type: "text",
          text: "[Image: no vision-capable model configured to describe this image]",
        };
      }
    }
  }
