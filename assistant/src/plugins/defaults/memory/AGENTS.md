...
| `isV3TierActive`                      | memory on AND v3 live (the v3 TIER: `memoryTier() === "v3"`)                       | `GET /memory-graph` + the `graph_supported` bit                                                                                                                                                                                         |
| `isRetrospectiveSkillAuthoringActive` | v3 tier active AND `memory.retrospective.skillAuthoring !== false` (defaults true) | procedural-memory-as-skills: the fork instruction's skill-authoring section, the retrospective wake's authoring-tool allowlist + preactivation, and the checker's origin-scoped grant (via the `procToSkillsActive` policy-context bit) |
