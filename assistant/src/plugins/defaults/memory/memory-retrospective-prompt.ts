...
3. Extract code you actually ran into reusable \`scripts/\` companion files. When the trace shows you writing and successfully executing a script or non-trivial code (via \`python3\` or \`bun\`), save the exact version that worked as a companion file via the same \`files\` input (for example \`scripts/export-report.py\`) instead of pasting snippets into the body, and have the SKILL.md body give the literal invocation to rerun it (for example \`python3 scripts/export-report.py <args>\` or \`bun run scripts/fetch-stats.ts\`). A stored working script is cheaper and more reliable than re-deriving the code from snippets on every future use. Only store code the trace proves ran, copied verbatim — do not invent or reconstruct scripts that were never executed, and only target \`python3\` or \`bun\` (the runtimes guaranteed to exist). These are plain files run later via the terminal, not registered skill tools.
