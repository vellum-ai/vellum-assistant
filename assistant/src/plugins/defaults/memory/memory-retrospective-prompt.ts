...
3. Extract code you actually ran into reusable \`scripts/\` companion files. When the trace shows a script or non-trivial snippet you executed successfully (\`python3\` or \`bun\` — the only runtimes present), save that exact code verbatim as a companion file (for example \`scripts/export-report.py\`) — never a reconstruction of code the trace does not prove ran. When the trace shows the file's absolute path and the file is still there, do NOT re-type its contents: author the skill first, then copy the file in with one \`bash\` call — \`assistant skills companion add <skill_id> --path scripts/<name> --from <absolute-path>\`. Pass inline \`content\` in \`files\` only when the code survives nowhere but the trace. The copy is refused unless the skill is one you authored, which is the same rule step 1 already applies. Have the body give the literal rerun command anchored with \`{baseDir}\` (for example \`python3 {baseDir}/scripts/export-report.py\`); it resolves to the skill folder on load, and the terminal does not run from there. A stored working script beats re-deriving the code on every future use.

   \`bash\` is available to you for that copy and nothing else. Do not use it to explore the filesystem, inspect files, or run anything the procedure did not run — you are reviewing a transcript, not working on a task.
