...
// imports beyond the two scored-hit types. `config.enabled` is the
// `memory.v3.gate.enabled` kill-switch supplied by the caller, which also
// feeds in the finder-lane hits.
...
export interface V3GateConfig {
  enabled: boolean; // the `memory.v3.gate.enabled` kill-switch, threaded through by observeTurn
