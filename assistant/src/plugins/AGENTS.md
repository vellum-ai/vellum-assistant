...
- **Namespace reservation**: the `plugins/<name>/` prefix under `/x/` resolves only against that plugin's own `routes/` directory — never against a workspace `routes/plugins/…` file — so plugins can't collide with workspace routes or each other. A missing file 404s; nothing is registered ahead of time.
- **Default plugins resolve from the source tree**: first-party default plugins (`plugins/defaults/<name>/`) live in the binary's source, not the workspace, so their routes resolve from `plugins/defaults/<name>/routes/` (keyed by the plugin's _directory_ name, e.g. `/x/plugins/platform-hosted/reengage`). This lookup is derived from the source location via `import.meta.dir` in `plugins/defaults/main.ts` (`getDefaultPluginRoutesDir` / `getDefaultPluginRouteRoots`), relying on the assistant shipping its source un-bundled. An installed workspace plugin of the same name that ships a `routes/` directory takes precedence, so it can override a default. The resolution + discovery merge lives in `runtime/routes/user-route-resolution.ts` (`resolveRouteLocation` / `listPluginRouteRoots`), shared by the dispatcher and the CLI so they never drift.
