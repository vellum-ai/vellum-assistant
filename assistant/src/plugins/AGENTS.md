...
- **Runtime access**: handlers take the Web `Request` and return a `Response` — no injected context. To reach daemon state (publish an event, run a conversation turn) they import the relevant helpers from `@vellumai/plugin-api` (e.g. `publishEvent`, `runConversationTurn`). Handlers execute in the route host subprocess (`routes/worker.ts`), off the daemon's event loop.
