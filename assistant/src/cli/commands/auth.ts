  ...
  $ assistant auth info --json`,
        ...
        .action(async (_opts: Record<string, unknown>, cmd: Command) => {
          ...
          if (!shouldOutputJson(cmd)) {
            log.info(`Platform URL:        ${r.result.platformUrl || "(not set)"}`);
            log.info(`Assistant ID:        ${r.result.assistantId || "(not set)"}`);
            log.info(`Organization ID:     ${r.result.organizationId || "(not set)"}`);
            log.info(`User ID:             ${r.result.userId || "(not set)"}`);
            log.info(`Authenticated:       ${r.result.authenticated ? "yes" : "no"}`);
            if (!r.result.authenticated && r.result.message) {
              log.info("");
