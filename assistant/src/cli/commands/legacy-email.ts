/**
 * Legacy email CLI — AgentMail-based provider commands.
 *
 * This file contains the original provider-agnostic email CLI built on top
 * of the AgentMail provider. It is preserved for backward compatibility
 * while the new Vellum-native email commands are built out in email.ts.
 *
 * Accessible via: `assistant email legacy`
 */
...
export function registerLegacyEmailCommand(parent: Command): void {
  const email = parent
    .command("legacy")
    .description("Legacy email operations (AgentMail provider)")
