...
Override a model's context-window ceiling (e.g. for a self-hosted model the
built-in catalog doesn't know, or to pin a smaller limit) via
llm.modelContextLimits — an array of { provider, modelPattern, contextWindowTokens }
matched by provider + longest modelPattern prefix.

Examples:
  $ assistant config set llm.default.provider anthropic
  $ assistant config set calls.enabled true
  $ assistant config set llm.modelContextLimits '[{"provider":"ollama","modelPattern":"llama","contextWindowTokens":32000}]'`,
        )
        .action(
