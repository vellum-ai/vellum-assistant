...
function requireId(
  ...
  if (!trimmed) {
    const error =
      "Memory item ID is required. Run 'assistant memory list' to find it.";
    if (opts.json) {
...
(${MEMORY_KINDS}),
a subject line, and a statement. The list/get/create/update/delete verbs
expose full CRUD over them. Items are normally created by the assistant via
the remember tool; 'memory create' exists for manual seeding and repair.
...
Examples:
  $ assistant memory list --search "coffee"
  $ assistant memory update 9f2c4f3a-3f1a-41e4-88e7-abc123 --statement "Prefers tea"
  $ assistant memory delete 9f2c4f3a-3f1a-41e4-88e7-abc123
...
Examples:
  ...
  $ assistant memory list --kind semantic --limit 20
  $ assistant memory list --search "favorite restaurants" --json`,
        )
        .action(
          async (
            opts: {
      ...
      memory
        ...
        .addHelpText(
          ...
          `
Arguments:
  <id>   Memory item ID (UUID) — run 'assistant memory list' to find it.

Examples:
  $ assistant memory get 9f2c4f3a-3f1a-41e4-88e7-abc123
  $ assistant memory get 9f2c4f3a-3f1a-41e4-88e7-abc123 --json`,
        )
...
Examples:
  $ assistant memory create --kind semantic --statement "User prefers dark mode"
  $ assistant memory create --kind procedural --subject "Deploys" \\
...
Behavior:
  ...
  retires it (same effect as 'assistant memory delete'). Fails with a
  conflict error when the new content duplicates another active item.

Examples:
  $ assistant memory update 9f2c4f3a-3f1a-41e4-88e7-abc123 --statement "Prefers tea over coffee"
  $ assistant memory update 9f2c4f3a-3f1a-41e4-88e7-abc123 --importance 0.9 --kind semantic
  $ assistant memory update 9f2c4f3a-3f1a-41e4-88e7-abc123 --status active`,
        )
        .action(
          async (
            id: string,
            ...
            if (Object.keys(body).length === 0) {
              log.error(
                "At least one update flag is required. Run 'assistant memory update --help' for the available flags.",
              );
              process.exitCode = 1;
              return;
            }
...
Behavior:
  ...
  'assistant memory update <id> --status active'.

Examples:
  $ assistant memory delete 9f2c4f3a-3f1a-41e4-88e7-abc123
  $ assistant memory delete 9f2c4f3a-3f1a-41e4-88e7-abc123 --force --json`,
        )
        .action(
          async (
            id: string,
