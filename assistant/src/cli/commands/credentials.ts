        ...
        .action(async (opts: { search?: string }, cmd: Command) => {
          try {
            ...
            if (!r.ok) {
              writeError(
                cmd,
                r.error ?? "Failed to connect to the assistant",
              );
              process.exitCode = 1;
              return;
            }

            const credentials = r.result?.credentials ?? [];
            const managedCredentials = r.result?.managedCredentials ?? [];
...
Examples:
  ...
  $ assistant credentials status --json`,
        ...
        .action(async (_opts: Record<string, unknown>, cmd: Command) => {
          try {
            const r =
              await cliIpcCall<Record<string, unknown>>("credential_status");
      ...
      credential
        ...
        .option(
          "--label <label>",
          'Human-friendly label (e.g. "prod", "work")',
        )
        .option(
          ...
          async (
            ...
            try {
              ...
              if (shouldOutputJson(cmd)) {
                ...
                log.info(
                  `Stored credential ${service}:${field} (${r.result?.credentialId})`,
                );
              }
            } catch (err) {
              const message = err instanceof Error ? err.message : String(err);
        ...
        .action(
          async (opts: { service: string; field: string }, cmd: Command) => {
            try {
              ...
              const r = await cliIpcCall<{ ok: true; service: string; field: string }>(
                "credential_delete",
                {
                  body: { service, field },
                },
        ...
        .action(
          async (
            ...
            try {
              ...
              if (opts.service) queryParams.service = opts.service;
              if (opts.field) queryParams.field = opts.field;
              ...
              if (shouldOutputJson(cmd)) {
                ...
                printCredentialHuman(r.result as Record<string, unknown>);
                if (r.result?.brokerUnreachable) {
                  log.info(
        ...
        .action(
          async (
            ...
            try {
              ...
              const r = await cliIpcCall<{ ok: true; value: string }>(
                "credential_reveal",
                { queryParams },
              );
              ...
              if (opts.injectionTemplates) {
                try {
                  ...
                  if (!Array.isArray(injectionTemplates)) {
                    writeError(
                      cmd,
                      "--injection-templates must be a JSON array",
                    );
                    process.exitCode = 1;
                    return;
                  }
