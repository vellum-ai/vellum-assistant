    ...
    build: (credential) => {
      ...
      subcommand(credential, "set").action(
        async (
          ...
          // A conversation ID in the environment means this process was
          // spawned by the agent bash tool or skill sandbox, so an inline
          // value likely transited the conversation and would persist
          // unredacted in the transcript.
          if (tryResolveConversationId() !== undefined && !opts.generated) {
            writeError(
              cmd,
              `Refusing to store an inline secret from an agent shell: the value likely transited the conversation and would persist in the transcript. ` +
                `Collect it securely via the app UI instead: assistant credentials prompt --service ${opts.service} --field ${opts.field} --label "…" ` +
                `If the value was machine-generated by the agent itself (e.g. "$(uuidgen)" or an API exchange result) and was never typed or pasted by the user, re-run with --generated.`,
            );
            process.exitCode = 1;
            return;
          }
