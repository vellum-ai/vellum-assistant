...
interface AuditInvocationsResult {
  invocations: Array<{
    createdAt: number;
    toolName: string;
    input: string;
...
Examples:
  ...
  $ assistant audit --limit 50`,
        ...
        .action(async (opts: { limit: string }) => {
          const response = await cliIpcCall<AuditInvocationsResult>(
            "audit_recent_invocations",
          ...
          if (!response.result) {
            log.error("audit_recent_invocations returned ok with no result body");
            process.exitCode = 1;
            return;
          }
