 ...
 * matter what `llm.profiles` holds, and the schema rejects it as
 * `activeProfile` or `advisorProfile`, so a user-created profile that
 * happens to carry the name would be silently replaced in the effective
 * catalog and its selections stranded.
 *
 ...
 * gives such a profile a fresh key, rewriting every reference to it so the
 ...
 *     rows
 *
 * The new key is `jev-managed-custom`, suffixed `-custom-2`, `-custom-3`,
 * ... when that is taken. Candidates are checked against every existing
 * profile key AND every reserved code-defined name, so a rename never lands
 * on another name the catalog owns.
 *
 * Only a workspace-owned profile is renamed. An entry whose `source` is
 * `managed` is a thin stub for the code-owned profile, and a non-object
 * value under the key is not a profile at all; both are left alone.
 *
 * The DB rewrite is not best effort: a pin the rewrite abandons would route
 * through the managed Jev profile instead of the user's own, so a database
 * that exists but cannot be written is retried with a short backoff and a
 * failure that outlives the retries throws; `retryFailedCheckpoint` re-runs
 * the whole migration on the next boot. The DB rewrite runs before the
 * config write, so a throw leaves config.json holding the old key.
 *
 * Idempotent: after a successful run no workspace-owned profile carries the
 * reserved name, so a re-run finds nothing to rename and writes nothing.
 */
...
/**
 * The newly reserved key this migration clears. Frozen snapshot as of
 * 2026-09-22 (migrations are self-contained).
 */
const RESERVED_KEYS = ["jev-managed"];

/**
 * Every code-defined profile name as of 2026-09-22: the default keys, the
 * backups, the flag-gated `os-beta`, and the Jev profile. A renamed profile
 * must not land on any of them, or the rename would just move the collision.
 */
...
export const renameCollidingJevProfileNameMigration: WorkspaceMigration = {
  ...
  async run(workspaceDir: string): Promise<void> {
    ...
    log.info(
      ...
      "Renamed a user profile colliding with the reserved managed Jev profile key",
    );
  },
...
/**
 * Old key -> new key for a workspace-owned profile sitting on the reserved
 * name. Deterministic: each assigned name is reserved against later
 * candidates, so the same config always produces the same mapping.
 */
function planRenames(profiles: Record<string, unknown>): Map<string, string> {
  ...
  for (const key of RESERVED_KEYS) {
    ...
    if (entry === null) {
      log.warn(
        { profile: key },
        "Left a non-object value under the reserved Jev profile key; it is not a profile to preserve",
      );
      continue;
    }
    if (entry.source === "managed") {
      // A managed stub is the workspace's overlay slot for the code-owned
      // profile, not a user profile; renaming it would manufacture a phantom
      // user profile.
      continue;
    }
