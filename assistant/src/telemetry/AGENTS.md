...
Pre-flush validation (`telemetry-wire-validation.ts`) checks outgoing events against the wire schemas. Events that fail their schema are dropped before POST, counted, and not retried (ack-mode rows are deleted; a watermark batch that is entirely invalid advances its cursor). Event types missing from the local wire copy are still sent. Validation never mutates an event and never substitutes a schema-transformed value for the original.

## Adding a new event type starts platform-side, not here

The ingest endpoint silently skips events whose type has no registered serializer (the batch still 2xxes and the daemon acks away its outbox rows), so an emitter shipped before its platform serializer loses every event it records. Pre-flush validation drops schema-invalid events before POST. Types missing from the local wire copy are still sent, so a serializer that has not synced yet is not purged client-side.
