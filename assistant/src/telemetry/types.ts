...
export interface LlmUsageTelemetryEvent extends TelemetryEventBase {
   ...
   * (`researcher`, `builder`, `advisor`, or `unscoped` for a spawn that named
   * none). An open string set: older rows carry role names nothing emits any
   * more. The advisor is a ROLE, not an `LLMCallSiteEnum` value, so this is the only
