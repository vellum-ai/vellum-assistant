 ...
 * pluggable behavior): the agent loop bounds each result at the `post-tool-use`
 * chokepoint before it joins the provider-bound history
 * ({@link truncateToolResult}), and the compaction overflow reducer shrinks
 * every result already in a history ({@link truncateToolResultsAcrossHistory}).
 * Side-effect free — importing it registers nothing.
 */
