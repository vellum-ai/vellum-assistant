...
describe("renderNotificationAvatarPng", () => {
  ...
  nativeTest(
    "fills the circle with an uploaded image when it has no accent",
    async () => {
  ...
  for (const [shape, width, height] of [
    ...
    nativeTest(
      `cover-crops a ${shape} upload to the full disc`,
      async () => {
        await writeUpload("png", null, { width, height });
  ...
  nativeTest(
    ...
    async () => {
      ...
      for (const point of [{ x: 128, y: 128 }, DISC_SAMPLE]) {
        const pixel = image.at(point.x, point.y);
