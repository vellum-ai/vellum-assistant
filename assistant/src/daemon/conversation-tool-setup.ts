...
export function createToolExecutor(
  ...
  return async (
    ...
    // pre-execution side effect — so a non-allowlisted tool can neither run
    // nor mutate conversation state. `skill_execute` is dispatch indirection:
    // it is gated on its resolved inner tool name inside the interception
    // below, mirroring how wire mode gates the underlying tool, not the
    // wrapper.
...
export interface SkillProjectionContext {
  ...
  /** Per-turn override profile for nested tool invocations. */
