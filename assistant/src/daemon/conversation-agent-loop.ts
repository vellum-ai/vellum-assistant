...
export async function runAgentLoopImpl(
  ...
  // Captured when the loop emits its terminal `budget_yield_unrecovered` exit.
  // SSE emission happens immediately at the detection site; assistant-row
  // persistence is deferred until after the pendingToolResults flush so we
  // don't orphan tool_use/tool_result pairs in the durable history.
   ...
   * truncation and `overflowRecovery` for its proactive budget gate and the
   * reactive overflow-recovery ladder, applying the long-history
   * safety-margin bump itself off its own running history. `maxAttempts`
   * bounds the reducer's middle tiers; `allowAutoCompressLatestTurn` is the
   * resolved overflow policy (the `auto_compress` vs `fail_gracefully`
   * decision from interactivity) that gates the ladder's terminal rung.
  ...
  const resolveContextWindow = (): {
    ...
    return {
      ...
      overflowRecovery: {
        enabled: overflowRecovery.enabled,
  ...
  try {
     ...
     * (subsuming the proactive turn-start compaction), compacts in place
     * whenever the gate trips, and owns reactive recovery when the provider
     * rejects a call as context-too-large. Reruns omit it and skip the
     * first-call gate.
     */
    ...
    // The agent loop owns reactive context-overflow recovery end-to-end:
    // it catches the provider's context-too-large rejection, recalibrates
    // the token estimator from the real token count, drives the compaction
    // plugin's reduction ladder one rung per rerun, and — once the ladder is
    // exhausted — emits its own terminal exit reason. Here we only render the
    // matching user-facing notice, since the durable rows can only be written
    // daemon-side. `context_too_large` is a genuine unrecoverable overflow;
    // `budget_yield_unrecovered` is a turn that gave up after the policy-gated
    // auto-compress rung ran.
    if (
      state.terminalOverflowReason === "context_too_large" &&
      ...
      state.terminalOverflowReason === "budget_yield_unrecovered" &&
      !abortController.signal.aborted
    ) {
      // The live SSE event fires now; the durable notice row + synthetic
      // inspector log are persisted below, after the pendingToolResults flush,
      // so the transcript reads as: tool-use → tool results → "I couldn't fit
      // the next step…" notice. Persisting earlier would orphan an
      // assistant(tool_use) from its user(tool_result), breaking provider
      // adjacency on replay.
    ...
    if (budgetYieldClassification && !abortController.signal.aborted) {
      ...
      // yield event.
      //
      // The agent loop already emitted its `agent_loop_exit`, so
      // `setAgentLoopExitReasonOnLatestLog` has stamped the prior real
      // mainAgent call with the exit reason. This synthetic row carries
      // `agent_loop_exit_reason` at insert time, preserving the existing
      // "latest LLM call carries the exit reason" invariant other consumers
      // depend on.
      //
