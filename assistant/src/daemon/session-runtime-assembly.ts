...
/** Session-level onboarding mode guidance generated by assistant orchestration. */
export interface OnboardingModeContext {
  channelId: string;
  phase: string;
  source: string;
  prompt: string;
...
/**
 * Prepend structured onboarding-mode context for assistant-owned onboarding orchestration.
 */
export function injectOnboardingModeContext(
  message: Message,
  ctx: OnboardingModeContext,
): Message {
  const lines: string[] = [
    '<onboarding_mode>',
    `channel_id: ${ctx.channelId}`,
    `phase: ${ctx.phase}`,
    `source: ${ctx.source}`,
    ...(ctx.homeBaseAppId ? [`home_base_app_id: ${ctx.homeBaseAppId}`] : []),
    '',
    'Onboarding runtime guidance:',
    ctx.prompt,
    '</onboarding_mode>',
  ];
...
/**
 * Strip `<onboarding_mode>` blocks injected by `injectOnboardingModeContext`.
 */
export function stripOnboardingModeContext(messages: Message[]): Message[] {
  return messages.map((message) => {
    if (message.role !== 'user') return message;
    const nextContent = message.content.filter((block) => {
      if (block.type !== 'text') return true;
      return !block.text.startsWith('<onboarding_mode>');
    });
    if (nextContent.length === message.content.length) return message;
    if (nextContent.length === 0) return null;
    return { ...message, content: nextContent };
...
export function applyRuntimeInjections(
  ...
  options: {
    ...
    onboardingMode?: OnboardingModeContext | null;
  ...
  if (options.onboardingMode) {
    ...
    if (userTail && userTail.role === 'user') {
      result = [
        ...result.slice(0, -1),
        injectOnboardingModeContext(userTail, options.onboardingMode),
      ];
    }
  }
