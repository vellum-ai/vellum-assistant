...
/**
 * The live, per-turn values a re-injection overlays onto the turn's base
 * injection options. A re-injection reuses the turn's original
 * {@link RuntimeInjectionOptions} bag (channel capabilities, disk-pressure
 * context, surfaces, etc.) but refreshes the scratchpad / memory / context
 * fields that may have changed since the turn's first injection, and applies
 * the caller's suppression policy by passing already-resolved values (a `null`
 * `workspaceTopLevelContext` or `slackChronologicalMessages` suppresses that
 * block).
 */
export interface ReinjectionContext {
  /** The turn's base injection options. */
  baseOptions: RuntimeInjectionOptions;
  /** Current NOW.md scratchpad content. */
  nowScratchpad: string | null;
  /** Current PKB context block. */
  pkbContext: string | null;
  /** Current memory-v2 static block. */
  memoryV2Static: string | null;
  /** Resolved workspace top-level context, or `null` to suppress it. */
  workspaceTopLevelContext: string | null;
  /** Resolved Slack chronological snapshot, or `null` to suppress it. */
  slackChronologicalMessages: Message[] | null;
  /** Injection mode for this re-injection. */
  mode: InjectionMode;
  /** Per-turn context forwarded to the injector chain. */
  turnContext: TurnContext;
}

/**
 * Re-apply the runtime-injection chain to an existing `history`, overlaying
 * the live {@link ReinjectionContext} values onto the turn's base options.
 *
 * This is the importable seam for re-injection: it receives the message
 * history plus the current injection context and returns the edited history
 * (and captured blocks), with no dependency on the agent loop's closure state.
 * The memory system is expected to own this transform as a post-compaction
 * hook in a future change.
 */
export async function reapplyRuntimeInjections(
  history: Message[],
  context: ReinjectionContext,
): Promise<RuntimeInjectionResult> {
  return applyRuntimeInjections(history, {
    ...context.baseOptions,
    pkbContext: context.pkbContext,
    memoryV2Static: context.memoryV2Static,
    nowScratchpad: context.nowScratchpad,
    workspaceTopLevelContext: context.workspaceTopLevelContext,
    slackChronologicalMessages: context.slackChronologicalMessages,
    mode: context.mode,
