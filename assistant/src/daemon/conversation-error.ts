...
// Managed-proxy quota/limit responses are generated by Vellum, even though
// they travel through provider SDKs and get wrapped as ProviderError.
const MANAGED_PROXY_LIMIT_PATTERNS = [
...
function classifyCore(
  ...
  if (error instanceof ProviderError && error.statusCode !== undefined) {
    ...
    if (error.statusCode === 429) {
      if (isManagedProxyLimitError(error, message)) {
        return {
...
function isManagedProxyLimitError(error: unknown, message: string): boolean {
  if (
  ...
  return MANAGED_PROXY_LIMIT_PATTERNS.some((p) => p.test(message));
}
