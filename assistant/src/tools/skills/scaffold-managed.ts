...
import { enqueueSkillUpdateReceiptTick } from "../../plugins/defaults/memory/skill-update-receipt-job.js";
import { recordSkillUpdate } from "../../plugins/defaults/memory/skill-update-receipt-store.js";
 ...
 * within minutes, so the rewrites accumulate on one receipt
 * (`plugins/defaults/memory/skill-update-receipt-store.ts`) and the tick job
 * announces the whole burst once it has settled, rather than one
 * notification per rewrite.
 *
