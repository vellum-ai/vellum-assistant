...
describe("executeBrowserStatus", () => {
  beforeEach(() => {
    // Seed the cdp-inspect backend enabled for real; the status path reads
    // `hostBrowser.cdpInspect.enabled` when reporting mode diagnostics.
    setConfig("hostBrowser", { cdpInspect: { enabled: true } });
