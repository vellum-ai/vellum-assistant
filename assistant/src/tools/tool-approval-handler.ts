import {
  ...
  peekContactToolGrantForInvocation,
...
/**
 * A contact_tool grant covers sandbox bash for a trusted contact
 * when the classified risk is not high. High-risk bash and host execution
 * still escalate.
 */
function isStandingContactBashGrantHonored(params: {
  toolName: string;
  executionTarget: ExecutionTarget;
...
export class ToolApprovalHandler {
  ...
  async checkPreExecutionGates(
    ...
    if (needsGrantConsumption && deferredConsumeParams) {
      if (
        context.requesterExternalUserId &&
        isStandingContactBashGrantHonored({
          toolName: name,
          executionTarget,
          trustClass: context.trustClass,
          riskLevel,
        ...
        const standing = peekContactToolGrantForInvocation({
          toolName: name,
          requesterExternalUserId: context.requesterExternalUserId,
        });
        if (standing) {
          log.info(
            ...
            "Standing contact_tool grant peeked - allowing trusted contact workspace command",
          );
