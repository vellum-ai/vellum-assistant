...
export class ToolApprovalHandler {
  ...
  async checkPreExecutionGates(
    ...
    // Peer assistant trust class: deny all tool execution. The trust
    // classification infrastructure (memory gate, history filter) provides
    // the security boundary — no scope system needed.
    if (context.guardianTrustClass === 'peer_assistant') {
      const reason = `Permission denied for "${name}": peer assistant actors have no tool execution capabilities.`;
      log.warn({
        toolName: name,
        sessionId: context.sessionId,
        conversationId: context.conversationId,
