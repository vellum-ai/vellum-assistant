...
export async function executeWatcherCreate(
  ...
  if (!providerId || typeof providerId !== "string") {
    return {
      content: "Error: provider is required and must be a string",
      isError: true,
    };
  }
  if (!actionPrompt || typeof actionPrompt !== "string") {
    return {
      content: "Error: action_prompt is required and must be a string",
      isError: true,
    };
  }
  ...
  if (!provider) {
    const available =
      listWatcherProviders()
        ...
        .join(", ") || "none";
    return {
      content: `Error: Unknown provider "${providerId}". Available: ${available}`,
      isError: true,
    };
  }
  ...
  const credentialService =
    (input.credential_service as string) ?? provider.requiredCredentialService;
