...
export async function probeInferenceProfile(
  ...
  // composition dispatch consumes, with the mix arm expanded), so a mix
  // cannot route the probe onto a managed or platform-billed path the stored
  // entry hid. Same seed as the dispatch below, so the judged arm is the
  // dispatched arm.
  ...
  // The profile's provider value names the row: an entry name directly, a
  // bare vendor value through that vendor's same-named default row.
  ...
  // A profile can name a row whose auth is platform-billed; the row's auth is
  // the billing fact, so it gates the probe too.
