...
describe("opencode host normalization", () => {
  test("create stores an opencode.ai openai-compatible row as provider opencode", () => {
    const db = bootDb();
    const result = createConnection(db, {
      name: "opencode-go",
      provider: "openai-compatible",
    ...
    expect(result.ok).toBe(true);
    expect(getConnection(db, "opencode-go")?.provider).toBe("opencode");
    expect(getConnection(db, "opencode-go")?.baseUrl).toBe(
      "https://opencode.ai/zen/go/v1",
    );
  });

  test("update repointing base_url at opencode.ai restamps the provider", () => {
    const db = bootDb();
    createConnection(db, {
      name: "my-proxy",
      provider: "openai-compatible",
      auth: { type: "api_key", credential: "cred" },
      baseUrl: "http://localhost:8080/v1",
      models: [{ id: "m" }],
    });
    expect(getConnection(db, "my-proxy")?.provider).toBe("openai-compatible");

    const result = updateConnection(db, "my-proxy", {
      auth: { type: "api_key", credential: "cred" },
      baseUrl: "https://opencode.ai/zen/v1",
    });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.connection.provider).toBe("opencode");
    }
    expect(getConnection(db, "my-proxy")?.provider).toBe("opencode");
  });

  test("non-opencode hosts and unparseable urls keep openai-compatible", () => {
    const db = bootDb();
    for (const [name, baseUrl] of [
      ["lookalike", "https://opencode.ai.evil.example/v1"],
      ["garbage", "not a url"],
    ] as const) {
      createConnection(db, {
        name,
      ...
      expect(getConnection(db, name)?.provider).toBe("openai-compatible");
    }
  });
});
