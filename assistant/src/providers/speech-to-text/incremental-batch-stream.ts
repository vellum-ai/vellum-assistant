/**
 * Incremental-batch streaming STT core.
 *
 * For providers without a native streaming endpoint, approximates streaming
 * by accumulating audio chunks and periodically submitting the accumulated
 * buffer to a batch transcription endpoint, then diffing the response
 * against the previous transcript to emit stable partial updates.
 *
 * Key design decisions:
 * - **Throttled polling**: A minimum interval between batch requests
 *   prevents excessive API calls while the user is speaking.
 ...
 *   `audio/pcm`, accumulated PCM chunks are wrapped in a WAV container via
 *   `encodePcm16LeToWav`, since batch endpoints require a supported
 *   container format.
 *
 ...
 * incremental-batch is not true token streaming, but a tighter poll cadence
 * reduces perceived latency for partial updates.
 */
...
export interface IncrementalBatchStreamOptions {
...
/**
 * Reusable core of the incremental-batch streaming strategy: accumulation,
 * throttled polling, forward-progress diffing, deterministic final, and
 * PCM-to-WAV wrapping. Providers without a native streaming endpoint extend
 * this and implement only {@link runBatchTranscription}.
 */
export abstract class IncrementalBatchStreamingTranscriber implements StreamingTranscriber {
  abstract readonly providerId: SttProviderId;
  ...
  protected constructor(options: IncrementalBatchStreamOptions = {}) {
  ...
  async start(onEvent: (event: SttStreamServerEvent) => void): Promise<void> {
    if (this.started) {
      throw new Error(`${this.providerId} streaming: already started`);
    }
    this.onEvent = onEvent;
    this.started = true;

    log.info(
      { provider: this.providerId, pollIntervalMs: this.pollIntervalMs },
      "Incremental-batch streaming session started",
    );
  }
  ...
  sendAudio(audio: Buffer, mimeType: string): void {
    if (!this.started) {
      throw new Error(
        `${this.providerId} streaming: sendAudio called before start()`,
      );
    }
    if (this.stopped) {
      return;
  ...
  stop(): void {
    ...
    log.info({ provider: this.providerId }, "Stopping streaming session");
  ...
  private async doPoll(): Promise<void> {
    if (this.stopped || this.polling || !this.audioDirty) {
      return;
    }
  ...
  private async emitFinal(): Promise<void> {
    ...
    try {
      ...
      log.info({ provider: this.providerId }, "Streaming session closed");
  ...
  /**
   * Provider hook: transcribe one complete audio buffer. Called with the
   * full accumulated (and, for PCM, WAV-wrapped) audio on every poll and
   * on the final flush.
   */
  protected abstract runBatchTranscription(
    audio: Buffer,
    mimeType: string,
    signal: AbortSignal,
  ): Promise<string>;
   ...
   * WAV container before submission since batch endpoints require a
  ...
  private async transcribeAccumulated(timeoutMs: number): Promise<string> {
    ...
    return this.runBatchTranscription(
      audio,
      mimeType,
  ...
  private emit(event: SttStreamServerEvent): void {
    ...
    try {
      ...
      log.warn(
        { error: err, provider: this.providerId },
        "Listener error in incremental-batch streaming adapter",
      );
    }
  }
}
