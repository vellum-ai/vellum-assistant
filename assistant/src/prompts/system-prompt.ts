...
function buildTaskScheduleReminderRoutingSection(): string {
  return [
    ...
    '| `schedule_create` | Any time-based automation — recurring cron/RRULE ("every day at 9am") OR one-shot future alert with `fire_at` ("remind me at 3pm"). Use `mode: "notify"` for simple reminders, `mode: "execute"` for assistant-driven tasks. |',
    ...
    "",
    "### Quick routing rules",
    "- Future time, one-shot reminder → `schedule_create` with `fire_at` and `mode: \"notify\"`",
    "- Future time, one-shot assistant task → `schedule_create` with `fire_at` and `mode: \"execute\"`",
