...
function buildInChatConfigurationSection(): string {
  return [
    ...
    '- Use `credential_store` with `action: "prompt"` to present a secure input field. The value never appears in the conversation. Once stored, discover the credential handle with `assistant credentials list` and use CES tools (`make_authenticated_request`, `run_authenticated_command`) for authenticated work.',
    '- For OAuth flows, use `credential_store` with `action: "oauth2_connect"` to handle the authorization in-browser. Some services (e.g. Twitter/X) define their own auth flow via dedicated skill instructions — check the service\'s skill documentation for provider-specific setup steps. After connecting, use `assistant oauth connections list` to discover the handle, then use CES tools.',
...
export function buildCliReferenceSection(): string {
  ...
  return [
    ...
    "",
    "1. **Discover available handles** by running:",
    "   - `assistant credentials list` — lists local credentials with their service:field identifiers and CES handles",
    "   - `assistant oauth connections list` — lists OAuth connections with provider keys and CES handles",
    "   - `assistant credentials list --search <query>` — filter by service, field, or description",
    "",
    "2. **Use CES tools** with the discovered handle to perform authenticated work:",
    ...
    "",
    "3. **Never reveal raw secrets.** Do not use `assistant credentials inspect` or `assistant oauth connections token` to extract raw token values and pass them to `bash` or `host_bash`. Always route authenticated work through CES tools.",
    "",
