...
/**
 * Why a (provider, model) pair cannot be vouched for, or null. The same reach
 * checks dispatch applies: routing identities against their routing table,
 * entry names against their row's dispatchable kind, vendor ids against the
 * catalog, with the entry row's advertised model list authoritative for
 * models the code-owned catalog doesn't know (a custom endpoint declares its
 * own models at connection-create time).
 */
...
function validateModel(
  ...
  const issue = modelReachIssue(provider, model);
  if (!issue) {
    return [];
  }
...
function profileConfigIssue(record: Record<string, unknown>): {
  ...
  if (record.allowUnlisted !== true) {
    const reach = modelReachIssue(provider, model);
    if (reach) {
...
async function handleCreateProfile({ body = {} }: RouteHandlerArgs) {
  ...
  assertSaneMaxTokens(provider, input.model, input.maxTokens);
