...
describe("plugin routes", () => {
  ...
  test("executes the handler as the plugin that owns the namespace", async () => {
    // Plugin-scoped host APIs (storeCredential, resolveCredential,
    // indexDocument) read the execution context to decide what the caller may
    // touch. A route handler that ran outside it would fall through to their
    // unscoped branch and reach every plugin's credentials.
    writePluginHandler(
      "my-plugin",
      "whoami.ts",
