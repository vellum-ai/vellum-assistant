 ...
 * scopes while `auth.test` still succeeds. One install produced 2 of the 18
 * scopes requested at the time, and the first real API call failed with
 * `missing_scope`. `auth.test` passing is therefore not evidence the install is
 * usable; the granted set has to be read off the `x-oauth-scopes` response
 * header and compared.
 *
