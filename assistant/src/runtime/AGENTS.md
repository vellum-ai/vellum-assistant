...
- **Interrupt if busy, queue otherwise**: When the conversation is processing, a user's own send interrupts the running turn and takes its place (see below). Every send that may not interrupt (machine sends, another actor's send, a lock held by something that is not an abortable turn) is queued and processed when the current agent turn completes. No 409 rejections.
...
Queueing remains the fallback, not a legacy path, and every non-`released` outcome takes it: a hidden machine send, an actor who does not own the running turn, a processing flag held with no abortable turn behind it, a turn that never releases inside the abort budget, a turn still finalizing past the commit budget, another waiter that took the lock first, and a repair that could not be persisted. A message is never dropped in favour of an interrupt. Machine-injected sends (subagent and ACP notifications, subagent guidance, wakes, plugin-api turns) enqueue directly and never interrupt, and the drain coalesces consecutive queued messages into one batched turn (`buildPassthroughBatch` / `drainBatch`), so a burst of machine updates runs as one turn instead of each cutting off the last. Nothing cuts a running turn short at a tool checkpoint to let the queue in: queued messages run on the end-of-turn drain. There is no client route to delete or reorder a queued message; a queued message leaves the queue by draining, by answering or superseding a parked `ask_question` (`supersedePendingInteractionsOnEnqueue`), or by a teardown discard.
