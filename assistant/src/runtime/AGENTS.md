...
**A user interrupt ends the turn, not the queue.** `POST /v1/conversations/:id/cancel` (Stop / Esc, and the CLI's cancel signal file) aborts the turn in flight; anything the same user queued behind it survives and runs on that turn's drain — the aborted loop reaches its `finally`, which calls `kickDrainQueue` like any other turn end. `abortConversation` only discards the queue for abort kinds where the conversation itself is going away (dispose, eviction, voice supersession, subagent teardown), and every discarded row gets `message_queued_deleted` alongside `generation_cancelled`. Do NOT "simplify" this back to an unconditional `queue.clear()`: a cleared queue emits no per-row terminal event, so clients keep rendering a queued row whose cancel/steer buttons are dead (DELETE 404s, steer reports `not_processing`) and whose message is silently lost on the next reload.
