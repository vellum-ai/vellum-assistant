...
A persistent stream's breadcrumb records what the message holds as well as its id: `progress` while a plan opened it and no reply text has been confirmed in it, `reply` once some has. Recovery finishes only a `reply` in place (`streamedReplyTsToReconcile`); a plan-only card is left as the card and the reply is posted beneath it, because rewriting the card destroys the plan the reader was following. The role advances only after the channel confirms the text, so the one window left is a crash between that confirmation and the role write, which posts the reply beside a message already showing part of it rather than overwriting the card. Slack documents no idempotency key for posts and no read-back shape for an unfinished stream, so nothing closer to exactly-once is claimed here.
