...
- **Authenticated (remote)**: 300 requests/minute — proxied non-loopback clients, keyed by the forwarded client IP. Configurable via `apiRateLimit.authenticatedMaxRequestsPerMinute` (positive integer, defaults to 300 when unset); read per-check so a config edit applies without a restart. The loopback and unauthenticated budgets are fixed and unaffected by this setting.
