...
- `GET /v1/plugins` and `GET /v1/plugins/search` serve the cached plugin catalog (or the bundled manifest when nothing is cached) and start a single-flight, TTL-gated platform refresh in the background. A refresh that changes what the catalog serves publishes `sync_changed` `plugins:list`, so clients refetch both lists.
