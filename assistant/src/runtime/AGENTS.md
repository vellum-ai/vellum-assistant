...
### Channel inbound honors "queue if busy"

Channel inbound turns (Slack/Telegram/etc. — `processChannelMessageInBackground`) obey the same "process when the current turn completes" contract.

**Telegram follow-ups while the conversation is mid-turn** go through `conversation.enqueueMessage` (same in-memory queue as `POST /v1/messages`). They appear in the web queued-messages UI (`message_queued`, list-messages snapshot) and drain as a passthrough batch. Drain is responsible for callback delivery: `completeQueuedChannelFollowUps` in `runtime/queued-channel-followup.ts` calls `finalizeEventDelivery` once per shared Telegram callback URL, then marks sibling inbound events processed/delivered. Deleting a queued Telegram row from the web client `markProcessed`s its inbound event so the retry sweep does not replay it.

**Slack (and idle Telegram)** still defer via `withChannelTurnAdmission` (`routes/inbound-stages/channel-turn-admission.ts`: per-conversation single-flight for FIFO ordering + event-driven `waitForIdle`), then run with live-ingress delivery orchestration intact. Slack stays on admission because drain historically had no Slack streaming session.

A channel turn that still hits `CONVERSATION_BUSY_MESSAGE` (a non-channel turn raced in after admission, or a Telegram follow-up rejected as `queue_full`) is re-scheduled for the channel-retry sweep via `deferRetryUntilIdle` — never `recordProcessingFailure` (which `classifyError` treats as fatal → dead-letter → silent drop, JARVIS-1346). The sweep is itself busy-aware: it `deferRetryUntilIdle`s a retry whose conversation is mid-turn. Busy deferral must **not** burn the retry budget: `deferRetryUntilIdle` pushes `retryAfter` forward but never increments `processingAttempts` and never dead-letters, so a conversation that stays busy across many sweeps re-defers indefinitely rather than dropping the reply at `RETRY_MAX_ATTEMPTS` (~10 min). Crash durability: a queued Telegram follow-up keeps its inbound row `pending`; a crash drops the in-memory queue and `recoverOrphanedChannelEvents` (`monitoring/recovery/`) promotes boot-fenced orphan `pending` rows onto the sweep's `failed` retry path. Admission-wait Slack/Telegram first turns are the same: the inbound row sits `pending` until the deferred turn runs.
