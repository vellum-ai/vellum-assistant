...
- **Interrupt if busy**: A message sent while the conversation is processing stops the running turn and takes its place. No 409 rejections, no queue. See below.
...
**Interrupt if busy.** A message sent while the conversation is processing stops the running turn and takes its place. `submitUserTurn` (`daemon/conversation-submit.ts`) makes that decision for every ingress that carries a user's send: the HTTP send route, the CLI signal, and surface action clicks. A busy conversation is handed to `interruptRunningTurn` (`daemon/conversation-interrupt.ts`), which aborts the turn with `preempted_by_new_message`, waits for it to release the processing lock AND for its turn-boundary commit to finish, repairs the abandoned `tool_use` blocks with a persisted synthetic `tool_result`, and answers `released`. The send then runs the code an idle conversation already uses, so there is one turn dispatch rather than a second one for interrupts.

**Defer if it cannot interrupt.** A hidden machine send, another actor's send, a processing flag held by something that is not an abortable agent turn (`/compact`, `/clean`, a wake, a canned reply), or an interrupt that could not hand over (abort budget, finalization budget, lost lock, non-durable repair) is registered with `runWhenConversationIdle` (`daemon/conversation-admission.ts`) and runs when the lock frees, FIFO, behind the turn-boundary commit barrier. Machine-injected sends (subagent and ACP notifications, plugin-api turns, channel inbound) use the same primitive directly. There is no other holding place for a message: a send is never dropped in favour of an interrupt. A conversation may hold at most `MAX_PENDING_ADMISSIONS` waiting sends; past that the send is refused, as `429 { accepted: false, error: "busy" }` before acceptance and as a `SEND_FAILED` event after it.

**The handover runs off the response, not on it.** Fire-and-forget is not suspended for an interrupt. The eligibility decision is synchronous (`classifyInterruptEligibility`), so the route answers `202 { accepted, requestId }` the moment it accepts the message, then runs the abort, the release wait, the commit wait, the repair, the persist and the dispatch in a detached task. Holding the request would cost the abort budget (`ABORT_RELEASE_WAIT_MS`) plus the turn-boundary commit wait, long enough for a client to time out and retry a message the daemon is still placing. That 202 carries `messageId` as well as `requestId`, and they are the same value: a user turn persists its row under its `requestId` (`persistUserMessage` passes `id: requestId`), which is what lets the id be answered before the row exists. Both are required, not decorative: `postChatMessage` rejects an accepted response without a string `messageId` and the client drops the optimistic row. Clients reconcile that row on `user_message_echo`. Two rules for anything added to that task: it must not be able to drop the message (a `run` that loses the processing lock rejects with `CONVERSATION_BUSY_MESSAGE`, which `submitUserTurn` answers by deferring), and a duplicate `clientMessageId` is answered from the existing row BEFORE the abort, since the insert's own dedup settles too late to un-abort a turn.

A deferred send runs `repairInterruptedToolUseBlocks` with `requireDurable` at the head of its own run, because an interrupt that could not make its repair durable is one of the reasons a send lands there. A second persist failure fails the send rather than persisting the prompt behind an unmatched durable `tool_use`.

**The two interrupt texts are split by job, and the behavior instruction lives in exactly one of them.** A synthetic `tool_result` (`PREEMPTED_TOOL_RESULT_TEXT`) states the fact about the one call it answers: the call stopped early and may have taken effect anyway. It is repeated once per stopped call in a parallel batch, so it holds no instruction. Every completed handover arms `pendingInterruptNote`, whether or not a tool call was stopped. The persist of the interrupting user message consumes that flag exactly once: it stamps `interruptedPriorTurn` on the row and appends `INTERRUPTED_TURN_NOTE_TEXT` to the message's LLM-facing content only, so the persisted row stays what the user typed and no client renders the note. That note is the single place the behavior after an interrupt is written, it leads with the short acknowledgement the user is waiting for, and it sits after the user's words instead of before them. `reinjectInterruptTurnNote` rebuilds the identical block at history load, next to the attachment annotations and for the same reason: a reload that rebuilds it differently breaks provider prefix-cache parity. Do not add a third annotation, and do not put instructions back in the tool result.
...
**Stop is a message.** `POST /v1/conversations/:id/cancel` (the CLI's cancel signal file, and programmatic callers) aborts the turn in flight and nothing else. The web has no Stop control: a send while the assistant is working is what stops it.

### Channel inbound defers until idle

Channel inbound turns (Slack/Telegram/etc., `processChannelMessageInBackground`) never interrupt: a channel turn delivers its reply back through the provider callback URL (streaming session + `finalizeEventDelivery` + processed/delivery bookkeeping), so it is **deferred until the processing lock frees** (`runWhenConversationIdle` in `daemon/conversation-admission.ts`: per-conversation single-flight for FIFO ordering + event-driven `waitForIdle`), then run with its delivery orchestration intact. That module is the one deferral primitive every machine-injected send shares (subagent and ACP notifications, plugin-api turns, channel inbound).

**Invariant:** a channel turn's delivery orchestration is part of the run registered with `runWhenConversationIdle`; a path that runs the turn without it would produce a reply that never reaches the channel. A channel turn that still hits `CONVERSATION_BUSY_MESSAGE` (a non-channel turn raced in after admission) is re-scheduled for the channel-retry sweep via `deferRetryUntilIdle` — never `recordProcessingFailure` (which `classifyError` treats as fatal → dead-letter → silent drop, JARVIS-1346). The sweep is itself busy-aware: it `deferRetryUntilIdle`s a retry whose conversation is mid-turn. Busy deferral must **not** burn the retry budget: `deferRetryUntilIdle` pushes `retryAfter` forward but never increments `processingAttempts` and never dead-letters, so a conversation that stays busy across many sweeps re-defers indefinitely rather than dropping the reply at `RETRY_MAX_ATTEMPTS` (~10 min). Crash durability: while the in-memory admission waits, the inbound row sits `pending`; a crash mid-wait is recovered at startup by `recoverOrphanedChannelEvents` (`monitoring/recovery/`), which promotes boot-fenced orphan `pending` rows onto the sweep's `failed` retry path.
...
Nothing in the turn layer wires the proxy: it reads the roster on every send, and the conversation's own event sink is fixed for its life (see `assistant/AGENTS.md`, "Conversation event delivery and turn presence"), so a deferred turn reaches the extension exactly like a live one.
