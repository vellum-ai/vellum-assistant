...
`POST /v1/shared/conversations/:id/messages` is not a second ingress implementation. It is a trusted contact's scoped entry into the same pipeline: the route checks membership (404 on a miss), resolves the contact's trust on `vellum-shared` with the channel admission floor applied, and supplies a conversation acquire that never creates one, then calls `handleSendMessage` with a `ContactSender`. Queue-if-busy, `clientMessageId` dedup and every queue fallback are the pipeline's, unchanged. The `ContactSender` gates off only what a contact may not do: interrupt a running turn (a contact's send always queues), answer or supersede the guardian's pending interactions, reach the onboarding and greeting paths, or reset the heartbeat. A contact's text is fenced as untrusted input with the raw text kept as display copy. Anything added to the send pipeline must decide whether it applies to a contact.
...
**Exempt endpoints** (`isRateLimitExemptEndpoint`): the SSE streams (`events`, `shared/events`) and liveness/readiness probes (`health`, `healthz`, `readyz`) bypass the per-minute limiter entirely. Each stream is one long-lived connection, not a burst of requests, and 429-ing it drops the stream, which drives a client reconnect + full re-bootstrap loop that generates far more load than the limiter saves. Liveness probes must always answer or the client treats the assistant as down and reconnects harder. The stream routes still enforce auth downstream, and stream memory is bounded by SSE backpressure shedding + subscriber caps (per contact for `shared/events`).
