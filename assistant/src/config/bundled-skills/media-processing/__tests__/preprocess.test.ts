...
describe('createSegments', () => {
  ...
  it('creates a shorter final segment for non-even durations', () => {
    ...
    expect(segments[2]).toEqual({ id: 'seg-003', startSeconds: 40, endSeconds: 50 });
  });
  ...
  it('creates segments across multiple live ranges', () => {
    ...
    const segments = createSegments(live, 20);
    expect(segments).toHaveLength(3);
    expect(segments[0]).toEqual({ id: 'seg-001', startSeconds: 0, endSeconds: 20 });
    expect(segments[1]).toEqual({ id: 'seg-002', startSeconds: 20, endSeconds: 30 });
    expect(segments[2]).toEqual({ id: 'seg-003', startSeconds: 60, endSeconds: 80 });
  });
