...
import { parseContactFile, bulkEnroll } from '../../../../sequence/importer.js';
...
export async function run(input: Record<string, unknown>, _context: ToolContext): Promise<ToolExecutionResult> {
  ...
  try {
    ...
    if (!autoEnroll) {
      ...
      if (parsed.contacts.length > 5) lines.push(`  ... and ${parsed.contacts.length - 5} more`);
      lines.push('');
      lines.push(`Call again with auto_enroll=true to enroll all ${parsed.contacts.length} contacts in "${seq.name}".`);
    ...
    if (parsed.errors.length > 0) {
      lines.push('');
      lines.push(`${parsed.errors.length} row(s) in file had errors (not enrolled).`);
    }
