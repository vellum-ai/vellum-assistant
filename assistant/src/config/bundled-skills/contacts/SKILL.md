...
Manage the user's contacts, relationship graph, access control (trusted contacts), and invite links. This skill covers contact CRUD with multi-channel tracking, controlling who can message the assistant through external channels (Telegram, voice, email, WhatsApp, Slack), and creating/managing invite links that grant access. Use Vellum CLI for read operations where available, and use gateway control-plane `curl` calls for mutating actions.
...
Trusted contacts control who is allowed to send messages to the assistant through external channels like Telegram, voice (phone calls), email, WhatsApp, and Slack.
...
- **Channel type**: The messaging platform (e.g., `telegram`, `voice`, `email`, `whatsapp`, `slack`).
...
- `channels` -- array of channel entries, each with:
  ...
  - `channel` -- the channel type (e.g., `telegram`, `voice`, `email`, `whatsapp`, `slack`)
...
- `--channels` -- at least one channel entry with:
  - `type` -- the channel type (e.g., `telegram`, `voice`)
...
If the user provides a name but not an external ID, explain that you need the channel-specific user ID or chat ID to create the contact entry. For Telegram, this is a numeric user ID; for voice, this is the phone number in E.164 format.
