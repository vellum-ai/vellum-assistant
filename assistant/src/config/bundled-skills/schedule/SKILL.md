...
**Check for an existing skill before writing one.** An installed skill may already do the work the script is about to reimplement. Before authoring anything non-trivial, search the installed skills for one that covers the task rather than starting from scratch.

**If the skill says how to use it from a schedule, follow that.** Some ship a script meant to be copied into the schedule's own directory so the schedule owns it and skill upgrades leave it alone. Those instructions win over anything here.

Otherwise, run the skill's script where it is installed. `assistant skills inspect <id> --json` reports its location; resolve that inside the scheduled command rather than baking a path in when the schedule is written, since roots differ by skill source and a skill's directory moves when a workspace is restored elsewhere. Change into the directory before running the script, as skill scripts usually assume their own directory for relative paths and sibling imports. The tradeoff is that a later edit to the skill can rewrite the script underneath the schedule: running in place picks up fixes but can break, while a copy is stable but never improves.
