...
/** List calendar events using delta queries for incremental sync. */
export async function listEventsDelta(
  ...
  // If a deltaLink is provided, use it directly for continuation
  if (options?.deltaLink) {
    const deltaUrl = new URL(options.deltaLink);
    const path = deltaUrl.pathname;
    const query: Record<string, string> = {};
    deltaUrl.searchParams.forEach((v, k) => {
      query[k] = v;
    });
    return request<OutlookCalendarEventListResponse>(
      connection,
  ...
  // Initial delta query
  const query: Record<string, string> = {};
  if (options?.startDateTime) query["startDateTime"] = options.startDateTime;
  if (options?.endDateTime) query["endDateTime"] = options.endDateTime;
  if (options?.select) query["$select"] = options.select;
