...
- No audio can also mean STT or TTS credentials are missing: on media-stream the daemon performs STT and TTS itself, so a missing `services.stt.provider` or `services.tts.provider` API key will block the call (an inbound call with missing credentials plays a "setup required" message and hangs up). Verify both provider keys are configured.
...
- This often means the media-stream connection or TTS voice configuration failed after the TwiML fetch
