...
**Note on STT configuration:** Telephony STT is driven by `services.stt.provider`. The `calls.voice.transcriptionProvider` field is a legacy config key retained for schema compatibility but no longer participates in call setup routing.
