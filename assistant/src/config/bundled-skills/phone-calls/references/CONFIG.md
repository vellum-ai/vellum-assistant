...
| `services.stt.provider`                     | STT provider for transcription and telephony. All phone calls stream raw audio to the daemon over the Twilio media-stream and the daemon transcribes it with this provider (realtime streaming for providers that support it, e.g. Deepgram/xAI; batch otherwise). A working API key for this provider is required for calls to connect. | `deepgram`                                                                                               |
...
During phone calls the daemon synthesizes all TTS audio server-side and streams it to the caller over the Twilio media-stream (PCM is transcoded to 8 kHz mu-law). Every provider therefore requires a working API key — there is no Twilio-side synthesis:

| Provider     | Description                                                                                                                                                            |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `elevenlabs` | The daemon synthesizes audio server-side via ElevenLabs' HTTP API (PCM output) and streams it to the caller over the media-stream.                                     |
| `fish-audio` | The daemon synthesizes audio server-side via Fish Audio's HTTP API and streams it to the caller over the media-stream. Requires a `referenceId`.                       |
| `deepgram`   | The daemon synthesizes audio server-side via Deepgram's HTTP API and streams it to the caller over the media-stream. Uses the same API key as Deepgram speech-to-text. |
| `xai`        | The daemon synthesizes audio server-side via xAI's HTTP API and streams it to the caller over the media-stream.                                                        |
