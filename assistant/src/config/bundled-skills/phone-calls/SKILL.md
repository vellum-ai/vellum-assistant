...
3. **Automatic execution**: If the guardian chooses to call back or send a message, the system resolves the original caller's phone number from the call record and executes the action automatically.
...
The follow-up flow works across all guardian channels. The system handles cross-channel routing transparently.
