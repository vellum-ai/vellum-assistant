...
import { VoiceConfigSchema, VoiceFrontModelConfigSchema } from "../voice.js";
...
describe("VoiceFrontModelConfigSchema", () => {
  test("empty object parses to defaults", () => {
    const parsed = VoiceFrontModelConfigSchema.parse({});
  ...
  test("rejects non-positive endpointDecisionTimeoutMs", () => {
    const result = VoiceFrontModelConfigSchema.safeParse({
  ...
  test("rejects a heartbeat ceiling shorter than the idle tick interval", () => {
    ...
    if (!result.success) {
      ...
      expect(
        msgs.some((msg) =>
          msg.includes("voice.frontModel.progress.maxSilenceMs"),
        ),
      ).toBe(true);
    }
    // The floor itself is allowed: the tick is exactly on the ceiling.
    expect(
      VoiceFrontModelConfigSchema.safeParse({
  ...
  test("rejects a non-boolean progress.enabled", () => {
    ...
    if (!result.success) {
      ...
      expect(
        msgs.some((m) => m.includes("voice.frontModel.progress.enabled")),
      ).toBe(true);
    }
  });
});
...
describe("VoiceConfigSchema", () => {
  test("empty object parses to front-model defaults", () => {
    expect(VoiceConfigSchema.parse({})).toEqual({
      frontModel: FRONT_MODEL_DEFAULTS,
    });
  });

  test("partial frontModel overrides merge with defaults", () => {
    ...
    expect(parsed.frontModel.progress).toEqual(PROGRESS_DEFAULTS);
  });
});
