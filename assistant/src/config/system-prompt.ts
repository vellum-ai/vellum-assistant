...
function buildDynamicUiSection(): string {
  return [
    ...
    '',
    '### Loading app tools',
    'The following app tools are provided by the `app-builder` skill: `app_create`, `app_update`, `app_file_edit`, `app_file_write`, `app_file_read`, `app_file_list`, `app_delete`, `app_list`, `app_query`. Before using any of these, call `skill_load` with `id: "app-builder"` to load them. You only need to load the skill once per session. Note: `app_open` is always available as a core tool — no skill_load needed.',
    '',
    '### App type selection',
