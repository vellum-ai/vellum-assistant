...
function buildBrowserInteractionSection(): string {
  return [
    ...
    '3. **FIRST: check for modals/overlays** — scan the snapshot for modal dismiss buttons ("Got It", "Accept", "Close", "X", "OK", "Dismiss") and click them BEFORE doing anything else. Modals block all other interactions.',
    '4. Use `browser_click` / `browser_type` / `browser_press_key` with `element_id` from the snapshot',
    '5. After any action that changes the page (click, navigation, form submit), take a new `browser_snapshot` before the next interaction',
    '',
