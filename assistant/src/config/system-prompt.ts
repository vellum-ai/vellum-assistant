...
function buildConfigSection(): string {
  ...
  return [
    ...
    '- `onboarding/playbooks/*.md` — Channel onboarding playbooks (for example `desktop_onboarding.md`, `telegram_onboarding.md`).',
    '- `onboarding/playbooks/registry.json` — Tracks which channels have started onboarding.',
    ...
    '',
    '**onboarding/playbooks/*.md** — update when onboarding progresses:',
    ...
    '- Permission asks for voice/computer-control during onboarding should be user-initiated (explicit user ask or Home Base optional-task action IDs)',
    '',
