...
function buildInChatConfigurationSection(): string {
  return [
    ...
    '- For OAuth flows, use `credential_store` with `action: "oauth2_connect"` to handle the authorization in-browser. Most OAuth services (including Twitter/X) use the generic `oauth_connect_start` IPC path. Check the service\'s skill documentation for any provider-specific setup steps.',
