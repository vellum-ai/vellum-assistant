...
- **A completed repair settles the item, the way a decided request does.** The client marks the item `acted_on`, which retires the button, drops the callout, and leaves a receipt in its place. The remediation stays on the item as the record of what fixed it, so anything asking "does this still want me?" reads the item's status, never the presence of a remediation. A reader returning later sees the receipt, not the button they already pressed.
- **Copy names the reader's experience, not the mechanism.** The producer authors both the label and the body, so internal vocabulary reaching a surface is a producer bug, not a rendering one. "Vellum-managed models paused" is the condition; "the managed inference credential was rejected" is our word for its cause. Reuse the vocabulary the product already shows: the profile row says "Managed by Vellum" and the provider row says "Runs on Vellum's managed credentials", so a notification about the same thing says it the same way.
