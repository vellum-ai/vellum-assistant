...
// the plugin owns (`field` or `service` matches its manifest name); outside
// any plugin it is unscoped. Throws CredentialResolutionError when the ref
// does not resolve, the store is unreachable, or the credential is out of the
// plugin's scope.
export {
...
// "service/field" reference. A plugin may only write credentials it owns
// (`field` or `service` matches its manifest name), and the write fails closed
// with no plugin in context. Throws CredentialStoreError when there is no
// calling plugin, the ref is malformed, the value is invalid, the store
// rejects the write, or the credential is out of the plugin's scope.
export type {
