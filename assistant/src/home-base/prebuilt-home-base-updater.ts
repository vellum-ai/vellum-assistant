...
export function isPrebuiltHomeBaseApp(
  app: Pick<AppDefinition, "name" | "description" | "htmlDefinition">,
): boolean {
...
export function validatePrebuiltHomeBaseHtml(html: string): {
  valid: boolean;
  ...
  for (const anchor of HOME_BASE_REQUIRED_ANCHORS) {
    const hasId =
      html.includes(`id="${anchor}"`) || html.includes(`id='${anchor}'`);
    if (!hasId) {
