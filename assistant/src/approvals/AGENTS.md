...
A redeemed guardian code takes a channel from its current guardian only when the session records the guardian's consent to replace that guardian (`replacesGuardianAddress`). The gateway decides that record at mint from the caller's `replaceGuardian`, and carries it onto a resend or a deep-link handoff from the session being continued; nothing infers it from the shape of a session. Text-channel redemption binds through `applyRedeemedGuardianBinding` (`gateway/src/verification/guardian-redemption.ts`), and a new redemption path does the same instead of deciding a conflict of its own. Transitional: the two phone paths (the outbound binding in `gateway/src/verification/session-service.ts` and `gateway/src/http/routes/twilio-voice-verify-callback.ts`) still carry their own conflict rule until they move onto it. See "Replacing a Channel's Guardian" in `gateway/ARCHITECTURE.md`.
