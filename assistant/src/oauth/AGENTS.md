...
The minted base is that path prefix, not a host root. Stock clients resolve a leading `/` against the origin (`/upload/gmail/...`, `/v1/charges`), which would otherwise miss the daemon catch-all. The gateway's runtime-proxy remaps those host-absolute paths onto `/v1/oauth/proxy/<segment>` when the bearer is an `oauth_proxy_v1` grant (`gateway/src/http/routes/oauth-proxy-path.ts`). An already-prefixed path, including one naming a different provider, is left alone so the daemon's subject check still applies. Traversal and `//host` stay in the remainder for `normalizeProxyPath` to reject.
