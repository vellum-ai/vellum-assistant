 ...
 */
export function canReplaceTitle(conversation: {
  title: string | null;
  isAutoTitle: number;
}): boolean {
  return (
...
export async function generateAndPersistConversationTitle(
  ...
  if (title) {
    ...
    // Whatever the configuration was, it serves this call site now — stop
...
function logTitleGenerationFailure(
  ...
  if (reason === "model_unavailable") {
    ...
    recordTitleModelFault({
      model: asOptionalString(fields.model),
      provider: asOptionalString(fields.provider),
      connectionName: asOptionalString(fields.connectionName),
    });
    return;
  }
...
/** Narrow a log field back to a string, dropping anything else. */
