...
/**
 * Title was named explicitly by a person or caller: a rename, or a title
 * supplied when the conversation was created. The automatic titler never
 * replaces it.
 */
export const TITLE_USER_SET = 0;
...
/**
 * Whether the conversation's title was named explicitly. Every automatic
 * title write checks this first, so an explicit name is kept even when its
 * text matches a placeholder such as "New Conversation".
 */
export function isUserSetTitle(conversation: { isAutoTitle: number }): boolean {
  return conversation.isAutoTitle === TITLE_USER_SET;
}
 ...
 * (`AUTO_TITLE_DETERMINISTIC`); false for user-set titles and titles
...
export function applyDeterministicTitleIfReplaceable(
  ...
  if (
    conversation &&
    (isUserSetTitle(conversation) || !isReplaceableTitle(conversation.title))
  ) {
...
export async function regenerateConversationTitle(
  ...
  const conversation = getConversation(conversationId);
  if (!conversation || isUserSetTitle(conversation)) {
    return { title: conversation?.title ?? UNTITLED_FALLBACK, updated: false };
  }
  ...
  if (title) {
    // Re-check before persisting (race guard against manual rename)
    const current = getConversation(conversationId);
    if (
      !current ||
      isUserSetTitle(current) ||
