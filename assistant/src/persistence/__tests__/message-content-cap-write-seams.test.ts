...
describe("persisted message size cap at the write seams", () => {
  test("an inserted 50 MB message is stored trimmed, not dropped", async () => {
    /**
  ...
  test("an in-place content update cannot grow a row past the cap", async () => {
    ...
    expect(messageContentBytes(storedContent(message.id))).toBeLessThanOrEqual(
      MAX_PERSISTED_MESSAGE_BYTES,
    );
  });

  test("finalizing a streamed message caps the content it folds inline", async () => {
    /**
