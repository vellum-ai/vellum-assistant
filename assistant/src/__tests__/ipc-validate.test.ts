...
import {
  isClientMessageEnvelope,
  validateClientMessage,
} from "../daemon/ipc-validate.js";
...
describe("IPC Validate", () => {
  describe("validateClientMessage", () => {
    ...
    test("rejects object without type field", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('missing a string "type" field');
    });
    ...
    test("accepts user_message with attachments", () => {
      const result = validateClientMessage({
        ...
        sessionId: "session-1",
        attachments: [
          { filename: "f.txt", mimeType: "text/plain", data: "abc" },
        ],
        interface: "cli",
      });
      expect(result.valid).toBe(true);
    ...
    test("rejects user_message without sessionId", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('non-empty string "sessionId"');
    });
    ...
    test("rejects session_create with non-string title", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('"title" must be a string');
    });
    ...
    test("rejects session_create with non-number maxResponseTokens", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('"maxResponseTokens" must be a number');
    });
    ...
    test("rejects confirmation_response without requestId", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('non-empty string "requestId"');
    });
    ...
    test("rejects confirmation_response with invalid decision", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('"decision" must be one of');
    });
    ...
    test("rejects secret_response with non-string value", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('"value" must be a string');
    });
    ...
    test("rejects ui_surface_action without surfaceId", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('non-empty string "surfaceId"');
    });
    ...
    test("rejects ui_surface_action without actionId", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('non-empty string "actionId"');
    });
    ...
    test("rejects add_trust_rule without toolName", () => {
      ...
      expect(result.valid).toBe(false);
      if (!result.valid)
        expect(result.reason).toContain('non-empty string "toolName"');
    });
  ...
  describe("contract parity", () => {
    ...
    test("validator rejects types not in the live contract", () => {
      const result = validateClientMessage({
        type: "fabricated_type_not_in_contract",
      });
      expect(result.valid).toBe(false);
    });
  });
