...
describe('email provider factory', () => {
  test('getActiveProviderName defaults to agentmail', async () => {
    const { getActiveProviderName } = await import('../email/providers/index.js');
...
describe('email service guardrails integration', () => {
  test('GuardrailError has correct structure', async () => {
    ...
    const err = new GuardrailError('outbound_paused', 'Send blocked', { extra: true });
...
describe('AgentMailProvider interface', () => {
  test('AgentMailProvider implements all required methods', async () => {
    const { AgentMailProvider } = await import('../email/providers/agentmail.js');
    ...
    const requiredMethods = [
      'health',
      'setupDomain', 'getDomainDnsRecords', 'verifyDomain',
      'ensureInboxes',
      'setupWebhook',
      'createDraft', 'listDrafts', 'getDraft', 'deleteDraft', 'sendDraft',
