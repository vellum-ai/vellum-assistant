...
describe("onboarding template contracts", () => {
  describe("BOOTSTRAP.md", () => {
    ...
    test("discovers personality organically instead of asking directly", () => {
      const lower = bootstrap.toLowerCase();
      // Personality step must instruct organic discovery
    ...
    test("em dash prohibition is not duplicated in bootstrap", () => {
      // The em dash instruction lives in the system prompt (buildStyleGuidelinesSection),
      // so BOOTSTRAP.md should NOT contain its own copy to avoid duplication.
      const lower = bootstrap.toLowerCase();
      expect(lower).not.toContain("em dashes");
    });
