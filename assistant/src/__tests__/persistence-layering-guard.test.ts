 ...
 * v2 state, the graph bootstrap, the conversation-key store) that the
 * persistence layer still depends on. They violate the one-way memory →
 * persistence direction and are scheduled for decoupling in a follow-up
 * (move the depended-on feature logic behind a persistence-owned seam, or
 * invert the dependency so memory injects it). Until then they are pinned
 * here so the guard fails the moment a NEW back-import is introduced.
 *
