...
describe("gateway-client managed outbound lane", () => {
  ...
  test("translates managed callback URL into managed outbound-send request", async () => {
    await deliverChannelReply(
      "https://platform.test/v1/internal/managed-gateway/outbound-send/?route_id=route-123&assistant_id=assistant-123&source_channel=voice&source_update_id=SM-inbound-123&callback_token=runtime-token",
      {
        chatId: "+15550001111",
  ...
  test("retries managed outbound send on retriable upstream responses with stable idempotency key", async () => {
    ...
    await deliverChannelReply(
      "https://platform.test/v1/internal/managed-gateway/outbound-send/?route_id=route-retry&assistant_id=assistant-retry&source_channel=voice&source_update_id=SM-retry",
      {
        chatId: "+15550002222",
  ...
  test("falls back to standard callback delivery for non-managed callback URL", async () => {
    ...
    expect(calls).toHaveLength(1);
    const call = calls[0];
    expect(call.url).toBe("https://gateway.test/deliver/telegram");
