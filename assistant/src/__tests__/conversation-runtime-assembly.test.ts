...
describe("Slack channel chronological rendering — multi-thread", () => {
  ...
  test("focus block self-resolves the live conversation's compaction watermark", async () => {
    ...
    // the injection options. `applyRuntimeInjections` must read them off the
    // live conversation so pre-watermark thread rows are filtered out of the
    // self-resolved `<active_thread>` block.
    const rows: MessageRow[] = [
      userRow({
        id: "thread-root",
        createdAt: 1700000000_000,
    ...
    // The watermark-filtered focus block keeps only the post-watermark reply.
    expect(focusBlock).not.toBeNull();
