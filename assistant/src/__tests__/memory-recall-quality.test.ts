...
function insertConversation(db: ReturnType<typeof getDb>, id: string, createdAt: number) {
...
function insertMessage(db: ReturnType<typeof getDb>, id: string, conversationId: string, role: string, text: string, createdAt: number) {
...
function insertSegment(db: ReturnType<typeof getDb>, id: string, messageId: string, conversationId: string, role: string, text: string, createdAt: number) {
  db.run(`
    INSERT INTO memory_segments (
      id, message_id, conversation_id, role, segment_index, text, token_estimate, created_at, updated_at
    ) VALUES (
      '${id}', '${messageId}', '${conversationId}', '${role}', 0, '${text.replace(/'/g, "''")}', ${Math.ceil(text.split(/\s+/).length * 1.3)}, ${createdAt}, ${createdAt}
    )
  `);
}

function insertItem(db: ReturnType<typeof getDb>, opts: {
  id: string;
  kind: string;
  subject: string;
  statement: string;
...
function insertItemSource(db: ReturnType<typeof getDb>, itemId: string, messageId: string, createdAt: number) {
...
function assertPrecisionAtK(
  ...
  if (found.length < minFound) {
    ...
    throw new Error(
      ...
      `found ${found.length}/${expectedSubstrings.length} (${(precision * 100).toFixed(0)}%), ` +
      `need at least ${minFound}. ` +
      `Missing: ${expectedSubstrings.filter((s) => !text.includes(s)).join(', ')}`,
    );
  }
}
...
describe('Memory Recall Quality', () => {
  ...
  describe('preference recall', () => {
    test('preferences are recalled when querying about user preferences', async () => {
      ...
      insertMessage(db, 'msg-pref-1', 'conv-pref', 'user', 'I prefer dark mode and concise answers', now);
      insertMessage(db, 'msg-pref-2', 'conv-pref', 'user', 'My favorite editor is Neovim', now + 1000);
      insertMessage(db, 'msg-fact-1', 'conv-pref', 'user', 'The server runs on port 3000', now + 2000);

      insertSegment(db, 'seg-pref-1', 'msg-pref-1', 'conv-pref', 'user', 'I prefer dark mode and concise answers', now);
      insertSegment(db, 'seg-pref-2', 'msg-pref-2', 'conv-pref', 'user', 'My favorite editor is Neovim', now + 1000);
      insertSegment(db, 'seg-fact-1', 'msg-fact-1', 'conv-pref', 'user', 'The server runs on port 3000', now + 2000);

      const recall = await buildMemoryRecall('what are my preferences', 'conv-pref', TEST_CONFIG);
    ...
    test('high-importance preferences outrank low-importance facts in recall', async () => {
      ...
      insertMessage(db, 'msg-hi', 'conv-rank', 'user', 'I strongly prefer TypeScript over JavaScript', now);
      insertSegment(db, 'seg-hi', 'msg-hi', 'conv-rank', 'user', 'I strongly prefer TypeScript over JavaScript', now);
      insertItem(db, {
        id: 'item-hi-pref',
        kind: 'preference',
      ...
      // Low-importance fact
      insertMessage(db, 'msg-lo', 'conv-rank', 'user', 'The default port is 8080', now + 1000);
      insertSegment(db, 'seg-lo', 'msg-lo', 'conv-rank', 'user', 'The default port is 8080', now + 1000);
      insertItem(db, {
        id: 'item-lo-fact',
        kind: 'fact',
      ...
      const recall = await buildMemoryRecall('TypeScript preference language', 'conv-rank', TEST_CONFIG);
  ...
  describe('contradiction suppression', () => {
    test('superseded memory items do not appear in recall', async () => {
      ...
      insertMessage(db, 'msg-old-pref', 'conv-contra', 'user', 'I prefer vim for editing code', now - 50_000);
      insertSegment(db, 'seg-old-pref', 'msg-old-pref', 'conv-contra', 'user', 'I prefer vim for editing code', now - 50_000);
      insertItem(db, {
        id: 'item-old-pref',
        kind: 'preference',
      ...
      insertMessage(db, 'msg-new-pref', 'conv-contra', 'user', 'I now prefer neovim with LazyVim for editing code', now);
      insertSegment(db, 'seg-new-pref', 'msg-new-pref', 'conv-contra', 'user', 'I now prefer neovim with LazyVim for editing code', now);
      insertItem(db, {
        id: 'item-new-pref',
        kind: 'preference',
        subject: 'editor preference',
      ...
      const recall = await buildMemoryRecall('editor preference', 'conv-contra', TEST_CONFIG);
    ...
    test('only active items are included in entity-based recall', async () => {
      ...
      insertMessage(db, 'msg-entity-active', 'conv-entity-status', 'user', 'We decided to use PostgreSQL as the database', now);
      insertSegment(db, 'seg-entity-active', 'msg-entity-active', 'conv-entity-status', 'user', 'We decided to use PostgreSQL as the database', now);
      insertItem(db, {
        id: 'item-active-db',
        kind: 'decision',
      ...
      insertItemSource(db, 'item-superseded-db', 'msg-entity-active', now - 100_000);

      const recall = await buildMemoryRecall('database choice decision', 'conv-entity-status', TEST_CONFIG);
  ...
  describe('stale memory suppression', () => {
    test('recently mentioned memories outrank old memories via recency scoring', async () => {
      ...
      // Recent mention
      insertMessage(db, 'msg-recent', 'conv-stale', 'user', 'We are using Bun as our runtime environment', now - 1000);
      insertSegment(db, 'seg-recent', 'msg-recent', 'conv-stale', 'user', 'We are using Bun as our runtime environment', now - 1000);

      // Old mention (same topic)
      insertMessage(db, 'msg-old', 'conv-stale', 'user', 'We are using Node as our runtime environment', oneMonthAgo);
      insertSegment(db, 'seg-old', 'msg-old', 'conv-stale', 'user', 'We are using Node as our runtime environment', oneMonthAgo);

      const recall = await buildMemoryRecall('runtime environment', 'conv-stale', TEST_CONFIG);
    ...
    test('frequently accessed items get a retrieval reinforcement boost', async () => {
      ...
      insertMessage(db, 'msg-freq', 'conv-access', 'user', 'User timezone is America/Los_Angeles', now);
      insertSegment(db, 'seg-freq', 'msg-freq', 'conv-access', 'user', 'User timezone is America/Los_Angeles', now);
      insertItem(db, {
        id: 'item-freq',
        kind: 'profile',
        subject: 'timezone',
      ...
      insertMessage(db, 'msg-rare', 'conv-access', 'user', 'User timezone offset is UTC-8', now + 1000);
      insertSegment(db, 'seg-rare', 'msg-rare', 'conv-access', 'user', 'User timezone offset is UTC-8', now + 1000);
      insertItem(db, {
        id: 'item-rare',
        kind: 'profile',
      ...
      const recall = await buildMemoryRecall('timezone', 'conv-access', TEST_CONFIG);
  ...
  describe('multi-source recall', () => {
    test('lexical and item-based results are merged into a single recall', async () => {
      ...
      insertMessage(db, 'msg-seg', 'conv-multi', 'user', 'Deploy to staging before production always', now);
      insertSegment(db, 'seg-deploy', 'msg-seg', 'conv-multi', 'user', 'Deploy to staging before production always', now);
      ...
      const recall = await buildMemoryRecall('deployment staging production', 'conv-multi', TEST_CONFIG);
    ...
    test('recall with no matching content returns empty injection', async () => {
      ...
      const recall = await buildMemoryRecall('completely unrelated xyzzy topic', 'conv-empty', TEST_CONFIG);
  ...
  describe('precision@k assertions', () => {
    ...
    test('assertPrecisionAtK fails when too few expected items are found', () => {
      ...
      expect(() => {
        assertPrecisionAtK(text, ['item-a', 'item-b', 'item-c'], 2, 'test-label');
      }).toThrow(/precision@3 too low.*found 1\/3.*need at least 2.*Missing: item-b, item-c/);
    });
    ...
    test('precision@k guard for preference recall fixture', async () => {
      ...
      const prefs = [
        { msg: 'msg-pk-1', seg: 'seg-pk-1', text: 'I prefer dark mode over light mode' },
        { msg: 'msg-pk-2', seg: 'seg-pk-2', text: 'I like using TypeScript for all projects' },
        { msg: 'msg-pk-3', seg: 'seg-pk-3', text: 'I prefer tabs over spaces for indentation' },
      ];
      ...
      const recall = await buildMemoryRecall('what do I prefer', 'conv-pk', TEST_CONFIG);
