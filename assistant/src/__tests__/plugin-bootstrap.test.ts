...
describe("plugin bootstrap", () => {
  ...
  test("startup ordering: defaults registered before the window closes survive bootstrap replay", async () => {
    ...
    expect(names).toContain("default-token-estimate");
    expect(names.indexOf("default-token-estimate")).toBeLessThan(
  ...
  test("requiresFlag disabled: plugin middleware and injectors are dropped from the registry", async () => {
    ...
    const gatedMiddleware: PipelineMiddlewareMap["tokenEstimate"] = async (
      args,
      next,
    ) => next(args);
    const plugin = buildPlugin(
      "gated-middleware",
      {
        middleware: { tokenEstimate: gatedMiddleware },
        injectors: [
          {
    ...
    // tokenEstimate middleware / injectors, so we key on identity rather than
    // asserting empty lists.
    expect(getMiddlewaresFor("tokenEstimate")).not.toContain(gatedMiddleware);
    expect(
