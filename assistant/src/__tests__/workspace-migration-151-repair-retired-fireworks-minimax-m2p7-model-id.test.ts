...
import { repairRetiredFireworksMinimaxM2p7ModelIdMigration } from "../workspace/migrations/151-repair-retired-fireworks-minimax-m2p7-model-id.js";
...
function freshWorkspace(): void {
  workspaceDir = join(
    tmpdir(),
    `vellum-migration-151-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe("151-repair-retired-fireworks-minimax-m2p7-model-id migration", () => {
  test("has correct migration id and is registered", () => {
    expect(repairRetiredFireworksMinimaxM2p7ModelIdMigration.id).toBe(
      "151-repair-retired-fireworks-minimax-m2p7-model-id",
    );
