/**
 * Unit tests for media-stream TwiML generation.
 *
 * Tests that `generateStreamTwiML` emits the `<Connect><Stream>` transport
 * with callSessionId / token encoded as path segments and `<Parameter>`
 * children, and never any ConversationRelay STT attributes.
 */
