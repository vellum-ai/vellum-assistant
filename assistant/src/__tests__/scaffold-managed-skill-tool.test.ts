...
// overwrite, captured at the store's boundary so the tests read what was
// recorded rather than standing up the memory database and the tick job.
import type * as ReceiptStore from "../plugins/defaults/memory/skill-update-receipt-store.js";

let recordedUpdates: ReceiptStore.SkillUpdateEntryInput[] = [];
let recordUpdateFails = false;
let receiptTicks = 0;
mock.module(
  "../plugins/defaults/memory/skill-update-receipt-store.js",
  (): Partial<typeof ReceiptStore> => ({
    recordSkillUpdate: (input) => {
      recordedUpdates.push(input);
      return recordUpdateFails
        ? { recorded: false, reason: "memory database unavailable" }
        : { recorded: true, receiptId: "receipt-1", inserted: true };
    },
  }),
);
mock.module("../plugins/defaults/memory/skill-update-receipt-job.js", () => ({
...
describe("background skill update notification", () => {
  ...
  test("a background overwrite of an existing skill records a receipt entry keyed to the tool call", async () => {
    ...
    expect(result.isError).toBe(false);
    // Nothing is announced here: the entry joins the open receipt, and the
    // tick job announces the burst once it has settled.
