...
function makeMockStore(overrides: Partial<AppStore> = {}): AppStore {
  return {
    ...
    writeAppFile: () => {},
    editAppFile: () =>
      ({ ok: true, updatedContent: 'new', matchCount: 1, matchMethod: 'exact', similarity: 1, actualOld: 'old', actualNew: 'new' }) as EditEngineResult,
    ...overrides,
  };
}
...
describe('executeAppList', () => {
  test('returns mapped list of apps', () => {
    const store = makeMockStore({
      listApps: () => [
        makeApp({ id: 'a1', name: 'First', description: 'desc1', updatedAt: 100 }),
    ...
    expect(parsed[0]).toEqual({ id: 'a1', name: 'First', description: 'desc1', updatedAt: 100 });
...
describe('executeAppDelete', () => {
  test('deletes the app and returns confirmation', () => {
    ...
    expect(result.isError).toBe(false);
    expect(JSON.parse(result.content)).toEqual({ deleted: true, appId: 'app-1' });
  ...
  test('passes replace_all through to store', () => {
    ...
    const store = makeMockStore({
      editAppFile: (_appId, _path, _old, _new, replaceAll) => {
        ...
        return { ok: true, updatedContent: '', matchCount: 1, matchMethod: 'exact' as const, similarity: 1, actualOld: '', actualNew: '' };
      },
    });
    executeAppFileEdit(
      {
