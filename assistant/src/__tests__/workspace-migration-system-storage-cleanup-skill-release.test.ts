...
import { releaseNotesSystemStorageCleanupSkillMigration } from "../workspace/migrations/069-release-notes-system-storage-cleanup-skill.js";
...
function freshWorkspace(): void {
  workspaceDir = join(
    tmpdir(),
    `vellum-migration-069-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe("workspace migration 069-release-notes-system-storage-cleanup-skill", () => {
