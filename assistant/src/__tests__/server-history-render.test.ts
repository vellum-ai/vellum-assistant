...
describe("mergeToolResults", () => {
  ...
  test("suppresses tool_result-only user messages from visible history", () => {
    ...
    expect(result.every((m) => !(m.role === "user" && m.text === ""))).toBe(
      true,
    );
  });
    ...
    const result = mergeToolResults([
      {
        ...
        timestamp: 1,
        toolCalls: [
          { name: "unknown", input: {}, result: "data", isError: false },
        ],
        toolCallsBeforeText: false,
        textSegments: ["user typed something"],
        contentOrder: ["text:0"],
        surfaces: [],
      },
    ]);
  ...
  test("does not mutate original messages", () => {
    const original = [
      ...
      },
      {
        ...
        timestamp: 2,
        toolCalls: [
          { name: "unknown", input: {}, result: "output", isError: false },
        ],
        toolCallsBeforeText: false,
        textSegments: [] as string[],
    ...
    expect(
      (original[0].toolCalls[0] as Record<string, unknown>).result,
    ).toBeUndefined();
  });
});
