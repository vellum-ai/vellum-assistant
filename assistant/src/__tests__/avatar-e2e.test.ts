...
/** Standard successful Vertex predict response (as returned by the runtime proxy). */
...
describe("avatar E2E integration", () => {
  ...
  test("managed success — correlation ID is propagated through the pipeline", async () => {
    ...
    // The correlation ID is generated by the client via crypto.randomUUID(),
    // which is mocked to return "00000000-0000-0000-0000-000000000000"
    const correlationLogged = logInfoCalls.some(([meta, message]) => {
      ...
      return (meta as { correlationId?: unknown }).correlationId != null;
    });
