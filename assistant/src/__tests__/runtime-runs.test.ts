...
describe('runtime runs — swarm lifecycle', () => {
  ...
  test('run transitions to completed after agent loop finishes', async () => {
    ...
    const run = await orchestrator.startRun('self', conversation.id, 'Build a feature');
  ...
  test('run transitions to failed when agent loop reports error', async () => {
    ...
    const run = await orchestrator.startRun('self', conversation.id, 'Run swarm');
  ...
  test('run enters needs_confirmation when tool requires approval', async () => {
    ...
    const run = await orchestrator.startRun('self', conversation.id, 'Delegate a swarm task');
  ...
  test('decision endpoint transitions run back to running', async () => {
    ...
    const run = await orchestrator.startRun('self', conversation.id, 'Run with approval');
    await new Promise((r) => setTimeout(r, 50));
  ...
  test('second run on busy session is rejected', async () => {
    ...
    await orchestrator.startRun('self', conversation.id, 'First run');
    await new Promise((r) => setTimeout(r, 20));

    // Second run on the same session should be rejected
    try {
      await orchestrator.startRun('self', conversation.id, 'Second run');
  ...
  test('principal context flows through to pending confirmation', async () => {
    ...
    const run = await orchestrator.startRun('self', conversation.id, 'Run with principal');
    await new Promise((r) => setTimeout(r, 50));
