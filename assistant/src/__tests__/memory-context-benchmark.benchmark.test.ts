...
function makeLongMessages(turns: number): Message[] {
  const rows: Message[] = [];
  const userTail = 'Need fast tests, deterministic memory recall, and stable prompt budgeting in long sessions.';
  const assistantTail = 'Tracked: compaction boundaries, recall sources, and quality constraints for incremental rollout.';
  for (let i = 0; i < turns; i++) {
    rows.push({
      role: 'user',
      content: [{ type: 'text', text: `[U${i}] ${userTail} Topic ${i % 9} branch codex/memory-${i}.` }],
    });
    rows.push({
      role: 'assistant',
      content: [{ type: 'text', text: `[A${i}] ${assistantTail} Result ${i % 7} with additional diagnostics.` }],
    });
  }
  return rows;
}
