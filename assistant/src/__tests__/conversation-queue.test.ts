...
): event is Extract<AssistantEvent, { type: "error" }> {
  return event.type === "error" && event.scope === "message";
}
...
describe("Conversation message queue", () => {
  ...
  test("[experimental] drain continues after a queued message fails to persist", async () => {
    ...
    const err2 = events2.find(isMessageFailure);
...
describe("Batched drain correctness fixes", () => {
  ...
  test("failed tail persist uses last-successful requestId", async () => {
    ...
    const errMid = events3.find(isMessageFailure);
  ...
  test("failed tail persist is excluded from fanOutOnEvent agent events", async () => {
    ...
    expect(events3.find(isMessageFailure)).toBeDefined();
  ...
  test("a failed queued message with nothing behind it ends the turn it was to start", async () => {
    ...
    expect(events2.find(isMessageFailure)).toMatchObject({
      type: "error",
      scope: "message",
      conversationId: "conv-1",
      requestId: "req-2",
  ...
  test("a failed batch head whose siblings start no turn ends the turn it was to start", async () => {
    ...
    expect(events3.find(isMessageFailure)).toMatchObject({
      type: "error",
      scope: "message",
    });
  ...
  test("failed batch member error names its requestId and clientMessageId", async () => {
    ...
    expect(events2.find(isMessageFailure)).toBeUndefined();
    expect(events4.find(isMessageFailure)).toBeUndefined();
  ...
  test("failed batch member with no nonce is still scoped to the message", async () => {
    ...
    const midError = events3.find(isMessageFailure);
...
describe("Conversation checkpoint handoff", () => {
  ...
  test("[experimental] queued persistence failure does not strand later messages", async () => {
    ...
    const errB = eventsB.find(isMessageFailure);
