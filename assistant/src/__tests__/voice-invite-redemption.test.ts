...
describe('redeemVoiceInviteCode', () => {
  ...
  test('mixed code lengths: longer input redeems shorter invite via prefix matching', () => {
    const phone = '+15551234567';
    const { code: shortCode } = createVoiceInvite({ callerPhone: phone, voiceCodeDigits: 6 });

    // Pass the 6-digit code padded with extra digits (simulating max-length
    // collection from DTMF when another invite has a longer code)
    const paddedInput = shortCode + '99';
  ...
  test('mixed code lengths: exact-length input redeems longer invite', () => {
    const phone = '+15551234567';
    const { code: longCode } = createVoiceInvite({ callerPhone: phone, voiceCodeDigits: 8 });
  ...
  test('mixed code lengths: both invites are reachable for the same caller', () => {
    const phone = '+15551234567';
    const { code: code6 } = createVoiceInvite({ callerPhone: phone, voiceCodeDigits: 6 });
    const { code: code8 } = createVoiceInvite({ callerPhone: phone, voiceCodeDigits: 8 });

    // Redeem the 8-digit invite with exact-length input
    const result8 = redeemVoiceInviteCode({
      callerExternalUserId: phone,
      sourceChannel: 'voice',
      code: code8,
    });
    ...
    // Redeem the 6-digit invite with the 6-digit code
    const result6 = redeemVoiceInviteCode({
      callerExternalUserId: phone,
      sourceChannel: 'voice',
      code: code6,
    });
