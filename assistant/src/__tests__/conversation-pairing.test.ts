...
/**
 * A pairing row for a guardian card is a control-plane artifact about a turn
 * running somewhere else, not the assistant speaking in the conversation it
 * lands in. It must be stamped `transcriptOnly` so the history load drops it:
 * unstamped, the card the pairing layer appends to the conversation a tool
 * approval came from is replayed between that turn's `tool_use` and its
 * `tool_result`, and history repair destroys the real result.
 */
describe("pairDeliveryWithConversation guardian-card history suppression", () => {
  beforeEach(() => {
    createConversationMock.mockClear();
    addMessageMock.mockClear();
    getConversationMock.mockClear();
  ...
  /** The options the pairing layer passed to its single `addMessage` write. */
  function soleWriteOptions(): Record<string, unknown> {
    expect(addMessageMock).toHaveBeenCalledTimes(1);
    return (addMessageMock.mock.calls[0]![3] ?? {}) as Record<string, unknown>;
  }

  test.each(["guardian.question", "ingress.access_request"])(
    "%s card row is stamped transcriptOnly",
    async (sourceEventName) => {
      ...
      const options = soleWriteOptions();
      expect(options.metadata).toEqual({ transcriptOnly: true });
      // Still excluded from recall, as every pairing row always was.
      expect(options.skipIndexing).toBe(true);
    },
  );

  test("guardian card pinned to its source conversation is stamped", async () => {
    // The shape that caused the incident: `buildVellumCardAffinity` pins the
    // vellum card to the conversation the approval came from, so the row is
    // appended to a live conversation whose turn is parked mid-approval.
    ...
    expect(result.conversationId).toBe("conv-source");
    expect(result.createdNewConversation).toBe(false);
    expect(soleWriteOptions().metadata).toEqual({ transcriptOnly: true });
  });

  test("a passive notification seed is not stamped and stays replayable", async () => {
    await pairDeliveryWithConversation(
      makeSignal({ sourceEventName: "schedule.notify" }),
      "vellum" as NotificationChannel,
      makeCopy(),
    );
