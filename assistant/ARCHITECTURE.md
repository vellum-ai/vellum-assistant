...
**Trust class enforcement:** A policy's `allowedTrustClasses` names whose turn may call a route, and an absent list means guardian only. A token whose scope profile is trust-checked (`isTrustCheckedScopeProfile` in `@vellumai/gateway-client`: every profile not on its exempt list, which is `contact_client_v1` today plus any unknown profile) reaches only a route whose list admits the trust class its principal resolves to on the `vellum-shared` channel; every other route answers 404, as an unmatched path does. The trust class is read per request on both paths, so a revoked contact is refused on its next request. The daemon's HTTP router and the gateway's IPC proxy both apply this rule (`contactTokenMayReachRoute` in `@vellumai/gateway-client`), since the daemon's IPC server runs no policy check of its own. Other profiles are not looked up.
