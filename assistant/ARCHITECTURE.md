...
**Verification telemetry:** the resource monitor reports every kernel OOM kill in the container as a `watchdog` event with `check_name` `oom_kill` (`src/monitoring/oom-kill-reporter.ts`). It scans `/dev/kmsg` when the cgroup `memory.events` kill counter moves, on its first tick after start (the kill that took the daemon down restarts the monitor with it), and once a minute as a fallback where the counter is unavailable. The `detail` bag lists each victim's comm, `oom_score_adj` and resident size, the cgroup memory figures at the time, and `killed_daemon`, which is the fleet-wide check that the priority policy above holds. A cursor file (kernel boot id plus last kmsg sequence) on the monitor data dir keeps a restarted monitor from re-reporting kills, and resets when the kernel reboots.
