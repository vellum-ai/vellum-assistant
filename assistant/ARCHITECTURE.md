...
| `contact_client_v1`  | `chat.write`, `approval.write`, `attachments.write`, `shared.read`                                                                                    | Non-guardian contact clients                 |
...
**Trust class enforcement:** A policy's `allowedTrustClasses` names whose turn may call a route, and an absent list means guardian only. A token whose scope profile is trust-checked (`isTrustCheckedScopeProfile` in `@vellumai/gateway-client`: every profile not on its exempt list, which is `contact_client_v1` today plus any unknown profile) reaches only a route whose list admits the trust class its principal resolves to on the `vellum-shared` channel. That class is read per request, so a revoked contact is refused on its next request. A trust-exempt token counts as the guardian with no lookup, so it reaches only a route whose list admits `guardian`, which every route naming no classes does. A refused caller gets 404, as an unmatched path does. The daemon's HTTP router and the gateway's IPC proxy both apply this rule (`tokenMayReachRoute` in `@vellumai/gateway-client`), since the daemon's IPC server runs no policy check of its own.

**Shared conversation reads:** A trusted contact reads the conversations shared with them through `GET /v1/shared/conversations`, `GET /v1/shared/conversations/:id` and `GET /v1/shared/conversations/:id/messages` (`src/runtime/routes/shared-conversation-routes.ts`). They require `shared.read`, which only `contact_client_v1` carries, and admit `trusted_contact` alone, so a guardian token gets 404. Each route checks the caller's live membership in `conversation_participants` first and answers 404 on a miss, whether or not the conversation exists. Metadata is an allowlist, and every message passes through the contact projection (`src/persistence/contact-visible-content.ts`), with rows that project to nothing left out. Stored text wrapped in an `<external_content>` fence is shown with the fence removed.

**Shared conversation writes:** A trusted contact sends with `POST /v1/shared/conversations/:id/messages`, which requires `chat.write` and admits `trusted_contact` alone. The route checks membership first (404 on a miss), refuses text longer than the untrusted-content fence keeps whole (400), and reads the contact's trust fresh on `vellum-shared` with that channel's admission floor applied (404 unless admitted). It then hands the message to `handleSendMessage` with a `ContactSender` and a conversation acquire that never creates a row, so the one send pipeline queues, deduplicates and fails over exactly as for `POST /v1/messages`. For a contact, that pipeline:

- never interrupts a running turn, so a contact's message always queues behind it;
- never answers or supersedes the guardian's pending interactions;
- runs no onboarding or greeting path;
- treats slash commands as message text;
- runs the turn with the contact's trust and principal.

The contact's text is fenced as untrusted input and stored fenced, so a reload keeps the fence. Display surfaces unwrap it. The client's `clientMessageId` reaches events unchanged, while the stored row and its duplicate check use a sender-scoped key.

When a queued contact message reaches the drain, `gateSharedSenderHead` (`src/daemon/shared-sender-queue-gate.ts`) checks the sender again. An admitted sender's queued messages run with the freshly resolved trust. A denied sender's messages are dropped. An unverifiable sender's messages wait with backoff while other senders proceed, and are dropped after 30 minutes. Steps that act for the guardian (slash commands, guardian verification setup, guardian replies, notification preferences) run only when `mayActForGuardian` admits the turn's trust. Every discarded contact message is closed out through `announceQueuedMessageDeleted`, which notes the sender so their own event stream forwards the `message_queued_deleted`.
