...
The A2A system enables two Vellum assistants to communicate securely over the internet. The daemon hosts the `A2AConnectionService` -- a stateless, surface-agnostic orchestration layer that ties together the peer connection store, handshake state machine, peer authentication, and message delivery.
...
- **Fail-closed trust**: Peer assistants receive the `peer_assistant` trust classification with zero default capabilities. The existing trust infrastructure (memory recall off, history filtered, tools denied) enforces the security boundary.
...
3. Runtime verifies HMAC signature, checks connection status
