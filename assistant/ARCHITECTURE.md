...
- `LiveVoiceCallTransport` — consumes the controller's token stream, runs segmented streaming TTS, and emits `tts_audio`/`tts_done` frames. A failed synthesis job is reported to the session (`onTtsFailure`) rather than to the client, and the queue keeps draining so the end-of-turn `tts_done` still lands.
...
Failure handling is split by scope. Turn-scoped failures never end the session: a turn retired without an assistant response is announced with a `turn_cancelled` frame carrying a machine `reason` (`empty_transcript`, `stt_failed` for a transcription error during the user's turn, `turn_failed` for a failed assistant dispatch, `tts_failed` for a failed synthesis — the latter aborts the in-flight generation like a barge-in so straggler audio can't follow), and the client resumes listening. `error` frames are reserved for session-fatal conditions — `credentials_missing`, protocol violations, and unrecoverable STT (no transcriber resolvable at all) — and tear the client session down. Session startup is gated by `resolveLiveVoiceCredentialReadiness()` — a missing STT or TTS credential fails the `start` frame with a `credentials_missing` error instead of failing mid-conversation.

A server-initiated end (`[END_CALL]` in the model's response, or the session-wide duration limit `liveVoice.maxSessionDurationSeconds` enforced by the controller's in-app timers) drains the queued goodbye TTS (bounded), sends a `session_ended` frame with the controller's reason, then closes the session and releases the manager slot.

The assistant-side live voice module is intentionally bounded under `src/live-voice/`:

| File                                 | Boundary                                                                                                                                                                                      |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `protocol.ts`                        | Provider-agnostic client/server frame types (incl. `turn_boundary`, `interrupted`, `turn_cancelled`, `session_ended`, mic-mode negotiation), validation, binary audio parsing, and sequencing |
| `live-voice-session-manager.ts`      | Single-active-session lock, session factory context, `onSessionEnded` release notification, and dispatch/release lifecycle                                                                    |
| `live-voice-session.ts`              | Duplex composition root: wires ingest → CallController → transport, owns outbound frame serialization, per-turn archiving/metrics, barge-in/cancellation frames, and close                    |
| `live-voice-ingest.ts`               | Inbound PCM16 pipeline: energy VAD, turn detection, tiered streaming STT with startup buffering and batch fallback                                                                            |
| `live-voice-transport.ts`            | `CallTransport` implementation: token stream → speakable segments → serialized streaming TTS → `tts_audio`/`tts_done` frames; TTS failures surfaced to the session                            |
...
Live voice STT uses the same `resolveStreamingTranscriber()` path as conversation streaming, in three tiers. Providers that support utterance-boundary finals run the `boundary` tier (one provider session for the whole live-voice session, finals arrive per utterance). Providers with plain streaming run the `plain` tier: live partials flow continuously, and the ingest forces each turn's final by cycling the provider session at every local turn boundary (stop flushes the final; a fresh session starts for the next turn, with the startup buffer covering the restart gap). When no streaming transcriber resolves (or the provider closes mid-session), the ingest falls back to per-turn batch transcription via `resolveBatchTranscriber()` — a batch-capable provider keeps live voice usable at higher latency. In every tier the ingest emits exactly one final (possibly empty) per turn boundary.
