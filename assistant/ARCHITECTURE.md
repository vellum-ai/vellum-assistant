...
| `/v1/integrations/slack/channel/config` | DELETE | Clears all Slack channel credentials (bot, app, and user tokens) from secure storage and credential metadata. Surgical user-token-only deletion is exposed via `clearSlackUserToken`, which the `credentials delete` route invokes when targeting `slack_channel`/`user_token` so the OAuth connection survives; it is not reachable through this HTTP endpoint. |
