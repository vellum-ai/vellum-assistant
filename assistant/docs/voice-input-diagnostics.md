...
- `voice_input_speech_started` includes `source: "provider"` when a Flux `StartOfTurn` triggers the client speech signal and any active-turn cancellation. `source: "local"` identifies an immediate local onset; guarded local interruptions use `voice_input_barge_in_fired`.
