...
The assistant image includes desktop system packages and the pinned WezTerm terminal. First use downloads Chrome into `external/desktop` on the persistent workspace; subsequent container replacements reuse it and `data/desktop-profile` without reinstalling system packages. Older images retain automatic system-package installation as a fallback. Image builds and the fallback installer share `src/desktop/desktop-system-packages.json`.
