...
| Discord   | `expectedExternalUserId` = Discord user snowflake, no `expectedChatId`           | The code is delivered to a DM and answered there, but the DM channel does not exist until it is opened, so there is no chat id to record at mint time. The requester's guild channel is deliberately not used: it is a room rather than an identity, and binding to it would accept the code from anyone standing in it. |
