...
An execute-mode run owns its automatic result notification, including results from background commands and subagents. The scheduler waits for the firing's own active turns, queued continuations, commands, child tasks, and command wakes to settle before marking the run successful and emitting `schedule.result`. Unrelated work in a reused conversation does not extend the wait. Ownership is recorded before asynchronous turn setup, covers dequeued messages while their processing claim and persistence are pending, and follows wakes through queueing, hydration, and execution. Scheduled dequeued dispatches retain a cancellation signal until the active turn takes over. Unscheduled dispatch tracking is observational. User interrupts preserve accepted user prompts until dispatch hands off to the active loop; started turns remain cancellable. Background continuations remain visible to the reply-notification gate with their original work timing, suppressing stale kickoff alerts without hiding replies to newer user requests. Quiet runs skip this fallback, and explicit successful delivery suppresses duplicates.
