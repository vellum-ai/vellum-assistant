 ...
 * produced as `latestMessages` and resetting it to the user's original
 * `messages`. The net effect is "submit the user's pristine prompt list
 * — drop every prior injection / repair / overflow rewrite".
 *
 ...
 * would return `{ ...ctx, latestMessages: [...ctx.messages] }`.
 *
...
export default async function userPromptSubmit(
  ...
  ctx.latestMessages.push(...ctx.messages);
}
