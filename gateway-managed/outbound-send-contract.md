...
Response contract:
- `202`: accepted send envelope with `route_id`, `assistant_id`, `provider`, `route_type`, `identity_key`, optional `delivery_id`, and optional `provider_status`
...
- `503`: error envelope when managed Twilio provider credentials/config are missing
